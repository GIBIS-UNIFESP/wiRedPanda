...
SignalModel::SignalModel( int rows, int inputs, int columns, QObject *parent ) :
  QStandardItemModel( rows, columns, parent ), inputs( inputs ) {
}
...
SignalDelegate::SignalDelegate( int margin, QObject* parent ) : QItemDelegate( parent ), m_margin( margin ) {
}
...
BewavedDolphin::BewavedDolphin( Editor *editor, QWidget *parent ) :
  QMainWindow( parent ), ui( new Ui::BewavedDolphin ), editor( editor ), mainWindow( dynamic_cast< MainWindow* >( parent ) ), type( PlotType::line ) {
  scale = 1.0;
  ui->setupUi( this );
  resize( 800, 500 );
...
void BewavedDolphin::on_actionSave_as_triggered( ) {
  ...
  if( save( fname ) ) {
    ...
    if( mainWindow->getDolphinFilename( ) != fname ) {
      ...
      reply = QMessageBox::question( this, tr( "Wired Panda - Bewaved Dolphin" ), tr( "Do you want to link this Bewaved Dolphin file to your current Wired Panda file and save it?" ), QMessageBox::Yes | QMessageBox::No );
...
void BewavedDolphin::on_actionLoad_triggered( ) {
  ...
  QString fname = QFileDialog::getOpenFileName( this, tr( "Open File" ), defaultDirectory.absolutePath( ), tr( "All supported files (*.dolphin *.csv);;Dolphin files (*.dolphin);;CSV files (*.csv)" ) );
  if( fname.isEmpty( ) ) {
    return;
  }
  load( fname );
