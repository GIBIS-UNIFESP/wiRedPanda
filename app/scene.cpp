...
void Scene::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (auto *remoteDevice = dynamic_cast<Remote *>(itemAt(m_mousePos))) {
        openConfigAction();
    }
...
void Scene::openConfigAction()
{
    ...
    if (elms.size() != 1) {
        throw Pandaception(tr("Unable to trigger configure for more than a remote element at once."));
    }

    RemoteConfig remoteConfig(nullptr, elms.first());
    remoteConfig.start();
}
