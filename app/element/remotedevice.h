...
public:
    ...
    bool isAlive() const { return (static_cast<uint64_t>(QDateTime::currentMSecsSinceEpoch() / 1000) < (getAliveSince() + 10)); }
    ...
    void initTimeCount() {
        startedTimeEpoch = QDateTime::currentMSecsSinceEpoch() / 1000;
    ...
    uint32_t getPinId(const std::string& name, uint8_t pinType) {
        ...
        for (it = list.begin(); it != list.end() ; ++it) {
            ...
            if (p.getName().compare(name) == 0) {
                if (static_cast<Pin_Type>(p.getType()) == Pin_Type::General_Purpose || static_cast<int>(p.getType()) == static_cast<int>(pinType)) {
    ...
    bool mapPin(const std::string& name, uint8_t pinType) {
        ...
        if (id != 0) {
            ...
            if (static_cast<Pin_Type>(pinType) == Pin_Type::Input) { mappedInputs[id] = false; }
            if (static_cast<Pin_Type>(pinType) == Pin_Type::Output) { mappedOutputs[id] = false; }
            return true;
        }
    ...
    void setIsInQueue(bool boolean) {
        inQueue = boolean;
        queueWaitingSinceEpoch = QDateTime::currentMSecsSinceEpoch() / 1000;
    }
...
public:
    ...
    void setSkin(bool defaultSkin, const QString& filename) override { Q_UNUSED(defaultSkin) Q_UNUSED(filename) };
