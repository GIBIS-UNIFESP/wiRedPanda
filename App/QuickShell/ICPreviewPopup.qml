...
Item {
    ...
    // Mirrors ICPreviewPopup::enterEvent()/leaveEvent(): moving the cursor off the IC and onto
    // the popup itself cancels the pending auto-hide, letting it stay open indefinitely while
    // the cursor rests on it, instead of vanishing under the cursor on the original fixed timer.
    HoverHandler {
        onHoveredChanged: {
            if (hovered) {
                root.preview.cancelPendingHide();
            } else {
                root.preview.resumePendingHide();
            }
        }
    }
