 ...
 *
 * \details Every function here mirrors its Commands.cpp namesake, substituting CanvasItem for
 * Scene: findItems()/findElements()/findElm()/findConn() resolve through CanvasItem::
 * itemById(); addItems()/deleteItems() go through CanvasItem::addItem()/removeItem() (which
 * do double duty as both the id-registration and the m_elements/m_connections membership
 * half, see CanvasItem.h's doc comment on addItem()); loadItems()/saveItems()/loadList() are
 * otherwise unmodified, built entirely on the already-portable Serialization/
 * SerializationContext machinery. Unlike Commands.cpp's own real findItems()/findElements(),
 * a missing id is silently skipped rather than thrown via PANDACEPTION (Scene's Sentry
 * error-reporting infrastructure isn't wired into this Quick-only tree).
 *
 * Uses ItemWithId* throughout, not QGraphicsItem* -- this canvas's domain classes are not
 * QGraphicsItem-derived.
 */
namespace CanvasCommandUtils {
    ...
    /// Removes (canvas->removeItem()) and deletes every Connection currently attached to any
    /// of \a elm's ports, on both canvas's registry and the port itself. Callers that are about
    /// to call elm->load()/loadFromBlob() on an already-canvas-registered element MUST drain
    /// its connections this way first: an old Port's destructor drains (deletes) whatever
    /// Connection is still attached to it as an ordinary side effect of being replaced, but
    /// never tells canvas -- leaving canvas->connections() holding a dangling pointer until
    /// something (typically the next rebuildSpatialIndex(), computing a wire's paint stroke)
    /// dereferences it. Same reasoning already applied by CanvasChangePortSizeCommand's own
    /// drainPortConnections() call before it changes a port count directly.
    void drainAllConnections(GraphicElement *elm, CanvasItem *canvas);
 ...
 * connection, splitting it into two wire segments. Picks the canvas's deferred wire-splitting
 ...
 * while preserving connections (used by nextElm()/prevElm()'s type-cycling shortcuts).
 */
 ...
 * TruthTable element. Command logic only -- its real UI trigger (a cell click in a future
 * TruthTable editor dialog) is chrome, not exercised here.
 */
