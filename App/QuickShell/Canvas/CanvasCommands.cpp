...
void drainAllConnections(GraphicElement *elm, CanvasItem *canvas)
{
    for (int i = 0; i < elm->inputSize(); ++i) {
        // while(), not range-for: connections() returns a live reference to the port's own
        // list, and deleting a Connection detaches it from both ports (mutating that same
        // list mid-iteration) -- same hazard Port::drainConnections() itself guards against.
        auto *port = elm->inputPort(i);
        while (!port->connections().isEmpty()) {
            auto *conn = port->connections().constFirst();
            canvas->removeItem(conn);
            delete conn;
        }
    }
    for (int i = 0; i < elm->outputSize(); ++i) {
        auto *port = elm->outputPort(i);
        while (!port->connections().isEmpty()) {
            auto *conn = port->connections().constFirst();
            canvas->removeItem(conn);
            delete conn;
...
void CanvasUpdateCommand::refreshRuntimeState()
{
    ...
    // visibly wrong once a chrome property panel actually drives this command (a later phase).
    m_canvas->update();
}
...
void CanvasUpdateBlobCommand::loadData(QByteArray &itemData)
{
    ...
    // Real, confirmed bug found 2026-08-31 while porting TestICInline's
    // testOnChildICBlobSavedPortCountWithConnections() (a genuine use-after-free, not a test
    // artifact -- crashed inside rebuildSpatialIndex()'s wire-stroke pass, called from the very
    // next restartSimulation()): an IC's load() tears down and rebuilds its own ports, and each
    // old Port's destructor drains (deletes) whatever Connection is still attached to it --
    // exactly like CanvasChangePortSizeCommand's own port-shrink path, which is why THAT command
    // pre-drains via CanvasCommandUtils::drainPortConnections() before ever touching port counts.
    // This command called elm->load() directly with no equivalent step. See
    // CanvasCommandUtils::drainAllConnections()'s own doc comment; reconnectConnections() (via
    // the m_connections captured *before* this call) recreates whichever of them still fit the
    // new port layout, so nothing here needs those pointers to survive past this loop.
    for (auto *elm : elems) {
        CanvasCommandUtils::drainAllConnections(elm, m_canvas);
    }
