                                                        ...
                                                        const QString &dir, const QString &filter)
{
    ...
    if (!dir.isEmpty()) {
        // dir may be a bare directory or a full file path (both are real call shapes
        // WorkspaceManager.cpp's WorkSpace open/save-as ports will use, matching
        // QFileDialog::getSaveFileName()'s own dir semantics) -- QFileInfo::absolutePath()
        // alone would wrongly treat a bare directory's last path component as a filename, so
        // branch on whether it actually is one.
