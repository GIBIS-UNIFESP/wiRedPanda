...
void QuickExerciseController::close()
{
    ...
    // ExerciseEngine::stop() is a no-op once markCompleted() has already deactivated the engine
    // (StepEngineCore::stop()'s own "already inactive" guard) -- it never emits exerciseStopped(),
    // so onExerciseStopped() never runs. Closing a *completed* exercise must still work: mirrors
    // ExerciseOverlay's closeRequested handler (MainWindow.cpp), which always tears the overlay
    // widget down regardless of what stop() itself decided to do internally.
    if (m_completed) {
        m_completed = false;
