...
{
    // m_canvas is a QPointer, not a plain pointer: QuickWorkspaceManager::closeTab() erases
    // (and destroys) the previous tab's QuickWorkSpace -- and its CanvasItem -- before
    // currentTabChanged fires for the new current tab (same ordering
    // QuickAppController::m_boundCanvas already needed QPointer for). Without it, `if
    // (m_canvas)` would still be true against a dangling pointer, and disconnect() would
...
void QuickElementEditor::refresh()
{
    ...
    // Mirrors applyCapabilitiesToUi()'s m_manyLabels placeholder -- shown only until the field is
    // actually edited (setLabel() below overwrites m_label with the real typed text on the very
    // next keystroke, same as audioBoxFileName()'s identical placeholder-until-touched shape,
    // just computed once here instead of live in a getter since this field is editable).
    m_label = m_caps.hasLabel ? (m_caps.hasSameLabel ? firstElement->label() : tr("<Many labels>")) : QString();
    m_labelDirty = false;
    ...
    // Mirrors applyCapabilitiesToUi()'s m_manyTriggers placeholder -- see m_label's identical
    // comment above for why this is safe against setTrigger() overwriting it on the first edit.
    m_trigger = m_caps.hasTrigger ? (m_caps.hasSameTrigger ? firstElement->trigger().toString() : tr("<Many triggers>")) : QString();
...
QList<GraphicElement *> QuickElementEditor::readingOrder(QList<GraphicElement *> elements)
{
    ...
    // left-to-right X order. Mirrors ElementTabNavigator::readingOrder() exactly.
...
namespace {
/// Maps cycleSelection()'s \a fieldKey to the SelectionCapabilities flag ElementTabNavigator's
/// own installed-widget list checks implicitly via widget->isEnabled() (each Widgets field's
/// enabled/visible state is itself driven by the matching capability, in
/// ElementEditor::applyCapabilitiesToUi()).
bool hasFieldCapability(const SelectionCapabilities &caps, const QString &fieldKey)
{
    if (fieldKey == QLatin1String("label")) return caps.hasLabel;
    if (fieldKey == QLatin1String("color")) return caps.hasColors;
    if (fieldKey == QLatin1String("audio")) return caps.hasAudio;
    if (fieldKey == QLatin1String("volume")) return caps.hasVolume;
    if (fieldKey == QLatin1String("frequency")) return caps.hasFrequency;
    if (fieldKey == QLatin1String("delay")) return caps.hasDelay;
    if (fieldKey == QLatin1String("inputSize")) return caps.canChangeInputSize;
    if (fieldKey == QLatin1String("outputSize")) return caps.canChangeOutputSize;
    if (fieldKey == QLatin1String("outputValue")) return caps.hasLatchedValue;
    if (fieldKey == QLatin1String("locked")) return caps.hasOnlyInputs;
    if (fieldKey == QLatin1String("trigger")) return caps.hasTrigger;
    if (fieldKey == QLatin1String("wirelessMode")) return caps.hasWirelessMode;
    return false;
}
} // namespace

void QuickElementEditor::cycleSelection(bool forward, const QString &fieldKey)
{
    ...
    const qsizetype startPos = elements.indexOf(m_elements.constFirst());
    if (total == 0 || startPos < 0) {
        return;
    }

    const qsizetype step = forward ? 1 : -1;
    qsizetype pos = (total + startPos + step) % total;
    for (; pos != startPos; pos = (total + pos + step) % total) {
        if (hasFieldCapability(computeCapabilities({elements.at(pos)}), fieldKey)) {
            break;
        }
    }

    if (pos == startPos) {
        return; // wrapped all the way around -- no other element exposes this field
    }

    auto *target = elements.at(pos);
    m_canvas->selectOnly(target);
    m_canvas->centerOn(target->pos());
}
...
void QuickElementEditor::apply()
{
    ...
    if (m_caps.hasWirelessMode) {
        for (auto *elm : std::as_const(m_elements)) {
            ...
            // Known, recurring GCC 15 -Wnull-dereference false positive under an optimized
            // preset (-O2/-O3): iterating the QVector<GraphicElement*> CanvasItem::elements()
            // returns trips it purely from QList::begin()/end()'s own inlined internals, not
            // from anything this loop does -- see project memory
            // project_release_null_deref_quickappcontroller.md.
...
#pragma GCC diagnostic pop
    ...
    // A frequency/delay/locked change above may have moved a tracked Clock's next deadline
    // earlier, later, or removed it entirely -- re-derive Simulation's scheduled wake so it
    // doesn't fire on a now-stale deadline (or stay silent for a clock that just got
    // unlocked/sped up). Cheap and correctly a no-op when nothing relevant changed or the
