...
public:
    ...
    /// Clamps \a candidate (a minimap geometry -- either the persisted one being restored, or
    /// the widget's own current live geometry) into \a parentWidth x \a parentHeight, respecting
    /// the same margin/minimum-size constants Minimap.qml's declared defaults use. Mirrors
    /// WorkSpace::applyMinimapGeometry()'s shared clamp block (both its one-time-restore and
    /// subsequent-resize branches use this identical math) -- kept here, not in QML, since it's
    /// plain deterministic geometry math with no interactivity of its own, unlike the
    /// mouse-drag resize/move math Minimap.qml legitimately owns. Uses qreal throughout (not
    /// WorkSpace's int/qBound\<int\>) to match Item::x/y/width/height's own qreal type exactly.
    /// Width/height are clamped first, then x/y are clamped using those already-clamped
    /// width/height values (not the candidate's original ones) -- both Minimap.qml's own
    /// clampToParent() and WorkSpace::applyMinimapGeometry() rely on this exact order.
    Q_INVOKABLE QRectF clampGeometry(const QRectF &candidate, qreal parentWidth, qreal parentHeight) const;
...
private:
    ...
    /// QPointer, not a raw CanvasItem*: QuickWorkspaceManager::closeTab() erases (and destroys)
    /// the current tab's QuickWorkSpace -- and its CanvasItem -- before currentTabChanged fires
    /// for the new current tab. Without it, `if (m_canvas)` in setCanvas() would still be true
    /// against an already-freed pointer, and disconnect() would dereference it -- the same
    /// failure mode QuickElementEditor::m_canvas/QuickICPreview::m_canvas guard against.
