...
{
    // m_canvas is a QPointer for the same reason QuickElementEditor::m_canvas is: closeTab()
    // erases (and destroys) the previous tab's CanvasItem before currentTabChanged fires for
    // the new current tab, so a plain pointer here would already be dangling by the time this
    // runs -- disconnect() below would dereference freed memory without the QPointer's
    // automatic self-nulling.
    if (m_canvas) {
