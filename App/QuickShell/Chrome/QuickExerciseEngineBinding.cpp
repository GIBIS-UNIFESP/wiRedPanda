 ...
 * wiredpanda, so ExerciseEngine.cpp itself (whole-archived Layer 1 code, compiled once
...
void ExerciseEngine::setCanvas(CanvasItem *canvas)
{
    ...
    if (canvas) {
        ...
        // (-O2/-O3): copying the QVector<GraphicElement*> CanvasItem::elements() returns trips
        // it purely from QList's own inlined copy-constructor internals -- see project memory
