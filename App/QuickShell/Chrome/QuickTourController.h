 ...
 * ExerciseEngine): TourEngine has zero Scene/Widgets coupling at all (confirmed by reading its
 * full header/implementation), so there's no CanvasItem-binding split to route around --
 * see project memory project_shared_class_quick_extension_pattern.md for why ExerciseEngine
 * needed one and this class doesn't.
 *
...
signals:
    ...
    /// MainWindow.cpp's own explicit "click executes before resolveTourTarget computes widget
    /// rects" connection-ordering comment. This is a real emission-order guarantee, not a
    /// timing assumption: onEngineStepChanged() (below) emits this signal and returns from its
    /// QML-side Connections handler -- a direct, same-thread connection, so fully synchronous --
    /// *before* it calls applyStepData(), which is what actually updates currentTarget and
    /// emits stepChanged(). By the time any QML binding reads currentTarget, this step's clicks
    /// (e.g. a palette tab switch) have already been dispatched and applied.
