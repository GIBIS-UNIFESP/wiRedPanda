...
{
    // m_canvas has no QQuickItem parent at construction (Main.qml's canvasHost reparents it
    // later, per-tab-switch) -- an object with no parent that first crosses into QML/JS (e.g.
    // via canvas() below, called from Main.qml's showCurrentTab()) defaults to
    // JavaScriptOwnership, and setting a parent afterward does NOT retroactively change that
    // decision. Without this call, QML's garbage collector could delete m_canvas out from
    // under this unique_ptr at any point during normal use. Same fix technique as
    // AppControllerForeign::create()'s explicit CppOwnership call.
...
void QuickWorkSpace::autosave()
{
    ...
    // Choose the directory to write into: unsaved projects go to the global autosaves dir,
    // saved projects go next to their .panda file unless that directory is read-only OR is
    // the bundled Examples directory. Writing beside the file is deliberate (an autosave
    // stays with the project it protects), but the bundled examples are shipped content, not
    // the user's project: an installed copy is usually read-only and already took the
    // fallback, while a dev checkout's Examples/ is writable and would otherwise accumulate
    // hidden .<name>.<uuid>.panda files that .gitignore explicitly un-ignores. Mirrors
    // WorkSpace::autosave()'s identical guard.
    QDir path;
...
void QuickWorkSpace::onChildICBlobSaved(int icElementId, const QByteArray &blob)
{
    ...
    try {
        for (auto *target : targets) {
            // Must run before loadFromBlob(): see CanvasCommandUtils::drainAllConnections()'s
            // own doc comment -- loadFromBlob() tears down and rebuilds target's ports, and an
            // old port's destructor cascades-deletes whatever Connection is still attached to
            // it without ever telling this canvas, leaving m_canvas->connections() holding a
            // dangling pointer. reconnectConnections() (via the `connections` snapshot captured
            // above, before any of this ran) is what rebuilds whichever of them still fit once
            // the CanvasUpdateBlobCommand pushed below runs its own redo().
            CanvasCommandUtils::drainAllConnections(target, m_canvas.get());
