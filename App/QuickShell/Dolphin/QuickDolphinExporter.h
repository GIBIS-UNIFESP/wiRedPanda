 ...
 * \brief Model -> image conversions for the beWavedDolphin waveform export paths, standing in
 * for DolphinExporter::renderToPixmap()/exportToPng()/exportToPdf().
 *
 * \details DolphinExporter::renderToPixmap() (App/BeWavedDolphin/DolphinExporter.cpp) constructs
 * a real, throwaway QTableView to rasterize -- a real QWidget needs a live QApplication, not
 * just wiredpanda's QGuiApplication. This namespace reimplements the same visual output via
 * direct QPainter calls over the model instead, the same substitution
