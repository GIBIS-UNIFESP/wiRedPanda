...
void QuickDolphinController::loadNewTable(const QStringList &inputLabels, const QStringList &outputLabels)
{
    ...
    m_selectionAnchor = QModelIndex();
    m_selectionModel = new QItemSelectionModel(m_model, this);
    connect(m_selectionModel, &QItemSelectionModel::selectionChanged, this, &QuickDolphinController::hasSelectionChanged);
    // Mirrors BewavedDolphin::on_tableView_selectionChanged(): clear the main canvas's selection,
    // then highlight the input element for each selected row (output rows have none), giving the
    // user visual feedback about which circuit component a waveform row corresponds to.
...
void QuickDolphinController::run()
{
    ...
    // canvas since createWaveform() ran -- the main canvas and this window coexist, and nothing
    // blocks editing the former while the latter is open (mirrors BewavedDolphin::run()'s
    // identical elementsStillLive() guard and its own doc comment on why).
    ...
    m_simDriver->sweep(
        m_rows, m_canvas->elements(), m_model->columnCount(),
...
void QuickDolphinController::selectCell(int row, int column, Qt::KeyboardModifiers modifiers)
{
    ...
    if (modifiers.testFlag(Qt::ShiftModifier) && m_selectionAnchor.isValid()) {
        const QModelIndex topLeft = m_model->index((std::min)(m_selectionAnchor.row(), row), (std::min)(m_selectionAnchor.column(), column));
        const QModelIndex bottomRight = m_model->index((std::max)(m_selectionAnchor.row(), row), (std::max)(m_selectionAnchor.column(), column));
        // Real QAbstractItemViewPrivate::extendedSelectionCommand() returns SelectCurrent for
        // Shift, never ClearAndSelect -- not just Select: the Current flag matters too.
        // Without it, QItemSelectionModel::select()
        // permanently finalizes (bakes into d->ranges) the *previous* provisional selection on
        // every subsequent call, so a second consecutive Shift-click could only ever grow the
        // selection, never shrink it back down from a wider range to a narrower one anchored at
        // the same start cell. SelectCurrent keeps each Shift-click's range provisional and
        // replaceable, exactly like real Qt, while still unioning with (not clearing) whatever a
        // prior Ctrl-click already selected.
        m_selectionModel->select(QItemSelection(topLeft, bottomRight), QItemSelectionModel::SelectCurrent);
        return;
    }
...
void QuickDolphinController::saveAs()
{
    ...
    // Same fallback chain as load() above (m_currentFile -> m_host->currentFile() -> home), for
    // a waveform that has never been saved -- mirrors BewavedDolphin::on_actionSaveAs_triggered().
    QString initialPath = m_currentFile.absoluteFilePath();
    if (initialPath.isEmpty()) {
        initialPath = (m_host && m_host->currentFile().exists()) ? m_host->currentDir().absolutePath() : QDir::homePath();
    }

    const auto result = FileDialogs::provider()->getSaveFileName(nullptr, tr("Save File as..."), initialPath, fileFilter);
...
void QuickDolphinController::load()
{
    ...
    // fallback chain (m_currentFile -> m_host->currentFile() -> QDir::homePath()).
    QString defaultDir;
...
bool QuickDolphinController::elementsStillLive() const
{
    ...
    // (-O2/-O3): copying the QVector<GraphicElement*> CanvasItem::elements() returns trips it
    // purely from QList's own inlined copy-constructor internals.
...
void QuickDolphinController::saveToTxt(QTextStream &stream) const
{
    ...
    const QVector<Status> oldInputValues = WaveformSimulator::captureInputs(m_inputs, m_inputPorts);
    {
        ...
        m_simDriver->sweep(
            m_rows, m_canvas->elements(), columns,
