 ...
 *
 * fitScreen() is a deliberate simplification of DolphinZoom::fitScreen()'s iterative fixed-point
 * convergence: the original loop exists because QHeaderView's real sizeHint() (which depends on
 * the *new* font) is available synchronously mid-computation, so it can re-solve against the
 * post-resize header size within the same call. Quick's HorizontalHeaderView/VerticalHeaderView
 * resize through the normal property-binding/layout pass instead (not synchronously queryable
 * mid-call), so this version is single-pass: the caller supplies the *current* (pre-update)
 * header sizes as an estimate. This can under/overshoot slightly on an extreme single zoom jump
 * (e.g. resetZoom() immediately followed by fitScreen() on a very differently-sized table) --
 * a real, accepted, documented behavioral simplification, not a silent bug -- verify empirically
 * (screenshot before/after Fit Screen) rather than assuming parity.
 */
class QuickDolphinZoom
{
public:
