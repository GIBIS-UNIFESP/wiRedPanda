    ...
    : QGraphicsPathItem(parent)
{
    ...
    updateTheme();
    // Start in the Unknown visual state; the wire colour is updated once both ports attach.
...
{
    // Detach from the previous port before attaching to the new one to avoid dangling
    // entries in the old port's connection list (e.g. during IC hot-reload).
...
{
    // Skip expensive Bézier path construction when there is no visible rendering. In
    // non-interactive (test/headless) mode, connection geometry is never painted, so
    // building and comparing QPainterPaths is pure waste.
    ...
    // (small Y fraction) and curves gently in the middle (0.25/0.75 X fraction). This
    // gives an S-curve appearance typical of schematic wire-routing tools.
...
double Connection::angle()
{
    ...
    if (port1 && port2) {
        ...
        // QLineF::angle() measures from the positive X axis counter-clockwise, so the
...
void Connection::setStatus(const Status status)
{
    ...
    // Error wires are drawn thicker (5 px) to draw attention to the problem; other wires
    // are thinner (3 px) to reduce visual clutter during simulation. Unknown (undriven)
    // wires use a distinct gray from Error (red).
    switch (status) {
...
void Connection::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    // Highlight is drawn as a wider blue halo beneath the normal wire, so users can easily
    // see which wires belong to a selected element.
    if (m_highLight) {
        painter->save();
    ...
    // When the wire itself is selected (clicked), switch to the selection colour; otherwise
    // use the status-driven pen set by setStatus().
...
{
    // When the wire is selected/deselected, visually highlight both endpoint ports so the
    // user can see which element pins are connected by this wire.
...
{
    // Expand beyond the path's tight bounding box by 10 px on all sides to ensure the
    // thick selection outline and highlight halo are fully covered during repaints.
    return path().boundingRect().adjusted(-10, -10, 10, 10);
}

bool Connection::sceneEvent(QEvent *event)
{
    // Swallow Ctrl+click so the scene can use Ctrl+click for multi-selection without the
    // wire consuming the event and blocking the rubber-band/deselect behaviour.
