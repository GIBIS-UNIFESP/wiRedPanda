...
void MainWindow::setupConnections()
{
    ...
    connect(m_ui->elementEditor, &ElementEditor::editSubcircuitRequested, this, [this](const QString &blobName, int icElementId) {
        if (m_currentTab) {
            Q_ASSERT(m_currentTab->scene());
            Q_ASSERT(m_currentTab->scene()->icRegistry());
...
{
    Q_ASSERT(!blobName.isEmpty());
    Q_ASSERT(!blob.isEmpty());
    if (!m_currentTab) {
        return;
    }
    ...
    Q_ASSERT(parentWorkspace);

    createNewTab();
    Q_ASSERT(m_currentTab);

    Q_ASSERT(parentWorkspace->scene());
