...
void ElementEditor::updateElementAppearance()
{
    ...
    if (m_ui->widgetAppearanceStates->isVisible() && m_ui->buttonGroupAppearanceStates->checkedButton()) {
        ...
        if (needsMacro) {
            m_scene->undoStack()->beginMacro(i18n("Change appearance")); // LCOV_EXCL_LINE — the tile grid (widgetAppearanceStates) is only ever made visible for a single-element selection (see applyCapabilitiesToUi()'s `m_elements.size() == 1` guard), so entering this branch structurally guarantees m_elements.size() == 1, making needsMacro permanently false.
        }
