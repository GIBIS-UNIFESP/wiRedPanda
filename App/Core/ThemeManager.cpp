...
void ThemeAttributes::setTheme(const Theme theme)
{
    ...
    if (theme == Theme::System) {
        ...
        // Theme-invariant hover-label chip colors (see the shared comment on the
        // Light/Dark branches below) — deliberately not derived from KColorScheme
        // so the floating hover label always reads clearly regardless of the
        // active color scheme.
