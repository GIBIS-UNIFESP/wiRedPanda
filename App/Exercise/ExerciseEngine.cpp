...
bool ExerciseEngine::loadFromResource(const QString &resourcePath)
{
    ...
    m_title = root.value("title").toString();
    ...
    for (const QJsonValue &stepVal : stepsArray) {
        ...
        ExerciseStep step;
        step.instruction = stepObj.value("instruction").toString();
        step.hint        = stepObj.value("hint").toString();
        ...
        step.show    = stepObj.value("show").toVariant().toStringList();
        step.hide    = stepObj.value("hide").toVariant().toStringList();
...
bool ExerciseEngine::validateConnections(const QVector<ExerciseConnectionRequirement> &reqs) const
{
    ...
    for (const ExerciseConnectionRequirement &req : reqs) {
        ...
        for (const GraphicElement *elm : elements) {
            ...
            for (const QNEInputPort *port : elm->inputs()) {
                for (const QNEConnection *conn : port->connections()) {
                    if (conn->startPort()
