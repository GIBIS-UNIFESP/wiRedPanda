...
class GraphicElement : public QGraphicsObject, public ItemWithId
{
    ...
    /// Persistence is delegated to GraphicElementSerializer, which reaches this element's
    /// ports/orientation/appearance/label/trigger state through this friendship.
...
public:
    ...
    InputPort *inputPort(const int index = 0);

    /// Returns the output port at \a index (default 0).
    OutputPort *outputPort(const int index = 0);
    ...
    /// Re-derives the displayed pixmap from the current orientation (rotation/flip).
    /// Internal collaboration point invoked by ElementOrientation after it changes the
    /// item transform; mirrors how ElementAppearance reads orientation back via getters.
...
protected:
    ...
    /// forwards its port-access interface here and keeps the polymorphic geometry layout.
...
private:
    ...
    /// Owns the rotation angle + flip flags and the transform math that applies them to the
    /// item and its ports; this element forwards its rotation/flip interface here.
