...
{
    // Resolve the guarded pointer once: QPointer::operator-> re-checks the target
    // on every access, so repeated m_pendingIC-> uses read as a possible null
    // dereference under optimization (-Werror=null-dereference). A single raw
    // pointer after the guard is provably non-null for the rest of the function.
    IC *ic = m_pendingIC;
    if (!ic) {
        return;
    }
    ...
    const QString title = ic->displayName();
    const QPixmap &preview = ic->previewPixmap();
