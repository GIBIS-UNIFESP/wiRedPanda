...
void ICLoader::loadFile(IC &ic, const QString &fileName, const QString &contextDir)
{
    ...
    if (auto *scene_ = qobject_cast<Scene *>(ic.scene())) {
        ...
        if (!cached.isEmpty()) {
            ...
            ic.setToolTip(fileInfo.fileName());
            if (ic.label().isEmpty()) {
...
void ICLoader::loadFileDirectly(IC &ic, const QFileInfo &fileInfo)
{
    ...
    if ((preamble.version < FormatRev::current) && Application::migrationEnabled) {
        migrateFile(ic, fileInfo, items, preamble.version, fileRegistry);
    }
...
void ICLoader::migrateFile(IC &ic, const QFileInfo &fileInfo, const QList<QGraphicsItem *> &items,
...
void ICLoader::processLoadedItems(IC &ic, const QList<QGraphicsItem *> &items)
{
...
void ICLoader::loadFromBlob(IC &ic, const QByteArray &blob, const QString &contextDir)
{
    ...
    if (!ic.m_blobName.isEmpty()) {
        ic.setToolTip(ic.m_blobName);
    }
