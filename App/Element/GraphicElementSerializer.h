 ...
 * element's ports/orientation/appearance/label/trigger state through that friendship. The
 * element keeps thin virtual save()/load() entry points that delegate here, so subclasses
 * (e.g. IC) can still extend the load via their override. All methods are static and take
 * the element explicitly; no serializer instance state is needed.
 */
class GraphicElementSerializer
{
