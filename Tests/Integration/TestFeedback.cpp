...
{
    // NOT gate connected to itself (single element feedback)
    Scene scene;
...
void TestFeedback::verifyConvergence(Scene *scene, bool shouldConverge)
{
    ...
    if (shouldConverge) {
        ...
        QHash<LogicElement *, Status> snapshot;
        for (auto *elem : scene->elements()) {
            if (elem->logic()) {
        ...
        for (auto *elem : scene->elements()) {
            if (elem->logic()) {
                Status currentValue = elem->logic()->outputValue(0);
                Status snapshotValue = snapshot.value(elem->logic(), currentValue);
