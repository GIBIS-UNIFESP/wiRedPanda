    ...
    Simulation *sim)
{
    ...
    const int numDataBits = roles.dataIdxs.size();
...
void TestSystemVerilogExport::testSystemVerilogExportHelper(const QString &icFile)
{
    ...
    if (!seqSteps.isEmpty()) {
        ...
        const int numInputs = switches.size();
        const int numOutputs = leds.size();
        ...
        if (!testVectors.isEmpty()) {
            ...
            if (tbFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                for (int v = 0; v < testVectors.size(); ++v) {
                    ...
                    for (int i = inputPins.size() - 1; i >= 0; --i) {
                        inputNames << inputPins[i].m_varName;
                    }
                    tb << inputNames.join(", ") << "}, {";

                    QStringList outputNames;
                    for (int o = outputPins.size() - 1; o >= 0; --o) {
...
{
    testSystemVerilogExportHelper("level5_stack_memory_interface.panda");
}
