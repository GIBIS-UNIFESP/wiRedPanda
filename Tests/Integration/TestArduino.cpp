...
void TestArduino::testCodeStructure()
{
    ...
    auto setupPos = content.indexOf("void setup()");
    auto loopPos = content.indexOf("void loop()");
...
void TestArduino::testICElementError()
{
    ...
    try {
        ArduinoCodeGen generator(filePath, elements);
        generator.generate();
        QFAIL("Expected Pandaception for IC element");
...
#ifdef Q_OS_WIN
    QSKIP("Windows ACLs do not prevent file creation via POSIX permission bits");
#endif
