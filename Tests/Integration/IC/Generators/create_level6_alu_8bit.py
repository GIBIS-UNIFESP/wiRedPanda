...
class ALU8BitBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        # Create ground for low ALU CarryIn (no carry-in for 8-bit operation)
        ...
        # Connect ground to low ALU CarryIn
        if not await self.connect(ground_id, alu_low, target_port_label="CarryIn"):
            return False

        # Connect low ALU CarryOut to high ALU CarryIn for proper carry propagation
        if not await self.connect(alu_low, alu_high, source_port_label="CarryOut", target_port_label="CarryIn"):
            return False
        await self.log("  ✓ Connected carry between 4-bit ALUs for 8-bit arithmetic")
        ...
        await self.log(f"  🔍 Creating Zero flag with {len(result_outputs)} result outputs")
        zero_nor_id = await self.create_element("Nor", selector_x, selector_y + (2 * VERTICAL_STAGE_SPACING), "Zero_NOR")
        if zero_nor_id is None:
            return False
        await self.log(f"  ✓ Created Zero NOR (id={zero_nor_id})")

        # Set NOR gate to 8 inputs
        await self.log(f"  🔍 Setting NOR input_size to 8")
        set_props = await self.mcp.send_command("set_element_properties", {
            "element_id": zero_nor_id,
            "input_size": 8
        })
        if not set_props.success:
            self.log_error(f"Failed to set input_size=8 for Zero NOR: {set_props.error}")
            return False
        await self.log(f"  ✓ Set NOR input_size to 8")

        # Connect all selector outputs to NOR
        await self.log(f"  🔍 Connecting {len(selector_ids)} selector outputs to NOR")
        for i in range(8):
            if not await self.connect(selector_ids[i], zero_nor_id, source_port_label="out", target_port=i):
                return False
        await self.log(f"  ✓ Connected all selector outputs to NOR")
