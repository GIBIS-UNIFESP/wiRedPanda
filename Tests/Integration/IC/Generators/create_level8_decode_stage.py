    ...
    async def create(self) -> bool:
        ...
        for i in range(5):
            opcode_id = await self.create_element(
                "InputSwitch", input_x + (i * HORIZONTAL_GATE_SPACING), input_y, f"OpCode[{i}]"
            )
        ...
        # MemWrite output
        memwrite_led_id = await self.create_element(
            "Led", output_x + (3 * HORIZONTAL_GATE_SPACING), input_y, "MemWrite"
        )
        ...
        # decodes the low nibble. level2_decoder_4to16 has no enable port — it
