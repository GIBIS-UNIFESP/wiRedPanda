...
Inputs: Data[0..3], Sel[0..1], Enable
Output: Output
...
- Active-high enable (strobe) ANDed onto the output: Output = mux_out AND Enable
  (Enable=0 forces Output=0, matching the 74153 enable)

Usage:
    python3 create_level2_mux_4to1.py
"""
...
class Multiplexer4to1Builder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        await self.log(
            f"✅ Successfully created 4:1 Multiplexer IC ({self.element_count} elements, {self.connection_count} connections)"
        )
