...
class FetchStageBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        prog_addr_inputs = []
        for i in range(8):
            prog_addr_id = await self.create_element(
        ...
        pc_id = await self.instantiate_ic(
            "level7_cpu_program_counter_8bit", input_x + (2 * HORIZONTAL_GATE_SPACING), 250.0, "PC"
        )
        if pc_id is None:
            return False
        ...
        imem_id = await self.instantiate_ic(
            "level7_instruction_memory_interface", input_x + (6 * HORIZONTAL_GATE_SPACING), 250.0, "InstrMem"
        )
        if imem_id is None:
            return False
        ...
        ir_id = await self.instantiate_ic(
            "level7_instruction_register_8bit", input_x + (10 * HORIZONTAL_GATE_SPACING), 250.0, "InstrReg"
        )
        if ir_id is None:
            return False
        ...
        for i in range(8):
            if not await self.connect(
                addr_mux_id, imem_id, source_port_label=f"Out[{i}]", target_port_label=f"Address[{i}]"
            ):
                return False
        ...
        for i in range(8):
            if not await self.connect(
                imem_id, ir_id, source_port_label=f"Instruction[{i}]", target_port_label=f"Data[{i}]"
            ):
                return False
