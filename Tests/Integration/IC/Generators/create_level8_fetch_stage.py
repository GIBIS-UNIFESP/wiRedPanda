...
class FetchStageBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        # Program Counter
        pc_id = await self.instantiate_ic("level7_cpu_program_counter_8bit", input_x + (2 * HORIZONTAL_GATE_SPACING), 250.0, "PC")
        if pc_id is None:
            return False
        await self.log("  ✓ Instantiated Program Counter")

        # Instruction Memory Interface
        imem_id = await self.instantiate_ic("level7_instruction_memory_interface", input_x + (6 * HORIZONTAL_GATE_SPACING), 250.0, "InstrMem")
        if imem_id is None:
            return False
        await self.log("  ✓ Instantiated Instruction Memory Interface")

        # Address Mux: selects between PC address (In0) and ProgAddr (In1)
        addr_mux_id = await self.instantiate_ic("level4_bus_mux_8bit", input_x + (4 * HORIZONTAL_GATE_SPACING), 250.0, "AddrMux")
        if addr_mux_id is None:
            return False
        await self.log("  ✓ Instantiated Address Mux (PC vs ProgAddr)")

        # Instruction Register
        ir_id = await self.instantiate_ic("level7_instruction_register_8bit", input_x + (10 * HORIZONTAL_GATE_SPACING), 250.0, "InstrReg")
        if ir_id is None:
            return False
