...
class Multiplexer8to1Builder(ICBuilderBase):
    """Builder for 8-to-1 Multiplexer IC"""

    async def create(self) -> bool:
        """Create 8-to-1 Multiplexer IC"""
        data_inputs = 8
        select_bits = 3
        input_size = 11  # 8 data + 3 select

        await self.begin_build('Multiplexer 8-to-1')
        # Create new circuit
        ...
        mux_id = await self.create_element("Mux", mux_x, mux_y, "Mux_8to1")
        if mux_id is None:
            return False
        ...
        await self.log(f"✅ Successfully created 8:1 Multiplexer IC ({self.element_count} elements, {self.connection_count} connections)")
...
async def build(mcp) -> bool:
    ...
    builder = Multiplexer8to1Builder(mcp, verbose=True)
