...
Inputs: A[0-3], B[0-3], CarryIn, SubCarryIn (10 inputs)
Outputs: Result_ADD[0-3], Result_SUB[0-3], Result_AND[0-3], Result_OR[0-3], CarryOut, SubCarryOut (18 outputs)
...
class ALU4bitBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        subtractor_id = await self.instantiate_ic(
            "level4_ripple_adder_4bit", adder_x, subtractor_y, "Subtractor (Adder with ~B)"
        )
        ...
        for i in range(4):
            mux_id = await self.instantiate_ic(
                "level2_mux_2to1", adder_x + i * dense_spacing, and_or_y + VERTICAL_STAGE_SPACING, f"mux_or[{i}]"
            )
            if mux_id is None:
                return False
        ...
        # Connect SubCarryIn to subtractor CarryIn for proper chaining
        # For the lowest ALU in a chain, SubCarryIn=1 provides the +1 for two's complement
        if not await self.connect(sub_carryin_id, subtractor_id, target_port_label="CarryIn"):
            return False
        ...
        # Connect subtractor CarryOut to SubCarryOut LED
        if not await self.connect(subtractor_id, sub_carryout_led, source_port_label="CarryOut"):
            return False
