...
"""
Create the Temporal Ring Oscillator example (written to Examples/).

Three inverters in a loop (NOT -> NOT -> NOT -> back to the first), with the first
inverter's stage tapped by an LED labelled "Q".

A three-inverter ring has no stable logic value. In functional (zero-delay) mode the engine
therefore canonicalises it to Unknown -- which is the teaching point, and why the circuit
looks "dead" until it is swept with delays. Open BeWavedDolphin and switch on the Temporal
toolbar toggle: with the 5 ns per-inverter default the loop oscillates with a ~30 ns period,
visible as alternating columns. No per-element overrides -- the type defaults drive it.

Usage:
    python3 create_example_temporal_ring_oscillator.py
"""
...
class RingOscillatorBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        if None in (not1, not2, not3, led):
            return False
        assert not1 is not None and not2 is not None and not3 is not None and led is not None
