...
class JohnsonCounterBuilder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        # this column uses 2x VERTICAL_STAGE_SPACING, not 1x. A flat 1x
        # HORIZONTAL_GATE_SPACING from the input column isn't enough clearance
        # either: hold_mux0's row (y_hold=100.0) lands almost exactly level
        # with CountEnable's own row, so CountEnable's label -- drawn to its
        # right -- clipped hold_mux0's left edge; 1.5x pushes the whole bit
        # column clear of it.
