...
class Decoder2to4Builder(ICBuilderBase):
    """Builder for 2-to-4 Decoder IC"""
    ...
    async def create(self) -> bool:
        ...
        await self.begin_build("Decoder 2-to-4")
        ...
        # Create 2-input AND gates for one-hot encoding
        output_leds = []
        and_gates = []
        ...
        for i in range(num_outputs):
            ...
            # Widen to width+1 inputs so the active-high enable gates every output
        ...
        # Save circuit as IC
        output_file = str(IC_COMPONENTS_DIR / "level2_decoder_2to4.panda")
        if not await self.save_circuit(output_file):
            return False

        await self.log(f"✅ Successfully created 2-to-4 Decoder IC ({self.element_count} elements, {self.connection_count} connections)")
...
async def build(mcp) -> bool:
    ...
    builder = Decoder2to4Builder(mcp, verbose=True)
