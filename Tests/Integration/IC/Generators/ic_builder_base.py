...
try:
    ...
    if not (_mcp_client / "mcp_client").is_dir():
        raise ImportError(
...
class ICBuilderBase:
    ...
    async def instantiate_ic(self, component: str, x: float, y: float, label: str = "") -> "int | None":
        ...
        exists before instantiating, so every call site is a single uniform
        line and the dependency check can't be forgotten. Returns element_id,
        or None on failure (missing dependency or MCP error).
        """
