...
Inputs: addr[0], addr[1], addr[2], enable
Outputs: out[0] to out[7]

Circuit:
- 3 NOT gates (invert address bits)
- 8 AND gates (4-input, one-hot encoding)
  - Each AND combines 3 address inputs (direct or inverted) plus enable
...
class Decoder3to8Builder(ICBuilderBase):
    ...
    async def create(self) -> bool:
        ...
        await self.log(
            f"✅ Successfully created 3-to-8 Decoder IC ({self.element_count} elements, {self.connection_count} connections)"
        )
