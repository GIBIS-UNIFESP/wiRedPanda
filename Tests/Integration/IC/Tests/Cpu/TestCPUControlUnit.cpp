...
void TestCPUControlUnit::testControlUnit_data()
{
    ...
    QTest::newRow("cu_reset") << 0 << (int)CU_FETCH;           // 0 transitions: state 0
    QTest::newRow("cu_fetch_to_decode") << 1 << (int)CU_DECODE;  // 1 transition: state 1
    QTest::newRow("cu_decode_to_execute") << 2 << (int)CU_EXECUTE;  // 2 transitions: state 2
    QTest::newRow("cu_execute_to_writeback") << 3 << (int)CU_WRITEBACK;  // 3 transitions: state 3
    QTest::newRow("cu_writeback_to_fetch") << 5 << (int)CU_FETCH;  // 5 transitions: state 0 (wrap)
    QTest::newRow("cu_full_cycle") << 5 << (int)CU_FETCH;  // 5 transitions: state 0 (full cycle back to FETCH)
    QTest::newRow("cu_nop_sequence") << 5 << (int)CU_FETCH;  // 5 transitions: state 0
    QTest::newRow("cu_multi_instruction") << 12 << (int)CU_EXECUTE;  // 12 % 5 = 2: state 2
    QTest::newRow("cu_halt_state") << 4 << (int)CU_HALT;  // 4 transitions: state 4 (HALT)
    QTest::newRow("cu_state_stability") << 1 << (int)CU_DECODE;  // 1 transition: state 1
}
