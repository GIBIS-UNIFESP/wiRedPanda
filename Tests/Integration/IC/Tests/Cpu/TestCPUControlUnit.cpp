...
void TestCPUControlUnit::testControlUnitSignals()
{
    ...
    for (int s = 0; s < 5; ++s) {
        QCOMPARE(readMultiBitOutput(state), s);
        const Expect &e = expected[s];
        QCOMPARE(getInputStatus(pcInc, 0), e.pcInc != 0);
        QCOMPARE(getInputStatus(irLoad, 0), e.irLoad != 0);
        QCOMPARE(getInputStatus(aluEnable, 0), e.aluEnable != 0);
        QCOMPARE(getInputStatus(regWrite, 0), e.regWrite != 0);
        QCOMPARE(getInputStatus(flagWrite, 0), e.flagWrite != 0);
        clockCycle(sim, clock);
        sim->update();
    }
}
