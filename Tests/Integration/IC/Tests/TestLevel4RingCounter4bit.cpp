...
void TestLevel4RingCounter4Bit::testRingCounter()
{
    ...
    // Load ring counter IC
    IC *ringIC = loadBuildingBlockIC("level4_ring_counter_4bit.panda");

    // Add elements
    builder.add(&clk, &preset, ringIC);
    for (int i = 0; i < 4; ++i) {
        builder.add(&counterOut[i]);
    }

    // Connect clock and preset
    builder.connect(&clk, 0, ringIC, "CLK");
    builder.connect(&preset, 0, ringIC, "PRESET");

    // Connect counter outputs
    for (int i = 0; i < 4; ++i) {
        QString outputLabel = QString("Q%1").arg(i);
        builder.connect(ringIC, outputLabel, &counterOut[i], 0);
    }
