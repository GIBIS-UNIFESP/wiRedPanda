// ==================================================================== //
// ======= This code was generated automatically by wiRedPanda ======== //
// ==================================================================== //


// Module for LEVEL2_PARITY_GENERATOR (generated from level2_parity_generator.panda)
module level2_parity_generator_ic (
    input data0,
    input data1,
    input data2,
    input data3,
    input cascadein,
    output parity,
    output even
);

wire aux_xor_1;
wire aux_xor_2;
wire aux_xor_3;
wire aux_xor_4;
wire aux_not_5;

// Internal logic
assign aux_xor_1 = (data0 ^ data1);
assign aux_xor_2 = (data2 ^ data3);
assign aux_xor_3 = (aux_xor_1 ^ aux_xor_2);
assign aux_xor_4 = (aux_xor_3 ^ cascadein);
assign aux_not_5 = ~aux_xor_4;

assign parity = aux_xor_4;
assign even = aux_not_5;
endmodule

module level2_parity_generator (
/* ========= Inputs ========== */
input input_switch1,
input input_switch2,
input input_switch3,
input input_switch4,
input input_switch5,

/* ========= Outputs ========== */
output led7_1,
output led8_1
);
/* ====== Aux. Variables ====== */
// IC instance: LEVEL2_PARITY_GENERATOR (level2_parity_generator_ic)
wire w_level2_parity_generator_ic_inst_1_parity;
wire w_level2_parity_generator_ic_inst_1_even;


// Assigning aux variables. //
level2_parity_generator_ic level2_parity_generator_ic_inst_1 (
    .data0(input_switch1),
    .data1(input_switch2),
    .data2(input_switch3),
    .data3(input_switch4),
    .cascadein(input_switch5),
    .parity(w_level2_parity_generator_ic_inst_1_parity),
    .even(w_level2_parity_generator_ic_inst_1_even)
);

// Writing output data. //
assign led7_1 = w_level2_parity_generator_ic_inst_1_parity;
assign led8_1 = w_level2_parity_generator_ic_inst_1_even;
endmodule
