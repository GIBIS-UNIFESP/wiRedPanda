...
/// General behavioral coverage for QuickICHandler's create_ic/instantiate_ic/list_ics/embed_ic/
/// extract_ic validation chains and real success paths, distinct from TestQuickICHandlerSecurity
/// (path-traversal/security hardening). "No scene" is simulated by never calling
/// QuickAppController::newTab() -- it starts with zero tabs.
class TestQuickICHandler : public QObject
{
    Q_OBJECT
