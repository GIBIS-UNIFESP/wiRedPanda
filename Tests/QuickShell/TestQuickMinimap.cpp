...
void TestQuickMinimap::testThemeChangeInvalidatesCache()
{
    // Mirrors MinimapWidget.cpp's ThemeManager::instance().themeChanged -> invalidateCache()
    // connection: renderMinimapImage() paints real theme-following colors, so a theme switch
    // must trigger a fresh regen, not keep showing the stale cached thumbnail.
    ...
    QSignalSpy spy(&minimap, &QuickMinimap::refreshed);
    QVERIFY2(spy.wait(1000), "initial regenerate() should fire once the throttle elapses");
    spy.clear();

    ThemeManager::setTheme(originalTheme == Theme::Dark ? Theme::Light : Theme::Dark);

    QVERIFY2(spy.wait(1000), "themeChanged must trigger a fresh regen, not a stale cached image");

    ThemeManager::setTheme(originalTheme);
}

void TestQuickMinimap::testClampGeometryReturnsUnchangedWhenAlreadyWithinBounds()
{
    // A persisted geometry that already fits comfortably inside the parent bounds is
    // returned unchanged.
    QuickMinimap minimap;
    const QRectF result = minimap.clampGeometry(QRectF(50, 60, 200, 150), 800, 600);
    QCOMPARE(result, QRectF(50, 60, 200, 150));
}

void TestQuickMinimap::testClampGeometryReclampsWhenParentShrinks()
{
    // Geometry that fit at (30, 40, 200, 150) inside a larger parent must be re-clamped (both
    // size and position, not just position) once the parent shrinks to 200x180 -- comfortably
    // above the 160x120 minimum, so this exercises the maxWidth/maxHeight ceiling, not the floor.
    QuickMinimap minimap;
    const QRectF result = minimap.clampGeometry(QRectF(30, 40, 200, 150), 200, 180);
    QCOMPARE(result, QRectF(12, 18, 176, 150));
}

void TestQuickMinimap::testClampGeometryClampsOversizedCandidate()
{
    // A persisted geometry larger than the current parent (e.g. restored from a bigger
    // window/monitor) is clamped to fit, both in size and position, on restore.
    QuickMinimap minimap;
    const QRectF result = minimap.clampGeometry(QRectF(10, 10, 900, 700), 800, 600);
    QCOMPARE(result, QRectF(12, 12, 776, 576));
}
