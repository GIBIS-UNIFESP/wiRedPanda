...
void TestGraphicElementSerializer::testLoadInputOutputPortRenamesExistingIcPorts()
{
    ...
    // QuickCircuitBuilder is self-contained (owns its own registry, independent of any
    // CanvasItem), so it can't be used here -- this test needs the elements/connections
    // registered directly in subWorkspace's own canvas for QuickWorkSpace::save() to see them.
    // Wired manually instead, mirroring CircuitBuilder::connect()'s own
    // "create a Connection, wire it by port index, add to scene" contract exactly.
    //
    // Heap-allocated (not stack, unlike the Widgets original): CanvasItem::~CanvasItem()
    // qDeleteAll()s every still-registered element, so a stack object here would be deleted
    // twice -- once by its own normal stack unwind, once more (invalid) by subCanvas's
    // destructor once subWorkspace goes out of scope after them.
    QuickWorkSpace subWorkspace;
    CanvasItem *subCanvas = subWorkspace.canvas();
    auto *sw1 = new InputSwitch();
    auto *sw2 = new InputSwitch();
    auto *andGate = new And();
    auto *led = new Led();
    subCanvas->addItem(sw1);
    subCanvas->addItem(sw2);
    subCanvas->addItem(andGate);
    subCanvas->addItem(led);

    auto *conn1 = new Connection();
    conn1->setStartPort(sw1->outputPort());
    conn1->setEndPort(andGate->inputPort(0));
    subCanvas->addItem(conn1);

    auto *conn2 = new Connection();
    conn2->setStartPort(sw2->outputPort());
    conn2->setEndPort(andGate->inputPort(1));
    subCanvas->addItem(conn2);

    auto *conn3 = new Connection();
    conn3->setStartPort(andGate->outputPort());
    conn3->setEndPort(led->inputPort());
    subCanvas->addItem(conn3);

    const QString subPath = subDir.path() + "/reload_probe.panda";
    QCOMPARE(subWorkspace.save(subPath), QuickWorkSpace::SaveOutcome::Saved);
