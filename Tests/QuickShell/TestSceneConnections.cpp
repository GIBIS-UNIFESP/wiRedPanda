...
// NOTE: There is no Scene in this file. Every assertion below concerns Port/Connection/
// GraphicElement state directly, which Scene::addItem() only affects by assigning an id and
// taking ownership -- so elements here are owned directly via std::unique_ptr instead.
...
void TestSceneConnections::testConnectionWithMultiplePorts()
{
    ...
    // Connect multiple ports (each Connection is transitively owned by andGate/orGate's ports
    // once attached -- see ~ElementPorts()/Port::drainConnections() -- no separate tracking
    // needed here)
