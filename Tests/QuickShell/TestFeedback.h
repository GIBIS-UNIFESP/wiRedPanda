...
private:
    ...
    void verifyConvergence(QuickCircuitBuilder *builder);
    void verifyFeedbackDetection(QuickCircuitBuilder *builder);
    void verifyStableState(QuickCircuitBuilder *builder);

    // Locates the S/R switches and the Q/Q' NAND gates in a createSRLatchFromNAND() builder,
    // by their construction position (S-side elements at y=0, R-side at y=50).
    void findSRLatchParts(QuickCircuitBuilder *builder, InputSwitch *&switchS, InputSwitch *&switchR,
