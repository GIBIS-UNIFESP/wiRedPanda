 ...
 * now (see the qtquick-rewrite plan's Step 2), so nothing but a flat element list and a
 * SimulationHost implementation is actually needed to drive a Simulation.
 *
 ...
 * itself promised. Real call sites pass stack-allocated elements just as often as heap ones (see
 * e.g. Tests/Integration/TestSimulation.cpp's "InputSwitch sw1, sw2; ... builder.add(&sw1, &sw2)"
 ...
 * ElementFactory::buildElement() and returns it as one object (see e.g.
 * Tests/QuickShell/TestFeedback.cpp's createSRLatchFromNAND()-style factories) -- there the
 * builder is the only thing that will ever reference those elements again, so it must own them.
 *
 ...
 * element is destroyed (~ElementPorts() drains and deletes attached connections symmetrically),
 * so tracking it a second time here would risk a double free. For connections between two
 * caller-owned (non-owned) elements, the caller is likewise responsible for the Connection's
 * lifetime being covered by at least one of its two ports' owning elements.
 *
 ...
 * Scene::addItem()/CanvasItem::addItem() pattern (unassigned items get nextId(); pre-assigned
 * ones just advance the counter). A fresh QuickCircuitBuilder's id sequence always restarts at 0,
 * matching a fresh WorkSpace/Scene's own fresh SceneItemRegistry -- load-bearing for tests that
 * compare id-derived orderings (e.g. topological sort output) across repeated builds of an
 * identical circuit.
 *
 * Usage:
...
private:
    ...
    /// mirroring Scene::addItem()/CanvasItem::addItem() exactly.
