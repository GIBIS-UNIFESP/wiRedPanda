...
#include "Tests/QuickShell/TestAudioElementHelpers.h"
...
void TestAudioBox::testSetAudioWithAbsolutePath()
{
    ...
    // own (that happens at load time, via ExternalFilePath::forReading(); see
...
void TestAudioBox::testUpdateCommandUndoRedoPreservesFullAudioPath()
{
    ...
    canvas.undoStack()->push(new CanvasUpdateCommand({audioElm}, oldData, &canvas));

    // undo() must restore the default (resource) audio, not just leave the custom one
    canvas.undoStack()->undo();
    QVERIFY(dynamic_cast<GraphicElement *>(canvas.itemById(id))->externalFiles().isEmpty());

    // redo() reapplies the custom audio path, full and unstripped
    canvas.undoStack()->redo();
    QCOMPARE(dynamic_cast<GraphicElement *>(canvas.itemById(id))->externalFiles(), QStringList{audioPath});
}
