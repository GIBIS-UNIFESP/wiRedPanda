...
{
    // Adapted from TestBewavedDolphinGui's testSetClockWaveDisabledWithoutSelectionC9: Quick's
    // applyClockWave() itself has no empty-selection guard (DolphinWindow.qml's own button is
    // gated on hasSelection instead, mirroring the original's actionSetClockWave enabled state)
    // -- so the real thing worth locking in is that hasSelection tracks the selection model.
    CanvasItem canvas(nullptr, /*buildDemo=*/false);
    buildAndCircuit(canvas);
    QuickDolphinController controller;
    controller.createWaveform(&canvas);
...
void TestQuickDolphinController::testFitScreenClampsAndGuardsA26()
{
    ...
    // never blow up to +inf from a tiny divisor (the WIREDPANDA-HW class of bug the Widgets
...
void TestQuickDolphinController::testSetCellValueIsNotUndoable()
{
    ...
    // Real, confirmed behavioral difference from BewavedDolphin::on_tableView_cellDoubleClicked()
    // (which pushed a SetCellsCommand): setCellValue() -- the Quick double-click entry point --
    // mutates directly and never touches the undo stack.
    QVERIFY(!controller.canUndo());
}
