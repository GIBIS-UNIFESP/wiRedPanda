...
void TestSystemVerilogExport::initTestCase()
{
    ...
    // instead of skipping this entire class up front. Shared test environment (headless mode,
    // QSettings/QStandardPaths sandboxing, Application::interactiveMode) is set up by the
    // global runner (runQuickTestSuite()) before this slot.
}
...
void TestSystemVerilogExport::testSystemVerilogExportHelper(const QString &icFile)
{
    ...
    // Create a headless circuit builder (no Scene behind it).
    ...
    const QVector<GraphicElement *> allElements = builder.elements();

    QVERIFY2(!allElements.isEmpty(), "Circuit should contain elements");
    ...
    const QString expectedDir = QuickTestUtils::systemVerilogExpectedDir();
    ...
    if (generateMode) {
        ...
        QVector<QuickTestUtils::DiffStep> seqSteps;
        ...
        QVector<int> asyncHighReset;
        {
            ...
            if (isSequential) {
                ...
                seqSteps = QuickTestUtils::generateDifferentialVectors(
        ...
        if (!seqSteps.isEmpty()) {
            ...
            auto collectOutputBits = [&]() {
                ...
                for (int o = 0; o < numOutputs; ++o) {
                    bits[o] = QuickTestUtils::inputStatus(leds[o], 0) ? 1 : 0;
                }
                return bits;
            };
...
void TestSystemVerilogExport::testTopLevelFeedbackLatchHoldsStateInExport()
{
    ...
    // A cross-coupled NAND SR latch drawn at TOP LEVEL -- no IC anywhere in the circuit.
    ...
    auto *led = new Led();
    builder.addOwned(swS, swR, nandQ, nandQBar, led);
    swS->setLabel("NS");
    swR->setLabel("NR");
