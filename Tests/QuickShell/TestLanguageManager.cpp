...
void TestLanguageManager::testAvailableLanguages()
{
    ...
    // resources. Those are embedded only via qt_add_translations(...) scoped to the app
    // executable target specifically -- not available in this test binary -- so skip rather
    // than fail when only the fallback is found, the same pattern the Qt-translations tests
    // below already use.
    ...
    if (languages.size() == 1) {
        QSKIP("Only the hardcoded \"en\" fallback was found — wpanda_*.qm translations are "
              "embedded only in the app executable target, not in this test binary");
    }
...
{
    // "zz" isn't a real language QLocale recognises, so nativeLanguageName() degrades to
    // empty and displayName() falls through to the manual-fallback map's default: langCode
    // itself, capitalized.
    LanguageManager manager;
...
void TestLanguageManager::testLoadTranslationForNonEnglishLanguageWithoutEmbeddedResource()
{
    ...
    // to translationChanged() without installing anything.
    LanguageManager manager;
    QSignalSpy translationChangedSpy(&manager, &LanguageManager::translationChanged);
