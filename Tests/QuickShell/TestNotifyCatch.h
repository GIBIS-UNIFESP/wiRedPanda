...
private slots:
    ...
    /// Application::guardedSlot.  Asserts that the slot returns and the test
    /// process does not abort.
    void guardedSlotCatchesPostedSlotThrow();

    /// Same dispatch path with interactiveMode=true.  No Quick exception
    /// presenter is registered yet (App/Core/Application.h's own doc comment:
    /// "a future Qt Quick host would have its own" -- not built as of this
    /// domain-only phase), so handleException()'s presenter call is a
    /// documented no-op; this asserts guardedSlot still completes cleanly
    /// with interactiveMode on, rather than exercising an actual dialog.
    void guardedSlotCatchesPostedSlotThrowInteractive();
