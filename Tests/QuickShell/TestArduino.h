...
/// Most of this class's tests build GraphicElement/IC objects directly
/// (ElementFactory::buildElement()/`new`) and call generateFromElements() -- no Scene coupling.
/// Tests that embed an IC blob use CanvasICRegistry (owned by CanvasItem); tests that just need
/// a flat element list use QuickCircuitBuilder.
