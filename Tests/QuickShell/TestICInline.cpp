...
/// Local replacement for Tests/Common/ICTestHelpers.h's readFile()/embedIC() -- that header
/// pulls in App/Scene/CanvasICRegistry.h, which is WIDGETS_ONLY_SOURCES (not linked into
/// test_wiredpanda), and its embedIC() is typed for CanvasICRegistry* rather than
/// CanvasICRegistry*. Bodies are otherwise identical. Kept as free functions named exactly
/// like the originals (not a `using` alias) so every one of this file's many unqualified call
/// sites resolves via ordinary lookup unchanged.
...
void embedIC(IC *ic, const QByteArray &pandaBytes, const QString &blobName,
             const QString &contextDir, CanvasICRegistry *reg)
{
...
/// Local, CanvasItem-backed drop-in replacement for Tests/Common/TestUtils.h's Scene*-based
/// CircuitBuilder -- lets every one of this file's many builder.add()/builder.connect()/
/// builder.initSimulation() call sites stay textually unchanged rather than hand-rewriting
/// dozens of near-identical call sites into raw addItem()+Connection pairs. Real interactive
/// wiring goes through CanvasItem's own command-driven API (CanvasAddItemsCommand etc.); this
/// exists purely as a test-authoring convenience, matching QuickCircuitBuilder's own add()/
/// connect() shape but targeting a real CanvasItem (needed here for icRegistry() access, which
/// QuickCircuitBuilder doesn't have).
class CircuitBuilder
{
public:
    explicit CircuitBuilder(CanvasItem *canvas) : m_canvas(canvas) {}

    template<typename... Args>
    void add(Args *...elements) { (m_canvas->addItem(elements), ...); }
...
void TestICInline::testEmbedPreservesSimulation()
{
    ...
    CanvasItem scene(nullptr, /*buildDemo=*/false);
...
void TestICInline::testSaveLoadWithEmbeddedIC()
{
    ...
    QuickWorkSpace ws;
    ws.canvas()->setContextDir(m_fixtureDir);
    ws.canvas()->icRegistry()->setBlob("my_and", blob);

    auto *ic = new IC();
    ic->setBlobName("my_and");
    ic->loadFromBlob(blob, m_fixtureDir);
    ic->setPos(100, 100);
    ws.canvas()->addItem(ic);
    ...
    for (auto *elm : ws2.canvas()->elements()) {
    ...
    QVERIFY(ws2.canvas()->icRegistry()->hasBlob("my_and"));
}
...
void TestICInline::testBlobRegistryPersistence()
{
    ...
    QuickWorkSpace ws;
    ws.canvas()->setContextDir(m_fixtureDir);
    ws.canvas()->icRegistry()->setBlob("persisted", blob);
    ...
    QuickWorkSpace ws2;
    ws2.load(path);
    QVERIFY(ws2.canvas()->icRegistry()->hasBlob("persisted"));
    QVERIFY(!ws2.canvas()->icRegistry()->blob("persisted").isEmpty());
}
...
void TestICInline::testInlineTabSaveLoad()
{
    ...
    QuickWorkSpace parent;
    SimulationBlocker parentBlocker(parent.canvas()->simulation());
    parent.canvas()->setContextDir(m_fixtureDir);
    parent.canvas()->icRegistry()->setBlob("inline_test", blob);

    auto *ic = new IC();
    ic->setBlobName("inline_test");
    ic->loadFromBlob(blob, m_fixtureDir);
    parent.canvas()->addItem(ic);

    int icId = ic->id();

    // Open inline tab
    QuickWorkSpace child;
    SimulationBlocker childBlocker(child.canvas()->simulation());
    ...
    QVERIFY(!child.canvas()->elements().isEmpty());
}
...
void TestICInline::testChildBlobPropagation()
{
    ...
    SimulationBlocker parentBlocker(parent.canvas()->simulation());
    parent.canvas()->setContextDir(m_fixtureDir);
    parent.canvas()->icRegistry()->setBlob("prop_test", blob);
    ...
    SimulationBlocker childBlocker(child.canvas()->simulation());
    child.loadFromBlob(blob, &parent, icId, m_fixtureDir);
    connect(&child, &QuickWorkSpace::icBlobSaved, &parent, &QuickWorkSpace::onChildICBlobSaved);

    QSignalSpy spy(&child, &QuickWorkSpace::icBlobSaved);
...
void TestICInline::testInlineTabSaveThrowsWhenFileBackedIcMissingOnDisk()
{
    ...
    SimulationBlocker parentBlocker(parent.canvas()->simulation());
    parent.canvas()->setContextDir(m_fixtureDir);
    parent.canvas()->icRegistry()->setBlob("chain_b_test", blob);
...
void TestICInline::testUndoRedoEmbedExtract()
{
    ...
    // Embed via CanvasICRegistry (stores full .panda file bytes as the blob)
    auto *reg = ws.canvas()->icRegistry();
    reg->embedICsByFile(ic->file(), rawFileBytes, "undo_test");

    // After embed, IC should be embedded
    auto *embeddedIC = static_cast<IC *>(ws.canvas()->elements().first());
    QVERIFY(embeddedIC->isEmbedded());
    QCOMPARE(embeddedIC->blobName(), QString("undo_test"));

    // Undo
    ws.canvas()->undoStack()->undo();

    auto *undoneIC = static_cast<IC *>(ws.canvas()->elements().first());
    QVERIFY(!undoneIC->isEmbedded());
    QVERIFY(!undoneIC->file().isEmpty());

    // Redo
    ws.canvas()->undoStack()->redo();

    auto *redoneIC = static_cast<IC *>(ws.canvas()->elements().first());
...
void TestICInline::testRegistryEmbedICsByFile()
{
    ...
    ic1->setPos(0, 0);
    ws.canvas()->addItem(ic1);

    auto *ic2 = new IC();
    ic2->loadFile(filePath, m_fixtureDir);
    ic2->setPos(100, 0);
    ws.canvas()->addItem(ic2);
...
void TestICInline::testEmbedSimulatesCorrectly()
{
    ...
    CircuitBuilder builder(ws.canvas());
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        sim->update();
        QCOMPARE(QuickTestUtils::inputStatus(led), expectedAND[i]);
    }
}
...
void TestICInline::testEmbedSimulatesMultiLevel()
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        swB->setOn(truthTable[i][1]);
        sim->update();
        QCOMPARE(QuickTestUtils::inputStatus(led), expectedNAND[i]);
    }
}
...
void TestICInline::testEmbedExtractAtSceneLevel()
{
    ...
    // Embed via CanvasICRegistry
    reg->embedICsByFile(ic->file(), rawFileBytes, "embed_extract");

    auto *embedded = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(embedded);
    ...
    auto *extracted = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(extracted);
    ...
    ws.canvas()->undoStack()->undo();
    auto *undoneExtract = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    ...
    ws.canvas()->undoStack()->undo();
    auto *undoneEmbed = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(undoneEmbed);
...
void TestICInline::testEmbedExtractWithActiveConnections()
{
    ...
    QCOMPARE(ws.canvas()->connections().size(), 3);
    ...
    auto *extractedIC = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(extractedIC);
    ...
    auto *original = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(original);
...
{
    // Renaming blobName via CanvasUpdateCommand should propagate to all same-blobName
    ...
    for (int id : ids) {
        auto *elm = dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id));
        QVERIFY(elm);
    ...
    for (int id : ids) {
        targets.append(dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id)));
    }
    ...
    ws.canvas()->undoStack()->push(new CanvasUpdateCommand(targets, oldData, ws.canvas()));
...
void TestICInline::testWorkspaceInlineTab()
{
    ...
    SimulationBlocker parentBlocker(parent.canvas()->simulation());
    parent.canvas()->setContextDir(m_fixtureDir);
    auto *reg = parent.canvas()->icRegistry();
...
void TestICInline::testWorkspaceInlineSaveOrphanedParent()
{
    ...
    auto parent = std::make_unique<QuickWorkSpace>();
    int icId;
    {
        // Scoped so this guard's own destructor (which restarts the simulation it blocked --
        // unlike a fresh Widgets Scene, CanvasItem's constructor always starts its Simulation
        // immediately, so this guard genuinely has something running to block) runs here, on
        // a still-alive parent, rather than after parent.reset() below leaves it holding a
        // dangling Simulation* to restart.
        SimulationBlocker parentBlocker(parent->canvas()->simulation());
        parent->canvas()->setContextDir(m_fixtureDir);
        auto *reg = parent->canvas()->icRegistry();
        reg->setBlob("simple_and", blob);
...
void TestICInline::testInlineSaveContextDirSwitch()
{
    ...
    auto *updatedIC = dynamic_cast<IC *>(parent.canvas()->itemById(icId));
    QVERIFY(updatedIC);
...
void TestICInline::testInlineTabSaveAfterModification()
{
    ...
    int elementsBefore = static_cast<int>(child.canvas()->elements().size());
    QVERIFY(elementsBefore >= 3);

    // Add a new InputSwitch
    auto *newSw = new InputSwitch();
    newSw->setPos(96, 296);
    child.canvas()->addItem(newSw);

    QCOMPARE(child.canvas()->elements().size(), elementsBefore + 1);
...
void TestICInline::testNestedWorkspaceChainPropagation()
{
    ...
    QuickWorkSpace grandparent;
    SimulationBlocker gpBlocker(grandparent.canvas()->simulation());
    grandparent.canvas()->setContextDir(m_fixtureDir);
    auto *gpReg = grandparent.canvas()->icRegistry();
    ...
    gpIC->setPos(200, 200);
    grandparent.canvas()->addItem(gpIC);
    int gpICId = gpIC->id();
    ...
    auto *parentReg = parent.canvas()->icRegistry();
    ...
    parent.canvas()->addItem(parentIC);
    ...
    QObject::connect(&child, &QuickWorkSpace::icBlobSaved,
                     &parent, &QuickWorkSpace::onChildICBlobSaved);
    QObject::connect(&parent, &QuickWorkSpace::icBlobSaved,
                     &grandparent, &QuickWorkSpace::onChildICBlobSaved);

    // Save child → propagates to parent
    QSignalSpy childSpy(&child, &QuickWorkSpace::icBlobSaved);
    child.save({});
    QCOMPARE(childSpy.size(), 1);
    QCOMPARE(childSpy.at(0).at(0).toInt(), parentICId);

    // Save parent → propagates to grandparent
    QSignalSpy parentSpy(&parent, &QuickWorkSpace::icBlobSaved);
    parent.save({});
    QCOMPARE(parentSpy.size(), 1);
    QCOMPARE(parentSpy.at(0).at(0).toInt(), gpICId);

    auto *updatedGpIC = dynamic_cast<IC *>(grandparent.canvas()->itemById(gpICId));
    QVERIFY(updatedGpIC);
...
void TestICInline::testOnChildICBlobSavedOrphan()
{
    ...
    ic->setBlobName("simple_and");
    ic->loadFromBlob(blob, m_fixtureDir);
    ws.canvas()->addItem(ic.get());
    int icId = ic->id();

    ws.canvas()->removeItem(ic.get());
    ic.reset();
...
void TestICInline::testOnChildICBlobSavedPropagation()
{
    ...
    int origInputs = static_cast<IC *>(parent.canvas()->itemById(ids.at(0)))->inputSize();

    // Propagate NOT blob (different port count)
    parent.onChildICBlobSaved(ids.at(0), notBlob);

    for (int id : ids) {
        auto *ic = dynamic_cast<IC *>(parent.canvas()->itemById(id));
        QVERIFY(ic);
...
void TestICInline::testOnChildICBlobSavedNonEmbeddedTarget()
{
    ...
    int stackIndex = ws.canvas()->undoStack()->index();

    ws.onChildICBlobSaved(fileBackedId, blob);
    ws.onChildICBlobSaved(swId, blob);

    QCOMPARE(ws.canvas()->undoStack()->index(), stackIndex);
}
...
void TestICInline::testOnChildICBlobSavedCorruptBlob()
{
    ...
    // (WorkSpace::onChildICBlobSaved()'s catch(...) block) -- unparseable bytes like this always
    // throw, deterministically, never falling through to push a command.
    ...
    QCOMPARE(ws.canvas()->undoStack()->index(), stackIndex);
    auto *restored = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(restored);
...
void TestICInline::testOnChildICBlobSavedCorruptBlobMultiTarget()
{
    ...
    int origInputs = static_cast<IC *>(ws.canvas()->itemById(ids.at(0)))->inputSize();
    int stackIndex = ws.canvas()->undoStack()->index();
    ...
    for (int i = 0; i < 3; ++i) {
        auto *ic = dynamic_cast<IC *>(ws.canvas()->itemById(ids.at(i)));
...
void TestICInline::testOnChildICBlobSavedPortCountWithConnections()
{
    ...
    auto *updatedIC = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(updatedIC);
    QVERIFY(updatedIC->inputSize() < origInputs);

    // Some connections may have been destroyed
    int connAfter = static_cast<int>(ws.canvas()->connections().size());
    QVERIFY(connAfter <= 3);
...
void TestICInline::testOnChildICBlobSavedBlobNameMismatch()
{
    ...
    auto *updatedAlpha = dynamic_cast<IC *>(ws.canvas()->itemById(alphaId));
    auto *unchangedBeta = dynamic_cast<IC *>(ws.canvas()->itemById(betaId));
    ...
    ws.canvas()->undoStack()->undo();
    updatedAlpha = dynamic_cast<IC *>(ws.canvas()->itemById(alphaId));
    unchangedBeta = dynamic_cast<IC *>(ws.canvas()->itemById(betaId));
    QVERIFY(updatedAlpha);
    QVERIFY(unchangedBeta);
...
void TestICInline::testMultipleInstancesBlobUpdate()
{
    ...
    auto *u1 = dynamic_cast<IC *>(ws.canvas()->itemById(ic1Id));
    auto *u2 = dynamic_cast<IC *>(ws.canvas()->itemById(ic2Id));
    QVERIFY(u1);
    QVERIFY(u2);
...
void TestICInline::testMultipleInstancesBlobUpdateLabelsPreserved()
{
    ...
    auto *u1 = dynamic_cast<IC *>(ws.canvas()->itemById(id1));
    auto *u2 = dynamic_cast<IC *>(ws.canvas()->itemById(id2));
    QVERIFY(u1);
    QVERIFY(u2);
    ...
    ws.canvas()->undoStack()->undo();
    u1 = dynamic_cast<IC *>(ws.canvas()->itemById(id1));
    u2 = dynamic_cast<IC *>(ws.canvas()->itemById(id2));
    QVERIFY(u1);
    QVERIFY(u2);
    QCOMPARE(u1->label(), QString("LABEL_ALPHA"));
    QCOMPARE(u2->label(), QString("LABEL_BETA"));
...
void TestICInline::testRemoveEmbeddedICWithConnections()
{
    ...
    QVERIFY(!ws.canvas()->itemById(icId));
    QCOMPARE(ws.canvas()->connections().size(), 0);
}
...
void TestICInline::testRemoveEmbeddedICCrossConnections()
{
    ...
    QCOMPARE(ws.canvas()->connections().size(), 4);
    ...
    QCOMPARE(ws.canvas()->connections().size(), 1);
}
...
void TestICInline::testPortCountChangeConnectionCleanup()
{
    ...
    // are destroyed. CanvasUpdateBlobCommand restores them on undo.
    ...
    auto conns = ws.canvas()->connections();
    for (auto *conn : conns) {
    ...
    auto restoredConns = ws.canvas()->connections();
    ...
    ws.canvas()->undoStack()->redo();
    QCOMPARE(ws.canvas()->connections().size(), connAfter);
    ws.canvas()->undoStack()->undo();
    QCOMPARE(ws.canvas()->connections().size(), 3);
}
...
void TestICInline::testSetInlineDataRollbackSimulationWorks()
{
    ...
    swB->setOn(true);
    sim->update();
    QVERIFY(QuickTestUtils::inputStatus(led));
}
...
void TestICInline::testFlipRotateEmbeddedIC()
{
    ...
    ws.canvas()->undoStack()->push(new CanvasFlipCommand({ic}, 1, ws.canvas()));

    auto *flippedV = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(flippedV);
    ...
    ws.canvas()->undoStack()->push(new CanvasFlipCommand({flippedV}, 0, ws.canvas()));

    auto *flippedH = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(flippedH);
    ...
    ws.canvas()->undoStack()->push(new CanvasRotateCommand({flippedH}, 90, ws.canvas()));

    auto *rotated = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(rotated);
    ...
    auto *redone = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
    QVERIFY(redone);
...
{
    std::unique_ptr<QuickWorkSpace> ws = std::make_unique<QuickWorkSpace>();
    ws->canvas()->setContextDir(m_fixtureDir);
    auto *reg = ws->canvas()->icRegistry();
    ...
    QVector<GraphicElement *> allElements = ws->canvas()->elements();
...
void TestICInline::testEmbedICByDropConversion()
{
    ...
    auto *e1 = dynamic_cast<IC *>(ws.canvas()->itemById(id1));
    auto *e2 = dynamic_cast<IC *>(ws.canvas()->itemById(id2));
    QVERIFY(e1);
    QVERIFY(e2);
    ...
    ws.canvas()->undoStack()->undo();
    e1 = dynamic_cast<IC *>(ws.canvas()->itemById(id1));
    e2 = dynamic_cast<IC *>(ws.canvas()->itemById(id2));
    QVERIFY(e1);
    QVERIFY(e2);
    QVERIFY(!e1->isEmbedded());
    QVERIFY(!e2->isEmbedded());
...
void TestICInline::testEmbedICByDropCollisionAutoSuffix()
{
    ...
    // Exercise the real auto-suffix generator (CanvasICRegistry::uniqueBlobName(), the same one
...
void TestICInline::testMultiCycleUndoRedoPropagation()
{
    ...
    for (int cycle = 0; cycle < 3; ++cycle) {
        ws.canvas()->undoStack()->undo();
        auto *elm = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
        ...
        ws.canvas()->undoStack()->redo();
        elm = dynamic_cast<IC *>(ws.canvas()->itemById(icId));
...
{
    // Positions and rotations should be preserved through CanvasUpdateBlobCommand.
    ...
    auto *u1 = dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id1));
    auto *u2 = dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id2));
    QVERIFY(u1);
    QVERIFY(u2);
    ...
    ws.canvas()->undoStack()->undo();
    u1 = dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id1));
    u2 = dynamic_cast<GraphicElement *>(ws.canvas()->itemById(id2));
    QVERIFY(u1);
    QVERIFY(u2);
    QCOMPARE(u1->pos(), QPointF(104, 200));
    QCOMPARE(u2->pos(), QPointF(296, 400));
...
void TestICInline::testLoadFromBlobClearsFileState()
{
    ...
    // path and make the IC embedded. File watching is handled by CanvasICRegistry
...
{
    // removeEmbeddedIC pairs the IC deletion with a CanvasRemoveBlobCommand inside
    // an undo macro, so undo restores both the IC instances AND the blob
    // they reference.
    QuickWorkSpace ws;
    auto *scene = ws.canvas();
    scene->setContextDir(m_fixtureDir);
    auto *reg = scene->icRegistry();
    ...
    QVERIFY2(!reg->hasBlob("remove_undo"), "Blob removed by CanvasRemoveBlobCommand redo");
...
void TestICInline::testBlobRegistryMergeConflictSkipsExisting()
{
    ...
    QuickWorkSpace wsSource;
    auto *regSource = wsSource.canvas()->icRegistry();

    QuickWorkSpace wsDest;
    auto *regDest = wsDest.canvas()->icRegistry();
    ...
    const auto &srcMap = regSource->blobMapRef();
    ...
    const auto &srcMap2 = regSource->blobMapRef();
...
void TestICInline::testImportNestedBlobsViaRegisterBlob()
{
    ...
    QCOMPARE(reg->blobMapRef().size(), 1);
...
{
    // Renaming "A" to "B" when "B" already exists must be a no-op -- overwriting an existing,
    // different blob would permanently destroy it (and silently make every IC instance that
    // already references that name start behaving like the renamed-in circuit), with no undo
    // path back.
    QByteArray blobA = readFile(m_fixtureDir + "/simple_and.panda");
...
void TestICInline::testLoadICWithMissingBlobFallsBackToFile()
{
    ...
    QuickWorkSpace ws1;
    ws1.canvas()->setContextDir(m_fixtureDir);
    auto *reg1 = ws1.canvas()->icRegistry();

    auto *ic1 = new IC();
    embedIC(ic1, blob, "fallback_test", m_fixtureDir, reg1);
    ic1->setPos(100, 100);
    ws1.canvas()->addItem(ic1);
...
void TestICInline::testSaveLoadOrphanBlobPreserved()
{
    ...
    QuickWorkSpace ws1;
    ws1.canvas()->setContextDir(m_fixtureDir);
    ws1.canvas()->icRegistry()->setBlob("orphan_blob", blob);
    ...
    QVERIFY2(ws2.canvas()->icRegistry()->hasBlob("orphan_blob"),
             "Orphan blob should survive save/load round-trip");
    QCOMPARE(ws2.canvas()->icRegistry()->blob("orphan_blob"), blob);
}

void TestICInline::testReconnectConnectionsSkipsDeletedElement()
{
    // CanvasUpdateBlobCommand::reconnectConnections() must silently skip connections
    ...
    // Mutate the IC (re-embed same blob). Must drain first: loadFromBlob() tears down and
    // rebuilds ic's ports regardless of whether the new blob matches the old one, and an old
    // port's destructor cascades-deletes whatever Connection is still attached to it (conn,
    // here) without ever telling this canvas -- see CanvasCommandUtils::drainAllConnections()'s
    // own doc comment, and QuickWorkSpace::onChildICBlobSaved()'s identical call for the same
    // reason on the production side of this exact operation.
    CanvasCommandUtils::drainAllConnections(ic, ws.canvas());
    ic->loadFromBlob(blob, m_fixtureDir);

    // Delete the switch (source of connection) before undo
    ws.canvas()->removeItem(sw);
    swOwner.reset();

    // Create CanvasUpdateBlobCommand and try undo — must not crash
    auto *cmd = new CanvasUpdateBlobCommand(targets, oldData, connections, ws.canvas());
    cmd->setOldBlob(blob);
    cmd->setBlobName("reconnect_test");
    ws.canvas()->undoStack()->push(cmd);
...
{
    // When CanvasUpdateBlobCommand is undone, the old blob data must be restored
    // in the registry.
    QByteArray blobA = readFile(m_fixtureDir + "/simple_and.panda");
...
{
    // ElementEditor::apply() calls setLabel() live on every keystroke and then pushes a
    // CanvasUpdateCommand, whose redo()/undo() round-trips the element through save()/load().
    // For an IC, load() must not tear down and rebuild the internal sub-circuit when only the
    // label (a base GraphicElement property) changed -- doing so makes typing in the label
    // field of a large embedded IC visibly laggy.
    QByteArray blob = readFile(m_fixtureDir + "/simple_and.panda");
...
void TestICInline::testBlobContentChangeUnderSameNameStillReloads()
{
    ...
    // name staying the same. CanvasUpdateBlobCommand::undo()/redo() replace the registry's bytes
...
{
    // CanvasRenameBlobCommand undo reverses the rename, redo re-applies it — mirroring
    ...
    scene.undoStack()->push(new CanvasRenameBlobCommand("rename_cmd_old", "rename_cmd_new", &scene));
...
{
    // A rename is its own independent CanvasRenameBlobCommand, so a plain property
    // CanvasUpdateCommand never touches m_blobName -- each undo reverses exactly one action,
    // rather than an unrelated property undo trying to resolve a pre-rename blob name against a
    // registry that only has the new name anymore (which throws and corrupts the IC: App/
    // Element/IC.cpp's no-match fallback sets m_file to the bogus name before
    // ICLoader::loadFile() throws "not found").
    QByteArray blob = readFile(m_fixtureDir + "/simple_and.panda");
    ...
    ws.canvas()->undoStack()->push(new CanvasRenameBlobCommand("regress_old", "regress_new", ws.canvas()));
    ...
    QVERIFY2(!threw, "pushing the property CanvasUpdateCommand must not throw");
...
// ============================================================================
// Edge cases: isEmbedded, port names, CanvasRegisterBlobCommand, IC::load
// ============================================================================
...
{
    // CanvasRegisterBlobCommand undo removes the blob, redo re-registers it.
    QByteArray blob = readFile(m_fixtureDir + "/simple_and.panda");
    ...
    scene.undoStack()->push(new CanvasRegisterBlobCommand("cmd_test", blob, &scene));
...
void TestICInline::testRegisterBlobCommandRedoAfterExternalRemove()
{
    ...
    scene.undoStack()->push(new CanvasRegisterBlobCommand("ext_rm", blob, &scene));
...
{
    // CanvasRemoveBlobCommand undo restores the blob's original bytes, redo re-removes it —
    // mirroring testRegisterBlobCommandUndoRedo()'s shape for the sibling command.
    QByteArray blob = readFile(m_fixtureDir + "/simple_and.panda");
    ...
    scene.undoStack()->push(new CanvasRemoveBlobCommand("remove_cmd_test", &scene));
...
{
    // loadFile() with a nonexistent file must throw without corrupting the IC -- clearing
    // m_blobName before the file check would leave the IC in a broken state (not embedded,
    // no file) that crashes the simulation.
...
void TestICInline::testChildBlobSavedPartialFailureRollback()
{
    ...
    const int origInputs = static_cast<IC *>(ws.canvas()->itemById(ids.at(0)))->inputSize();
    const int stackBefore = ws.canvas()->undoStack()->index();
    ...
    QCOMPARE(ws.canvas()->undoStack()->index(), stackBefore);
...
void TestICInline::testInlineSaveConvertsFileBackedToEmbedded()
{
    ...
    parentWs.canvas()->setContextDir(m_fixtureDir);
    auto *parentScene = parentWs.canvas();
    ...
    connect(&childWs, &QuickWorkSpace::icBlobSaved, &parentWs, &QuickWorkSpace::onChildICBlobSaved);
    ...
    childWs.canvas()->addItem(droppedIC);
    ...
    QSignalSpy saveSpy(&childWs, &QuickWorkSpace::icBlobSaved);
    childWs.save("");

    QCOMPARE(saveSpy.size(), 1);

    // --- Step 5: Verify the dropped IC was converted to embedded ---
    auto *savedIC = dynamic_cast<IC *>(childWs.canvas()->itemById(droppedICId));
    ...
    for (auto *elm : reopenedWs.canvas()->elements()) {
        if (elm->elementType() == ElementType::IC && elm->isEmbedded()) {
...
void TestICInline::testNestedInlineSaveAndReopen()
{
    ...
    const QString exDir = QuickTestUtils::examplesDir();

    // --- Step 1: Load nested.panda as root workspace ---
    QuickWorkSpace rootWs;
    rootWs.load(exDir + "nested.panda");
    auto *rootScene = rootWs.canvas();
    ...
    QuickWorkSpace level1Ws;
    level1Ws.loadFromBlob(chainBBlob, &rootWs, chainBICId, rootScene->contextDir());
    connect(&level1Ws, &QuickWorkSpace::icBlobSaved, &rootWs, &QuickWorkSpace::onChildICBlobSaved);

    // Find chain_c IC inside level-1
    IC *chainCIC = nullptr;
    for (auto *elm : level1Ws.canvas()->elements()) {
        if (elm->elementType() == ElementType::IC) {
            auto *ic = static_cast<IC *>(elm);
    ...
    if (chainCIC->isEmbedded()) {
        chainCBlob = level1Ws.canvas()->icRegistry()->blob(chainCBlobName);
    } else {
        chainCBlob = readFile(chainCIC->file());
        level1Ws.canvas()->icRegistry()->registerBlob(chainCBlobName, chainCBlob);
        chainCIC->setBlobName(chainCBlobName);
        chainCIC->loadFromBlob(level1Ws.canvas()->icRegistry()->blob(chainCBlobName),
                               level1Ws.canvas()->contextDir());
        chainCBlob = level1Ws.canvas()->icRegistry()->blob(chainCBlobName);
    }
    QVERIFY(!chainCBlob.isEmpty());

    // --- Step 3: Open chain_c in level-2 workspace ---
    QuickWorkSpace level2Ws;
    level2Ws.loadFromBlob(chainCBlob, &level1Ws, chainCICId, level1Ws.canvas()->contextDir());
    connect(&level2Ws, &QuickWorkSpace::icBlobSaved, &level1Ws, &QuickWorkSpace::onChildICBlobSaved);
    ...
    const QByteArray updatedChainC = level1Ws.canvas()->icRegistry()->blob(chainCBlobName);
    ...
    bool reopenOK = false;
    try {
        reopenedWs.loadFromBlob(updatedChainC, &level1Ws, chainCICId, level1Ws.canvas()->contextDir());
        reopenOK = true;
    } catch (const std::exception &) {
    }
...
{
    // The inline-IC save loop must not call loadFromBlob() on a file-backed sub-IC to "convert"
    // it to embedded -- loadFromBlob destroys and rebuilds the IC's child ports, which Qt
    // cascade-deletes the Connections attached to those ports, silently dropping every scene
    // wire connected to the file-backed sub-IC. Skipping the loadFromBlob call lets the ports
    // survive so the wires stay attached.
    QuickWorkSpace parentWs;
    parentWs.canvas()->setContextDir(m_fixtureDir);
    ...
    auto *parentIC = new IC();
    embedIC(parentIC, chainCBytes, "chain_c", m_fixtureDir, parentWs.canvas()->icRegistry());
    parentIC->setPos(100, 100);
    parentWs.canvas()->addItem(parentIC);
    const int parentICId = parentIC->id();

    QuickWorkSpace childWs;
    childWs.loadFromBlob(parentWs.canvas()->icRegistry()->blob("chain_c"),
                         &parentWs, parentICId, parentWs.canvas()->contextDir());
    connect(&childWs, &QuickWorkSpace::icBlobSaved, &parentWs, &QuickWorkSpace::onChildICBlobSaved);
    ...
    auto *sw = new InputSwitch();
    sw->setPos(0, 200);
    childWs.canvas()->addItem(sw);

    auto *wire = new Connection();
    wire->setStartPort(sw->outputPort());
    wire->setEndPort(droppedIC->inputPort(0));
    childWs.canvas()->addItem(wire);

    auto wireSurvives = [&]() {
        for (auto *conn : childWs.canvas()->connections()) {
    ...
    // Save must not cascade-delete the wire via this loop's loadFromBlob.
    childWs.save("");
...
{
    // CanvasICRegistry::onFileChanged() must capture connections + an element snapshot and
    // push a CanvasUpdateBlobCommand (mirroring embedICsByFile), rather than calling
    // IC::loadFile() directly with no command on the undo stack -- otherwise every subsequent
    // undo hits the item-ID defensive net because the recorded item IDs no longer match the
    // freshly-rebuilt IC.
    ...
    auto *undoStack = ws.canvas()->undoStack();
    ...
    // The handler must have pushed exactly one CanvasUpdateBlobCommand.
    QCOMPARE(undoStack->count(), stackBefore + 1);

    // Undo must succeed and walk back the stack, without findItems-based undo commands
    // looking up stale IDs.
    undoStack->undo();
    QCOMPARE(undoStack->index(), stackBefore);
    QCOMPARE(static_cast<int>(ws.canvas()->elements().size()), 1);
...
{
    // Pasting an embedded IC must register its blob and add the element as a single undo
    // macro -- undoing the paste has to reverse both, not leave the blob permanently orphaned
    // in the registry, unreachable by any further undo/redo action.
    CanvasItem sourceScene(nullptr, /*buildDemo=*/false);
    ...
    CanvasItem targetScene(nullptr, /*buildDemo=*/false);
...
{
    // embedICsByFile() must guard mutating a live, already-in-scene IC with a SimulationBlocker,
    // matching sibling onFileChanged(): if the embedded sub-circuit contains a sequential
    // element, resetInternalState() frees exactly the objects Simulation::m_sequentialElements
    // points to, and doing so while the simulation timer could still be running is a
    // use-after-free. This can't reproduce the real UAF deterministically in single-threaded
    // QtTest (see TestDanglingPointer's bug6/bug7 for the same limitation) — this test instead
    ...
    const QString filePath = QuickTestUtils::examplesDir() + "jkflipflop.panda";
    QVERIFY2(QFile::exists(filePath), "jkflipflop.panda fixture not found");
    QByteArray rawFileBytes = readFile(filePath);

    QuickWorkSpace ws;
    ws.canvas()->setContextDir(QFileInfo(filePath).absolutePath());
    ...
    ws.canvas()->simulation()->update();

    ws.canvas()->simulation()->stop();
}

void TestICInline::testLoadFileDirectlyEnforcesNestingDepthLimit()
{
    // ICLoader::loadFileDirectly() (used when an IC isn't yet in a scene, e.g. during
    // deserialization) must be bounded by the same kMaxICNestingDepth as its sibling
    // deserializeAndLoad() (the blob-cache path) -- cycle detection alone doesn't stop a long,
    // non-cyclic chain of distinct legitimate files (no repeats, so the cycle guard never fires)
    // from recursing unbounded and exhausting the call stack. Build a chain deeper than the
    // shared limit and confirm it throws cleanly instead of crashing.
    QTemporaryDir tmp;
    QVERIFY(tmp.isValid());
    ...
    // IS the deepening non-cyclic chain the nesting-depth limit exists to bound.
    constexpr int kChainLength = 20;
    bool threw = false;
    QString message;
    for (int level = 1; level <= kChainLength; ++level) {
        QuickWorkSpace ws;
        ws.canvas()->setContextDir(tmp.path());
        auto *ic = new IC();
        try {
