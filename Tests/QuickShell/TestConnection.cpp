...
void TestConnection::testConnectionPathUpdate()
{
    ...
    QVERIFY(!connection.shape().isEmpty());
...
{
    // applyStatusPen() computes statusPen() unconditionally now that Connection is a plain
    // class with no QGraphicsScene attachment concept -- this must still reflect the correct
    // colour and width for every status, including the Error <-> non-Error transitions.
