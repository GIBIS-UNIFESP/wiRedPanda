 ...
 * \brief Tests WorkSpace::save()/load()'s core (readPayload/readBoundedMetadata/
 * deserializeBlobRegistry/Serialization::deserialize()), which never touches Scene/WorkSpace
 * itself -- the interactive-mode version-mismatch dialogs, m_lastId bookkeeping, and
 * real-ICRegistry blob population are the only Scene/WorkSpace-specific pieces, none of which
 * any test here exercises. saveToMemory()/loadFromMemory() below are thin wrappers around
 * QuickTestUtils::savePandaBytes()/loadPandaBytes(), a Scene-free WorkSpace::save(QDataStream&)/
 * load(QDataStream&,...) replacement.
 */
...
private slots:
    ...
    // GraphicElementSerializer::loadTrigger() (old-format, version 1.9-4.0) relies on the
    // bounded reader below for QKeySequence deserialization.
...
private:
    // Helper methods -- thin wrappers around QuickTestUtils::savePandaBytes()/loadPandaBytes()
    // (see this class's own doc comment above).
    QByteArray saveToMemory(QuickCircuitBuilder &builder);
    void loadFromMemory(QuickCircuitBuilder &builder, const QByteArray &data);
