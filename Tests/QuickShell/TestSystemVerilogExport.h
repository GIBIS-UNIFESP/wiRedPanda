...
/**
 * \class TestSystemVerilogExport
 * \brief Quick-native port of Tests/Integration/TestSystemVerilogExport.cpp -- unlike most of
 * this suite's deferrals, every single test here ports unchanged: none of
 * testSystemVerilogExportHelper()'s ~70+ callers, the two wireless-codegen tests, the IC-label
 * injection regression, or the top-level feedback-latch test ever construct a Scene/WorkSpace
 * for anything but a QuickCircuitBuilder-shaped role (add elements, connect, get a Simulation) --
 * SystemVerilogCodeGen itself takes a plain QVector<GraphicElement*>, not a Scene.
 */
