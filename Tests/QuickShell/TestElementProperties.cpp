...
void TestElementProperties::testLabelPersistence()
{
    ...
    QuickWorkSpace workspace1;
    auto *andGate = new And();
    const QString label = "TestLabel123";
    andGate->setLabel(label);
    workspace1.canvas()->addItem(andGate);
    ...
    const auto elements = workspace2.canvas()->elements();
...
void TestElementProperties::testPropertiesSaveLoad()
{
    ...
    led->setColor("Blue");
    workspace1.canvas()->addItem(led);

    auto *clock = new Clock();
    clock->setFrequency(50.0);
    clock->setDelay(0.5);
    clock->setLabel("SavedClock");
    workspace1.canvas()->addItem(clock);
    ...
    auto elements = workspace2.canvas()->elements();
