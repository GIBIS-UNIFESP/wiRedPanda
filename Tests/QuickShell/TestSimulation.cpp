...
// This file builds circuits directly via QuickCircuitBuilder -- there is no Scene/WorkSpace.
// Heap-allocated elements are wrapped in builder.addOwnedElement() so the builder owns them;
// stack-allocated elements go through the non-owning add()/addElement(). sortByTopology() here
// is ElementGraphUtils::sortByTopology(), the free-function twin of Scene::sortByTopology().
// inputStatus()/cpuComponentsDir() come from QuickTestUtils: don't add local copies of these in
// this file, since another file bringing QuickTestUtils::inputStatus into scope via `using` in
// the same unity translation unit would then make the call ambiguous.
using QuickTestUtils::inputStatus;
using QuickTestUtils::cpuComponentsDir;
...
void TestSimulation::testTopologicalSorting()
{
    ...
    const auto elements(sortByTopology(builder.elements()));
...
void TestSimulation::testElementProcessingOrderConsistency()
{
    ...
    verifyDeterministicAcrossRuns<int>(10, "Element processing order", []() -> QVector<int> {
        ...
        builder.add(&button1, &button2, &button3, &and1, &and2, &or1, &not1, &led);
...
void TestSimulation::testSceneInitializationDeterminism()
{
    ...
    verifyDeterministicAcrossRuns<int>(10, "Scene elements order", []() -> QVector<int> {
        ...
        builder.addElement(&button1);
        builder.addElement(&andGate);
        builder.addElement(&button2);
        builder.addElement(&led);
...
void TestSimulation::testSimulationOutputReproducibility()
{
    ...
    verifyDeterministicAcrossRuns<bool>(5, "Simulation output", []() -> QVector<bool> {
        ...
        builder.add(&button1, &button2, &andGate, &led);
...
void TestSimulation::testSimulationGraphStability()
{
    ...
    verifyDeterministicAcrossRuns<std::pair<int, int>>(10, "Simulation graph", []() -> QVector<std::pair<int, int>> {
        ...
        builder.add(&input1, &input2, &input3, &and1, &and2, &or1, &not1, &not2, &led1, &led2);
...
void TestSimulation::testCircuitWithFeedbackLoops()
{
    ...
    verifyDeterministicAcrossRuns<bool>(5, "Feedback circuit", [&firstRunOutputs]() -> QVector<bool> {
        ...
        builder.add(&setBtn, &resetBtn, &nand1, &nand2, &qLed, &notQLed);
...
void TestSimulation::testWirelessTxRxPropagation()
{
    ...
    InputSwitch sw;
    auto *tx = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rx = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    Led led;
...
void TestSimulation::testWirelessMultiRxFanOut()
{
    ...
    InputSwitch sw;
    auto *tx = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rx1 = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rx2 = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    Led led1, led2;
...
void TestSimulation::testWirelessInsideIC()
{
    ...
    const QString icFile = cpuComponentsDir() + "wireless_passthrough.panda";
...
void TestSimulation::testWirelessDuplicateTxIgnored()
{
    ...
    InputSwitch sw1, sw2;
    auto *tx1 = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *tx2 = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rx  = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    Led led;
...
void TestSimulation::testWirelessMultipleIndependentChannels()
{
    ...
    InputSwitch sw1, sw2;
    auto *txA = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rxA = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *txB = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rxB = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    Led led1, led2;
...
void TestSimulation::testWirelessNoneModeNodeDoesNotInterfere()
{
    ...
    auto *tx   = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *rx   = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    auto *none = qobject_cast<Node *>(builder.addOwnedElement(ElementFactory::buildElement(ElementType::Node)));
    Led led;
...
void TestSimulation::testWirelessOrphanedTxDoesNotCrash()
{
    ...
    builder.add(&sw1, &sw2, &sw3, &andGate, &led);
...
void TestSimulation::testWirelessOverridesPhysicalWire()
{
    ...
    builder.add(&swPhys, &swWireless, &led);
...
void TestSimulation::testWirelessFeedbackLoop()
{
    ...
    builder.add(&sw, &andGate, &notGate, &led);
