...
// Local, Quick-native equivalents of the two Tests/Common/TestUtils.h helpers this file uses --
// TestUtils.h itself pulls in App/Scene/Scene.h/Workspace.h, which no longer build.
...
void TestSimulation::testTopologicalSorting()
{
    ...
    const auto elements(CodeGenUtils::sortByTopology(builder.elements()));
...
void TestSimulation::testSceneInitializationDeterminism()
{
    ...
    for (int run = 0; run < NUM_RUNS; ++run) {
        ...
        builder.add(&button1, &andGate, &button2, &led);
...
void TestSimulation::testWirelessInsideIC()
{
    ...
    QuickCircuitBuilder builder;
    auto *ic = builder.addOwnedElement(new IC());
    ...
    try {
        static_cast<IC *>(ic)->loadFile(icFile, QFileInfo(icFile).absolutePath());
    ...
    for (auto *elm : static_cast<IC *>(ic)->internalElements()) {
