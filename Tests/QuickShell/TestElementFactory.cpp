...
{
    // Verify elements can be built and have correct metadata type
...
void TestElementFactory::testItemRegistry()
{
    ...
    canvas.addItem(elem.get());
    const int id = elem->id();
    QVERIFY2(id > 0, "Element ID should be positive after adding to scene");

    // Verify scene registry contains item
    QVERIFY2(canvas.itemById(id) != nullptr, qPrintable(QString("Registry missing ID: %1").arg(id)));
    QCOMPARE(canvas.itemById(id), elem.get());

    // Remove and verify. removeItem() only unregisters (mirrors QGraphicsScene::removeItem()'s
    // ownership-transfer-back-to-caller semantics -- it never deletes) -- elem's own unique_ptr
    // still owns and will delete it.
    canvas.removeItem(elem.get());
    QVERIFY2(canvas.itemById(id) == nullptr, "Item should be removed from registry after removal from scene");
}
...
void TestElementFactory::testCrossSceneIsolation()
{
    ...
    // Add two elements to canvas1 (IDs 1 and 2)
    auto *e1a = ElementFactory::buildElement(ElementType::And);
    auto *e1b = ElementFactory::buildElement(ElementType::And);
    canvas1.addItem(e1a);
    canvas1.addItem(e1b);

    // Add one element to canvas2 — its ID will also be 1 (independent counter)
    ...
    QCOMPARE(canvas1.itemById(s1_id1), e1a);
    QCOMPARE(canvas1.itemById(s1_id2), e1b);
    QCOMPARE(canvas2.itemById(s2_id1), e2a);

    // (a) canvas1 resolves ID 1 to e1a, NOT to e2a
    QVERIFY2(canvas1.itemById(1) != e2a,
             "canvas1 must resolve ID 1 to its own element, not canvas2's");

    // (a) canvas2 resolves ID 1 to e2a, NOT to e1a
    QVERIFY2(canvas2.itemById(1) != e1a,
             "canvas2 must resolve ID 1 to its own element, not canvas1's");

    // (b) canvas2 returns null for ID 2 which only exists in canvas1
    QVERIFY2(canvas2.itemById(s1_id2) == nullptr,
             "canvas2 must return null for IDs that only exist in canvas1");
}

void TestElementFactory::testSceneCounterIndependence()
{
    // Each scene maintains its own monotonic ID counter starting from 1.
    // Adding elements to canvas1 must not advance canvas2's counter.
    CanvasItem canvas1(nullptr, /*buildDemo=*/false);
    CanvasItem canvas2(nullptr, /*buildDemo=*/false);

    // Add several elements to canvas1
    for (int i = 0; i < 5; ++i) {
        auto *e = ElementFactory::buildElement(ElementType::And);
        canvas1.addItem(e);
    }

    // canvas1 counter is at 5; canvas2 counter is still at 0
    auto elm2 = std::unique_ptr<GraphicElement>(ElementFactory::buildElement(ElementType::Or));
    canvas2.addItem(elm2.get());

    // canvas2 should start its own counter from 1, not from 6
    QCOMPARE(elm2->id(), 1);
    // CanvasItem::~CanvasItem() qDeleteAll()s every still-registered element; elm2's unique_ptr
    // must release its own claim first (locals destruct in reverse declaration order, so elm2
    // would otherwise delete the element before canvas2's destructor runs, then canvas2's
    // qDeleteAll() would double-free the same, already-dangling pointer).
    elm2.release();
}
