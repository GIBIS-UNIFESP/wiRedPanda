...
#include "Tests/QuickShell/IC/Cpu8bitIsa.h"
...
struct MultiCycleCpu8bitFixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 8; ++i) {
            auto *pa = new InputSwitch(); builder->addOwnedElement(pa); progAddr.append(pa);
            auto *pd = new InputSwitch(); builder->addOwnedElement(pd); progData.append(pd);
            auto *rpd = new InputSwitch(); builder->addOwnedElement(rpd); regProgData.append(rpd);
        }
        for (int i = 0; i < 3; ++i) {
            auto *rpa = new InputSwitch(); builder->addOwnedElement(rpa); regProgAddr.append(rpa);
        }
        ...
        for (int i = 0; i < 2; i++) {
            auto *c = new Led(); builder->addOwnedElement(c); counterLeds.append(c);
        }
