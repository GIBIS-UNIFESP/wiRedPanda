...
struct ExecutionDatapathFixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 8; i++) {
            auto *a = new InputSwitch(); builder->addOwnedElement(a); operandAInputs.append(a);
            auto *b = new InputSwitch(); builder->addOwnedElement(b); operandBInputs.append(b);
            auto *led = new Led(); builder->addOwnedElement(led); resultLeds.append(led);
        }
        for (int i = 0; i < 3; i++) {
            auto *sw = new InputSwitch(); builder->addOwnedElement(sw); opcodeInputs.append(sw);
        }

        zeroLed = new Led(); builder->addOwnedElement(zeroLed);
        signLed = new Led(); builder->addOwnedElement(signLed);
...
void TestLevel7ExecutionDatapath::testInputPortIsolation_data()
{
...
void TestLevel7ExecutionDatapath::testInputPortIsolation()
{
    QFETCH(int, bitPosition);

    auto &f = *s_level7ExecDatapath;

    // AND (OpCode 010): a one-hot OperandA gated against OperandB = 0xFF must
    ...
    setMultiBitInput(f.opcodeInputs, 2);
...
void TestLevel7ExecutionDatapath::testOutputPortIsolation_data()
{
    QTest::addColumn<int>("bitPosition");
    for (int i = 0; i < 8; ++i) {
...
void TestLevel7ExecutionDatapath::testOutputPortIsolation()
{
    QFETCH(int, bitPosition);

    auto &f = *s_level7ExecDatapath;

    // OR (OpCode 011) with OperandB = 0x00: a one-hot OperandA drives exactly
    // one result bit; every other result bit must stay low.
    setMultiBitInput(f.operandBInputs, 0x00);
    for (int i = 0; i < 8; ++i) {
        f.operandAInputs[i]->setOn(i == bitPosition);
    }
    setMultiBitInput(f.opcodeInputs, 3);
