...
//
// NOTE: master's initTestCase()/cleanup() only called TestUtils::setupTestEnvironment() (a
// no-op here beyond what runQuickTestSuite() already does process-wide, see
// Tests/QuickShell/Runners/QuickRunnerUtils.h) and did nothing, respectively -- dropped.
...
void TestCPUProgramCounter::testProgramCounter()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildProgramCounter8bit(loadValueBits.data(), load, inc, reset, clock, pcBits.data()));
...
void TestCPUProgramCounter::testProgramCounter_data()
{
...
void TestCPUProgramCounter::testProgramCounterPriority()
{
    ...
    clock = new InputSwitch();
    std::unique_ptr<QuickCircuitBuilder> builder(buildProgramCounter8bit(loadValueBits.data(), loadSwitch, incSwitch, resetSwitch, clock, pcBits.data()));
    auto *sim = builder->simulation();
    // Set up load value
    for (int i = 0; i < 8; i++) {
        bool bit = (loadValue >> i) & 1;
