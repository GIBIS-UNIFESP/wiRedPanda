...
// (same convention already applied for TestCPUAlu/TestCPUBranch).
...
void TestCPUControlUnit::testControlUnit()
{
    ...
    aluEnable = new Led();
    flagWrite = new Led();
    std::unique_ptr<QuickCircuitBuilder> builder(buildControlUnit(reset, clock, state.data(), pcInc, irLoad, regWrite, aluEnable, flagWrite));
...
void TestCPUControlUnit::testControlUnitFlagWrite()
{
    ...
    QCOMPARE(inputStatus(flagWrite, 0), false);
    ...
    QCOMPARE(inputStatus(flagWrite, 0), true);  // FlagWrite asserted!
