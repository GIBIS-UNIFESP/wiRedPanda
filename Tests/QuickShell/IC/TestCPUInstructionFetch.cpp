...
// -- same reasoning as TestCPUInstructionExecute: runQuickTestSuite() already does the
// equivalent setup once, globally.
...
void TestCPUInstructionFetch::testInstructionFetch()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildProgramCounter8bit(pcLoadVal.data(), pcLoad, pcInc, pcReset, clock, pcOut.data()));
