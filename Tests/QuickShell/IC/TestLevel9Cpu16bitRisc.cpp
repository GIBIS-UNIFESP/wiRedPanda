...
struct Cpu16bitRiscFixture {
    ...
    bool build()
    {
        ...
        progWrite = new InputSwitch(); builder->addOwnedElement(progWrite);
        regProgWrite = new InputSwitch(); builder->addOwnedElement(regProgWrite);
        for (int i = 0; i < 8; i++) {
            progAddr[i] = new InputSwitch(); builder->addOwnedElement(progAddr[i]);
        }
        for (int i = 0; i < 16; i++) {
            progData[i] = new InputSwitch(); builder->addOwnedElement(progData[i]);
        }
        for (int i = 0; i < 5; i++) {
            regProgAddr[i] = new InputSwitch(); builder->addOwnedElement(regProgAddr[i]);
        }
        for (int i = 0; i < 16; i++) {
            regProgData[i] = new InputSwitch(); builder->addOwnedElement(regProgData[i]);
        }

        for (int i = 0; i < 8; i++) {
            auto *p = new Led(); builder->addOwnedElement(p); pcLeds.append(p);
        }
        for (int i = 0; i < 16; i++) {
            auto *r = new Led(); builder->addOwnedElement(r); resultLeds.append(r);
            auto *il = new Led(); builder->addOwnedElement(il); instrLeds.append(il);
        }
        for (int i = 0; i < 5; i++) {
            auto *o = new Led(); builder->addOwnedElement(o); opcodeLeds.append(o);
        }
...
void TestLevel9CPU16BitRISC::testCPUStructure()
{
    ...
    // Clock + Reset + ProgAddr[8] + ProgData[16] + ProgWrite
...
// overrides PCInc on every one of those clock pulses.
...
// fields. Word layout: [15:11]=OpCode, [10:6]=DestReg, [5:0]=SrcBits.
...
// register file, so every row pre-loads register R1 with the value the
// operand column means, and the instruction's DestReg field is fixed at 1.
...
// The OpCode high bits ([4:3]) select instruction class (level8_decode_stage's
// MemRead/MemWrite derivation): 00=ALU (tested above), 10=Load, 11=Store.
// Setting them to a non-ALU class must stop the ALU's low-3-bits op from
// reaching Result and switch to memory behavior instead.
