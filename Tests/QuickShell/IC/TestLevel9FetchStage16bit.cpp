...
struct FetchStage16bitFixture {
    ...
    bool build()
    {
        ...
        builder->addOwned(clk, reset, pcLoad, pcInc, instrLoad, ic);

        for (int i = 0; i < 8; ++i) {
            pcDataIn[i] = new InputSwitch();
            pc[i] = new Led();
            progAddr[i] = new InputSwitch();
            builder->addOwned(pcDataIn[i], pc[i], progAddr[i]);
        }
        progWrite = new InputSwitch();
        builder->addOwnedElement(progWrite);
        for (int i = 0; i < 16; ++i) {
            instruction[i] = new Led();
            rawInstr[i] = new Led();
            progData[i] = new InputSwitch();
            builder->addOwned(instruction[i], rawInstr[i], progData[i]);
        }
        for (int i = 0; i < 5; ++i) {
            opCode[i] = new Led();
            destReg[i] = new Led();
            builder->addOwned(opCode[i], destReg[i]);
        }
        for (int i = 0; i < 6; ++i) {
            srcBits[i] = new Led();
            builder->addOwnedElement(srcBits[i]);
        }
...
void TestLevel9FetchStage16Bit::testCPUStructure()
{
    ...
    // Clock + Reset + PCLoad + PCInc + InstrLoad + PCData[8] + ProgAddr[8] + ProgData[16] + ProgWrite
...
// fetched word on the next rising edge when InstrLoad=1.
