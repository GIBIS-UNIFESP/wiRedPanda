...
// Fully-wired fetch-stage harness (the inline testFetchStage below leaves the
...
struct FetchStageFixture {
    ...
    bool build()
    {
        ...
        builder->addOwned(ic, clk, reset, pcLoad, pcInc, instrLoad, progWrite);
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            instr[i] = new Led();
            raw[i] = new Led();
            builder->addOwned(pcData[i], progAddr[i], progData[i], pc[i], instr[i], raw[i]);
        }
        for (int i = 0; i < 5; ++i) {
            opcode[i] = new Led();
            builder->addOwnedElement(opcode[i]);
        }
        for (int i = 0; i < 3; ++i) {
            regAddr[i] = new Led();
            builder->addOwnedElement(regAddr[i]);
        }
...
// hold. The level-7 PC/IR hold paths are tested individually, but the
...
void TestLevel8FetchStage::testFetchStageStructure()
{
    ...
    // Outputs: 8 PC + 8 Instruction + 5 OpCode + 3 RegisterAddr + 8 RawInstr = 32 outputs
