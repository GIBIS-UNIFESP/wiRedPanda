...
struct DataForwardingUnitFixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 8; i++) {
            auto *a = new InputSwitch(); builder->addOwnedElement(a); dataAInputs.append(a);
            auto *b = new InputSwitch(); builder->addOwnedElement(b); dataBInputs.append(b);
            auto *c = new InputSwitch(); builder->addOwnedElement(c); dataCInputs.append(c);
            auto *d = new InputSwitch(); builder->addOwnedElement(d); dataDInputs.append(d);
            auto *led = new Led(); builder->addOwnedElement(led); outputLeds.append(led);
        }

        for (int i = 0; i < 2; i++) {
            auto *sw = new InputSwitch(); builder->addOwnedElement(sw); selectInputs.append(sw);
        }
