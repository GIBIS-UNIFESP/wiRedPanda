...
struct ExecuteStageFixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 3; i++) {
            auto *sw = new InputSwitch(); builder->addOwnedElement(sw); aluopInputs.append(sw);
        }
...
void TestLevel8ExecuteStage::testOutputPortIsolation()
{
    ...
    for (int i = 0; i < 8; ++i) {
        QCOMPARE(inputStatus(f.resultLeds[i]), i == bitPosition);
    }
}
...
void TestLevel8ExecuteStage::testExecuteStageStructure()
{
    ...
    // OperandA[8] + OperandB[8] + ALUOp[3] -- the stage is purely combinational
