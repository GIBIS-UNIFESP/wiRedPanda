...
void TestLevel6ALU8Bit::testALU8BitStructure() {
    ...
    // A[8] + B[8] + OpCode[3] + CarryIn + SubCarryIn + ShrIn + ShlIn
    QCOMPARE(f.ic->inputSize(), 23);
    // Result[8] + Zero + Negative + Carry + SubCarryOut
...
// Zero/Negative/Carry flags. CarryIn defaults to 0 and SubCarryIn to 1 (the
// two's-complement +1), so ADD = A+B and SUB = A-B.
