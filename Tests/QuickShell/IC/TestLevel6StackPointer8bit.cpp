...
struct StackPointer8bitFixture {
    ...
    bool build()
    {
        ...
        ic = static_cast<IC *>(builder->addOwnedElement(loadBuildingBlockIC("level6_stack_pointer_8bit.panda")));
        ...
        push = static_cast<InputSwitch *>(builder->addOwnedElement(new InputSwitch()));
        pop = static_cast<InputSwitch *>(builder->addOwnedElement(new InputSwitch()));

        for (int i = 0; i < 8; ++i) {
            loadVal[i] = static_cast<InputSwitch *>(builder->addOwnedElement(new InputSwitch()));
            sp[i] = static_cast<Led *>(builder->addOwnedElement(new Led()));
        }
