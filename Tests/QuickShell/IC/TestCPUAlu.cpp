...
#include "Tests/QuickShell/IC/CpuHelpers.h"
...
//
// NOTE: master's initTestCase()/cleanup() bodies were both empty comments -- dropped.
...
void TestCPUAlu::testALU8bit()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildALU8bit(a.data(), b.data(), op.data(), result.data(), zeroFlag, signFlag, carryFlag, overflowFlag));
...
void TestCPUAlu::testALU8bitFlags()
{
    ...
    bool zeroFlagSet = QuickTestUtils::inputStatus(zeroFlag);
    bool signFlagSet = QuickTestUtils::inputStatus(signFlag);
    bool carryFlagSet = QuickTestUtils::inputStatus(carryFlag);
    bool overflowFlagSet = QuickTestUtils::inputStatus(overflowFlag);
...
void TestCPUAlu::testALU8bitFlags_data()
{
    ...
    QTest::newRow("alu_zero_result")          << 5   << 5   << static_cast<int>(ALU_SUB) << 0   << true  << false;
    // 10 - 20 = 246: borrow (A<B) → carry=0; cin[7]=cout[7]=0 → overflow=0
    QTest::newRow("alu_sign_result")          << 10  << 20  << static_cast<int>(ALU_SUB) << 246 << false << false;
    // 64 + 64 = 128: cin[7]=1, cout[7]=0 → overflow=1 (+64 + +64 overflows signed range); no unsigned carry → carry=0
    QTest::newRow("alu_add_pos_overflow")     << 64  << 64  << static_cast<int>(ALU_ADD) << 128 << false << true;
    // 127 - (-1) = 128: A=0x7F, B=0xFF, NOT(B)=0x00, cin=1; cin[7]=1, cout[7]=0 → overflow=1; borrow → carry=0
    QTest::newRow("alu_sub_pos_overflow")     << 127 << 255 << static_cast<int>(ALU_SUB) << 128 << false << true;
    // 0xFF + 0x01 = 0: cin[7]=1, cout[7]=1 → overflow=0 (−1+1=0, no signed overflow); unsigned carry → carry=1
    QTest::newRow("alu_add_carry_no_overflow") << 255 << 1  << static_cast<int>(ALU_ADD) << 0   << true  << false;
}
...
void TestCPUAlu::testALU8bit_data()
{
    ...
    QTest::addColumn<int>("expectedResult");
    // AND operations
    QTest::newRow("alu_and_0xF0_0x0F") << 0xF0 << 0x0F << static_cast<int>(ALU_AND) << 0x00;
    QTest::newRow("alu_and_0xFF_0xF0") << 0xFF << 0xF0 << static_cast<int>(ALU_AND) << 0xF0;
    // OR operations
    QTest::newRow("alu_or_0xF0_0x0F") << 0xF0 << 0x0F << static_cast<int>(ALU_OR) << 0xFF;
    QTest::newRow("alu_or_0x00_0x00") << 0x00 << 0x00 << static_cast<int>(ALU_OR) << 0x00;
    // XOR operations
    QTest::newRow("alu_xor_0xAA_0x55") << 0xAA << 0x55 << static_cast<int>(ALU_XOR) << 0xFF;
    QTest::newRow("alu_xor_0xFF_0xFF") << 0xFF << 0xFF << static_cast<int>(ALU_XOR) << 0x00;
    // ADD operations (simplified)
    QTest::newRow("alu_add_5_3") << 5 << 3 << static_cast<int>(ALU_ADD) << 8;
    QTest::newRow("alu_add_0xFF_1") << 0xFF << 1 << static_cast<int>(ALU_ADD) << 0x00;
