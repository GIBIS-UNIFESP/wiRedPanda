...
struct RegFile32x16Fixture {
    ...
    bool build()
    {
        ...
        we = new InputSwitch(); builder->addOwnedElement(we);
        clk = new InputSwitch(); builder->addOwnedElement(clk);

        for (int i = 0; i < 5; ++i) {
            auto *w = new InputSwitch(); builder->addOwnedElement(w); writeAddr.append(w);
            auto *r = new InputSwitch(); builder->addOwnedElement(r); readAddr1.append(r);
        }
        for (int i = 0; i < 16; ++i) {
            auto *d = new InputSwitch(); builder->addOwnedElement(d); dataIn.append(d);
            auto *led = new Led(); builder->addOwnedElement(led); readData1.append(led);
        }
