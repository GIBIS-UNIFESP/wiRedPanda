...
struct StackMemoryInterfaceFixture {
    ...
    bool build()
    {
        ...
        builder->addOwned(spLoad, spPush, spPop, spReset, addressSelect,
                    memWrite, clk, ic);
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            builder->addOwned(dataInSwitches[i], addressSwitches[i], spOut[i], dataOutLeds[i], finalAddrLeds[i]);
        }
