...
struct CpuPC8bitFixture {
    ...
    bool build()
    {
        ...
        ic = static_cast<IC *>(builder->addOwnedElement(loadBuildingBlockIC("level7_cpu_program_counter_8bit.panda")));
        ...
        for (int i = 0; i < 8; i++) {
            auto *addr = static_cast<Led *>(builder->addOwnedElement(new Led()));
