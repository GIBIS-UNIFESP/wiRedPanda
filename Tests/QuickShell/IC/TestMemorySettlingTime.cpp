...
// The simulator runs in functional (zero-delay) mode: every combinational circuit
// settles in a single update() regardless of logic depth, so there is no multi-cycle
// settling to measure. These tests verify what these decoders can actually be
// checked for: correct one-hot decoding.
...
void buildSingleStageDecoder6bit(QuickCircuitBuilder *builder,
                                 InputSwitch*& address,
                                 InputSwitch*& control,
                                 Led*& outputs,
 ...
 * decode: every address bit is wired, so the outputs are one-hot.
 */
...
void buildCascadedDecoder6bit(QuickCircuitBuilder *builder,
                              InputSwitch*& address,
                              InputSwitch*& control,
                              Led*& outputs,
...
void TestMemorySettlingTime::testDecoderSettlingTime_cascaded()
{
    ...
    // The 2-level cascade is a genuine 6-bit decoder: one-hot like the single stage.
    QVERIFY2(isOneHot(outputs, addressValue),
...
void TestMemorySettlingTime::testReadMuxSettlingTime()
{
    ...
    // Both decoder topologies must produce the same one-hot result for a given address.
...
{
    // Verify both decoders decode every tested address correctly (one-hot).
    InputSwitch *addr_ss, *ctrl_ss, *addr_cas, *ctrl_cas;
    Led *out_ss, *out_cas;
