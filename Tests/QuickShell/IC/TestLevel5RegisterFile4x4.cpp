...
struct RegFile4x4Fixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            builder->addOwned(writeAddr[i], readAddr[i], readAddr2[i]);
        }
        for (int i = 0; i < 4; ++i) {
            writeData[i] = new InputSwitch();
            readData[i] = new Led();
            readData2[i] = new Led();
            builder->addOwned(writeData[i], readData[i], readData2[i]);
        }
...
void TestLevel5RegisterFile4X4::testRegisterFileTimingEdges()
{
    ...
    if (timingTest == 0) {
        ...
        // The write commits using the address that was stable at the point the write pulse
        // began -- reg0 gets the write (value 9), and reg1 (the address changed to mid-pulse)
        // is left untouched at its power-on default (0). Changing the address after the pulse
        // starts does not retarget the write.
        ...
        // The write commits using the data that was stable at the point the write pulse
        // began (3), not the value it was changed to mid-pulse (12) -- consistent with the
        // address_change_midwrite case above.
