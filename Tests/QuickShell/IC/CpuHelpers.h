...
#include "Tests/QuickShell/IC/CpuCommon.h"
...
// There is no Scene/WorkSpace here: every element in this file (and its callers) is
// heap-allocated with no manual delete anywhere, so every builder call uses the owning
// builder->addOwned()/addOwnedElement(), never the non-owning add()/addElement().
                                                           ...
                                                           Led* pc[8])
{
    ...
    IC *pcIC = new IC();
    QString icFilePath = QuickTestUtils::cpuComponentsDir() + "level6_program_counter_8bit_arithmetic.panda";
    ...
    builder->addOwned(load, inc, reset, clock, pcIC);
    for (int i = 0; i < 8; i++) {
        builder->addOwned(loadValue[i], pc[i]);
    }
                                                ...
                                                Led* ir[8])
{
    ...
    // Add all elements
    for (int i = 0; i < 8; i++) {
        builder->addOwned(instruction[i], ir[i], irStorage[i], irMuxes[i]);
    }
    builder->addOwned(load, clock);
                                                ...
                                                Led* decodedOpcode[16])
{
    ...
    IC *decoder = new IC();
    builder->addOwned(decoder);

    QString icFilePath = QuickTestUtils::cpuComponentsDir() + "level2_decoder_4to16.panda";
    ...
    for (int i = 0; i < 4; i++) {
        builder->addOwned(opcode[i]);
    }
    for (int i = 0; i < 16; i++) {
        builder->addOwned(decodedOpcode[i]);
    }
                                        ...
                                        Led* pcLoad)
{
    ...
    // Add all elements
    builder->addOwned(decoderJMP, decoderBEQ, decoderBNE, decoderBLT, decoderBGE);
    builder->addOwned(zeroFlag, negativeFlag);
    builder->addOwned(notZero, notNegative);
    builder->addOwned(beqEnable, bneEnable, bltEnable, bgeEnable);
    builder->addOwned(branchOR, pcLoad);
                                ...
                                Led* overflowFlag)
{
    ...
    for (int i = 0; i < 8; i++) {
        builder->addOwned(a[i], b[i], result[i], andGates[i], orGates[i], xorGates[i],
                    resultMuxes[i], notB[i], bMux[i], adderXor[i], adderAnd1[i], adderXor2[i],
                    adderAnd2[i], adderCarryOr[i], adderSum[i]);
        builder->addOwned(carryChain[i]);
    }
    for (int i = 0; i < 3; i++) {
        builder->addOwned(aluOp[i]);
    }
    builder->addOwned(zeroFlag, signFlag, carryFlag, overflowFlag);
    ...
    for (int i = 0; i < 8; i++) {
        IC *selector = new IC();
        builder->addOwned(selector);

        // Determine IC file path
        QString icFilePath = QuickTestUtils::cpuComponentsDir() + "level3_alu_selector_5way.panda";
        QFileInfo fileInfo(icFilePath);
    ...
    Not *zNot = new Not();
    builder->addOwned(zOr0, zOr1, zOr2, zOr3, zOr4, zOr5, zOr6, zNot);
                                    ...
                                    Led* flagWrite)
{
    ...
    // Add all elements
    for (int i = 0; i < 3; i++) {
        builder->addOwned(state[i], stateReg[i], stateMuxes[i], incrementXor[i], carryAnd[i]);
    }
    builder->addOwned(reset, clock, pcInc, irLoad, regWrite, aluEnable);
    if (flagWrite) {
        builder->addOwned(flagWrite);
    }
    ...
    builder->addOwned(notBit1, notBit0, detectState4, gateCarry02);
    ...
    builder->addOwned(gateIncBit0, gateIncBit1, gateIncBit2, notWrap);
    ...
    // Port 0 is tied to GND, not the current Q, so reset actually forces state
    // to 0 rather than merely holding whatever state the register was already in.
    InputGnd *stateGnd = new InputGnd();
    builder->addOwned(stateGnd);
    ...
    // EXECUTE=010, WRITEBACK=011, HALT=100) rather than wired to a raw state bit --
    // e.g. bit 2 alone is set in HALT (100) as well as EXECUTE, so aluEnable needs
    // the full 3-bit decode below to fire only in EXECUTE.
    Not *notBit2cs = new Not();
    builder->addOwned(notBit2cs);
    ...
    builder->addOwned(detectExecuteCs);
    ...
    builder->addOwned(detectWriteback);
    ...
    if (flagWrite) {
        ...
        builder->addOwned(notBit2, detectExecute);
                                         ...
                                         Led* outputs[8])
{
    ...
    QString icFilePath = QuickTestUtils::cpuComponentsDir() + "level2_decoder_3to8.panda";
    QFileInfo fileInfo(icFilePath);
    ...
    // Enable is left unconnected below — it defaults high, so the decoder stays enabled.
                                            ...
                                            Led* readOut[8])
{
    ...
    for (int b = 0; b < 8; b++) {
        ...
        builder->addOwned(registers[b], writeData[b], readOut[b]);
    }
    ...
    builder->addOwned(decoderOut, writeEnable, clock, andGate1, andGate2);
                                     ...
                                     Led* negativeOut)
{
    ...
    // Add all elements
    builder->addOwned(zeroIn, negativeIn, flagWrite, clock);
    builder->addOwned(zeroFF, negativeFF, clockGate);
    builder->addOwned(zeroOut, negativeOut);
                                        ...
                                        Led* readDataB[8])
{
    ...
    builder->addOwned(clockGatedDecoder);

    QString decoderPath = QuickTestUtils::cpuComponentsDir() + "level5_clock_gated_decoder.panda";
    ...
    for (int r = 0; r < 8; r++) {
        ...
        builder->addOwned(registerIC);

        QString regPath = QuickTestUtils::cpuComponentsDir() + "level6_register_8bit.panda";
    ...
    for (int b = 0; b < 8; b++) {
        ...
        builder->addOwned(readMuxesA[b], readMuxesB[b]);
    ...
    builder->addOwned(vcc, reset);

    // Add input/output elements
    for (int i = 0; i < 3; i++) {
        builder->addOwned(writeAddr[i], readAddrA[i], readAddrB[i]);
    }
    for (int b = 0; b < 8; b++) {
        builder->addOwned(writeData[b], readDataA[b], readDataB[b]);
    }
    builder->addOwned(writeEnable, clock);
                                        ...
                                        Led* dataOut[8])
{
    ...
    auto loadMux8to1 = [](const QString &context) -> IC* {
        IC *mux = new IC();
        QString muxFilePath = QuickTestUtils::cpuComponentsDir() + "level2_mux_8to1.panda";
    ...
    for (int addr = 0; addr < 64; addr++) {
        for (int b = 0; b < 8; b++) {
            builder->addOwned(memoryArray[addr][b]);
        }
        builder->addOwned(addressDecoders[addr], writeClockGates[addr]);
    }
    for (int i = 0; i < 6; i++) {
        builder->addOwned(notGatesAddr[i], address[i]);
    }
    // Add remaining address bits (6 and 7) even though unused for 64-location memory
    builder->addOwned(address[6], address[7]);
    for (int b = 0; b < 8; b++) {
        builder->addOwned(dataIn[b], dataOut[b]);
    }
    builder->addOwned(memRead, memWrite, memEnable, clock);
