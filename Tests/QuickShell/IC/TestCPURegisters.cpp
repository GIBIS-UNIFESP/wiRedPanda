...
// (same convention already applied for TestCPUAlu/TestCPUBranch/TestCPUControlUnit/
// TestCPUDecoders).
...
void TestCPURegisters::testFlagRegister()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildFlagRegister(zeroInput, negativeInput, flagWrite, clock, zeroOut, negativeOut));
    ...
    sim->update();
    // Verify outputs
    bool zeroResult = inputStatus(zeroOut, 0);
    bool negativeResult = inputStatus(negativeOut, 0);
...
void TestCPURegisters::testFlagRegisterPersistence()
{
    ...
    sim->update();
    QCOMPARE(inputStatus(zeroOut, 0), true);
    QCOMPARE(inputStatus(negativeOut, 0), false);
...
void TestCPURegisters::testInstructionRegister()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildInstructionRegister8bit(instruction.data(), load, clock, ir.data()));
    auto *sim = builder->simulation();
    ...
    if (shouldLoad) {
        ...
        load->setOn(false);
        sim->update();
        clockCycle(sim, clock);  // Rising edge (with load disabled)
        sim->update();
        clockCycle(sim, clock);  // Falling edge
        sim->update();
    }
    sim->update();
...
void TestCPURegisters::testSingleGatedRegister()
{
    ...
    clock = new InputSwitch();
    std::unique_ptr<QuickCircuitBuilder> builder(buildSingleGatedRegister(writeData, decoderOut, writeEnable, clock, readOut));
    auto *sim = builder->simulation();
