...
struct Level4Comparator4bitFixture {
    ...
    bool build()
    {
        ...
        builder->addOwned(swGtIn, swEqIn, swLtIn);
        ...
        ledEqual = new Led();
        ledLess = new Led();
        builder->addOwned(ledGreater, ledEqual, ledLess);
...
void TestLevel4Comparator4Bit::testComparator4BitCascade()
{
    ...
    f.sim->update();
    QVERIFY(inputStatus(f.ledGreater) && !inputStatus(f.ledEqual) && !inputStatus(f.ledLess));
