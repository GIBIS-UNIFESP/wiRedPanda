...
struct InstrRegister8bitFixture {
    ...
    bool build()
    {
        ...
        clock = new InputSwitch(); builder->addOwnedElement(clock);
        load = new InputSwitch(); builder->addOwnedElement(load);
        ...
        for (int i = 0; i < 5; i++) {
            auto *led = new Led(); builder->addOwnedElement(led); opcodeOutputs.append(led);
        }
        for (int i = 0; i < 3; i++) {
            auto *led = new Led(); builder->addOwnedElement(led); regaddrOutputs.append(led);
        }
