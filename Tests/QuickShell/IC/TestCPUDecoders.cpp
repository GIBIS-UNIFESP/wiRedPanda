...
void TestCPUDecoders::testDecoder3to8()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildDecoder3to8Debug(addr, outputs));
...
void TestCPUDecoders::testInstructionDecoder4to16()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildInstructionDecoder4to16(opcodeBits.data(), decodedOutput.data()));
    auto *sim = builder->simulation();
    ...
    int result = 0;
    for (int i = 0; i < 16; i++) {
        if (inputStatus(decodedOutput[i], 0)) {
            result |= (1 << i);
        }
    }
