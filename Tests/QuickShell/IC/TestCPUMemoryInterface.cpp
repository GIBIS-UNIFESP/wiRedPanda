...
// -- same reasoning as TestCPUInstructionExecute/TestCPUInstructionFetch: runQuickTestSuite()
// already does the equivalent setup once, globally.
...
static void writeAndVerify(int address, int dataToWrite, bool doWrite, int expectedData)
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildMemoryInterface(addr.data(), dataIn.data(), memRead, memWrite, memEnable, clock, dataOut.data()));
