...
// (same convention already applied for TestCPUAlu).
...
void TestCPUBranch::testBranchCondition()
{
    ...
    Led *pcLoad = new Led();
    std::unique_ptr<QuickCircuitBuilder> builder(buildBranchCondition(
    ...
    for (int testIdx = 0; testIdx < 10; ++testIdx) {
        ...
        sim->update();
        bool result = QuickTestUtils::inputStatus(pcLoad, 0);
...
void TestCPUBranch::testBranchIntegration()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder_pc(buildProgramCounter8bit(
    ...
    std::unique_ptr<QuickCircuitBuilder> builder_decoder(buildInstructionDecoder4to16(
    ...
    std::unique_ptr<QuickCircuitBuilder> builder_flags(buildFlagRegister(
    ...
    std::unique_ptr<QuickCircuitBuilder> builder_branch(buildBranchCondition(
