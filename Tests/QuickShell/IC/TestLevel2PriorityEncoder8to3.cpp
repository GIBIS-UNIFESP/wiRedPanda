...
struct PriorityEncoder8to3Fixture {
    ...
    bool build()
    {
        ...
        ei = static_cast<InputSwitch *>(builder->addOwnedElement(new InputSwitch()));
        for (int i = 0; i < 3; ++i) {
            outLeds[i] = static_cast<Led *>(builder->addOwnedElement(new Led()));
        }
        validLed = static_cast<Led *>(builder->addOwnedElement(new Led()));
        eoLed = static_cast<Led *>(builder->addOwnedElement(new Led()));

        ic = static_cast<IC *>(builder->addOwnedElement(loadBuildingBlockIC("level2_priority_encoder_8to3.panda")));
