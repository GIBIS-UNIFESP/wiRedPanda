...
/** \file
 * \brief Quick-native, Scene-free replacement for Tests/Integration/IC/Tests/CpuTestUtils.h's
 * CPUTestUtils namespace.
 */
...
inline int get8BitValue(Led* (&leds)[N]) {
    ...
    return QuickTestUtils::readMultiBitOutput(elements, 0);
}
...
inline IC* loadBuildingBlockIC(const QString &filename) {
    ...
    QString absoluteIcPath = QuickTestUtils::cpuComponentsDir() + filename;
