...
struct ShiftRegisterSipoFixture {
    std::unique_ptr<QuickCircuitBuilder> builder;
    IC *ic = nullptr;
    InputSwitch *clk = nullptr, *serialIn = nullptr;
    Led *dataOut[4] = {};
    ...
    bool build()
    {
        ...
        serialIn = static_cast<InputSwitch *>(builder->addOwnedElement(new InputSwitch()));

        ic = static_cast<IC *>(builder->addOwnedElement(loadBuildingBlockIC("level4_shift_register_sipo.panda")));
...
static std::unique_ptr<ShiftRegisterSipoFixture> s_level4ShiftRegisterSipo;

void TestLevel4ShiftRegisterSIPO::initTestCase()
{
    s_level4ShiftRegisterSipo = std::make_unique<ShiftRegisterSipoFixture>();
    QVERIFY(s_level4ShiftRegisterSipo->build());
}

void TestLevel4ShiftRegisterSIPO::cleanupTestCase()
{
    s_level4ShiftRegisterSipo.reset();
}

void TestLevel4ShiftRegisterSIPO::cleanup()
{
    if (s_level4ShiftRegisterSipo && s_level4ShiftRegisterSipo->sim) {
        s_level4ShiftRegisterSipo->sim->restart();
        s_level4ShiftRegisterSipo->sim->update();
    }
}
...
void TestLevel4ShiftRegisterSIPO::testShiftRegisterSIPO()
{
    ...
    auto &f = *s_level4ShiftRegisterSipo;

    f.clk->setOn(false);
    f.sim->update();

    for (int bit = 0; bit < shiftInSequence.size(); ++bit) {
        f.serialIn->setOn(shiftInSequence[bit]);
        f.sim->update();
