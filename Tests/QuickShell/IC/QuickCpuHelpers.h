...
/** \file
 * \brief Quick-native replacement for Tests/Integration/IC/Tests/Cpu/CpuHelpers.h -- same 11
 * circuit-building helper functions, ported onto QuickCircuitBuilder instead of
 * CircuitBuilder(Scene*)/WorkSpace (see the qtquick-rewrite plan's Widgets-test-parity audit,
 * Phase C/D). Each function now returns std::unique_ptr<QuickCircuitBuilder> instead of
 * std::unique_ptr<WorkSpace> -- QuickCircuitBuilder already serves the identical "own the
 * circuit + own the Simulation" role WorkSpace played here.
 *
 * QuickCircuitBuilder::add()/addElement() are non-owning (unlike the old Scene-backed
 * CircuitBuilder, where every added element became Scene-owned via QGraphicsScene::addItem()
 * semantics) -- every element `new`'d inside these functions (gates, ICs, flip-flops) is
 * registered via addOwnedElement() at the point of construction instead, through the local
 * `own()` lambda each function declares. Caller-supplied elements (the InputSwitch/Led pointer
 * function parameters, owned by the calling test file) stay on the non-owning add()/connect()
 * path unchanged. Getting this split wrong would either leak (owned element never added) or
 * double-register (an owned element also passed to add()) -- QuickCircuitBuilder::addElement()/
 * addOwnedElement() have no dedup check, so a double-add would append the element twice to
 * simulationItems(), corrupting the topological sort. Verified via the TestLevel1DFlipFlop
 * validation port (Phase C) and each ported Level test's real pass/fail.
 */
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildProgramCounter8bit(InputSwitch* loadValue[8],
                                                           InputSwitch* load,
                                                           InputSwitch* inc,
                                                           InputSwitch* reset,
                                                           InputSwitch* clock,
                                                           Led* pc[8])
{
    ...
    auto own = [&](auto *e) { builder->addOwnedElement(e); return e; };
    ...
    // Add caller-supplied elements
    builder->add(load, inc, reset, clock);
    for (int i = 0; i < 8; i++) {
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildInstructionRegister8bit(InputSwitch* instruction[8],
                                                InputSwitch* load,
                                                InputSwitch* clock,
                                                Led* ir[8])
{
    ...
    QVector<Mux *> irMuxes;
    for (int i = 0; i < 8; i++) {
        irStorage.append(own(new DFlipFlop()));
        irMuxes.append(own(new Mux()));
    }

    // Add caller-supplied elements
    for (int i = 0; i < 8; i++) {
        builder->add(instruction[i], ir[i]);
    }
 ...
 * 13-15: Reserved
 */
inline std::unique_ptr<QuickCircuitBuilder> buildInstructionDecoder4to16(InputSwitch* opcode[4],
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildBranchCondition(InputSwitch* decoderJMP,
                                        ...
                                        Led* pcLoad)
{
    ...
    // Flag inversion
    Not *notZero = own(new Not());
    Not *notNegative = own(new Not());

    // Branch enable ANDs (condition evaluation)
    And *beqEnable = own(new And());   // BEQ: decoder[9] AND zeroFlag
    And *bneEnable = own(new And());   // BNE: decoder[10] AND NOT zeroFlag
    And *bltEnable = own(new And());   // BLT: decoder[11] AND negativeFlag
    And *bgeEnable = own(new And());   // BGE: decoder[12] AND NOT negativeFlag

    // Final OR to combine all branch enables
    Or *branchOR = own(new Or());
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildALU8bit(InputSwitch* a[8],
                                InputSwitch* b[8],
                                InputSwitch* aluOp[3],
                                Led* result[8],
                                Led* zeroFlag,
                                ...
                                Led* overflowFlag)
{
    ...
    for (int i = 0; i < 8; i++) {
        andGates.append(own(new And()));
        orGates.append(own(new Or()));
        xorGates.append(own(new Xor()));
        resultMuxes.append(own(new Mux()));
    }
    ...
    for (int i = 0; i < 8; i++) {
        notB.append(own(new Not()));
        bMux.append(own(new Mux()));
        adderXor.append(own(new Xor()));
        adderAnd1.append(own(new And()));
        adderXor2.append(own(new Xor()));
        adderAnd2.append(own(new And()));
        adderCarryOr.append(own(new Or()));
        adderSum.append(own(new Xor()));
        carryChain.append(own(new Or()));  // For multi-bit carries
    }

    // Add caller-supplied elements
    for (int i = 0; i < 8; i++) {
        builder->add(a[i], b[i], result[i]);
    }
    for (int i = 0; i < 3; i++) {
        builder->add(aluOp[i]);
    }
    ...
    Or *zOr0 = own(new Or()); Or *zOr1 = own(new Or()); Or *zOr2 = own(new Or()); Or *zOr3 = own(new Or());
    Or *zOr4 = own(new Or()); Or *zOr5 = own(new Or()); Or *zOr6 = own(new Or());
    Not *zNot = own(new Not());
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildControlUnit(InputSwitch* reset,
                                    InputSwitch* clock,
                                    Led* state[3],
                                    Led* pcInc,
                                    Led* irLoad,
                                    ...
                                    Led* flagWrite)
{
    ...
    for (int i = 0; i < 3; i++) {
        stateReg.append(own(new DFlipFlop()));
        stateMuxes.append(own(new Mux()));
        incrementXor.append(own(new Xor()));
        carryAnd.append(own(new And()));
    }
    ...
    Not *notBit1 = own(new Not());
    Not *notBit0 = own(new Not());
    And *detectState4 = own(new And());  // AND(bit2, NOT(bit1), NOT(bit0))
    And *gateCarry02 = own(new And());   // Second stage: AND(bit2_AND_notBit1, notBit0)
    ...
    And *gateIncBit0 = own(new And());  // bit0_inc AND NOT(wrap_enable)
    And *gateIncBit1 = own(new And());  // bit1_inc AND NOT(wrap_enable)
    And *gateIncBit2 = own(new And());  // bit2_inc AND NOT(wrap_enable)
    Not *notWrap = own(new Not());
    ...
    InputGnd *stateGnd = own(new InputGnd());
    ...
    And *detectExecuteCs = own(new And());
    ...
    And *detectWriteback = own(new And());
    ...
    if (flagWrite) {
        Not *notBit2 = own(new Not());
        And *detectExecute = own(new And());
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildDecoder3to8Debug(InputSwitch* addr[3],
                                         Led* outputs[8])
{
    auto builder = std::make_unique<QuickCircuitBuilder>();
    auto own = [&](auto *e) { builder->addOwnedElement(e); return e; };
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildSingleGatedRegister(InputSwitch* writeData[8],
                                            ...
                                            Led* readOut[8])
{
    ...
    for (int b = 0; b < 8; b++) {
        registers.append(own(new DFlipFlop()));
        builder->add(writeData[b], readOut[b]);
    }

    // Add gating logic: AND(decoderOut, writeEnable, clock) -> gatedClock
    And *andGate1 = own(new And());  // AND(decoderOut, writeEnable)
    And *andGate2 = own(new And());  // AND(result, clock)
    builder->add(decoderOut, writeEnable, clock);
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildFlagRegister(InputSwitch* zeroIn,
                                     ...
                                     Led* negativeOut)
{
    ...
    DFlipFlop *zeroFF = own(new DFlipFlop());
    DFlipFlop *negativeFF = own(new DFlipFlop());

    // Clock gating: AND(flagWrite, clock)
    And *clockGate = own(new And());
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildRegisterBank8x8(InputSwitch* writeAddr[3],
                                        ...
                                        Led* readDataB[8])
{
    ...
    IC *clockGatedDecoder = own(new IC());
    ...
    for (int b = 0; b < 8; b++) {
        readMuxesA.append(own(new Mux()));
        readMuxesB.append(own(new Mux()));
    ...
    InputVcc *vcc = own(new InputVcc());
    InputSwitch *resetSignal = own(new InputSwitch());
    resetSignal->setLabel("Reset");
    Q_UNUSED(vcc);
    ...
    for (int r = 0; r < 8; r++) {
        ...
        builder->connect(resetSignal, 0, reg, "Reset");
    }
    ...
    resetSignal->setOn(true);
    sim->update();
    resetSignal->setOn(false);
    sim->update();
 ...
 */
inline std::unique_ptr<QuickCircuitBuilder> buildMemoryInterface(InputSwitch* address[8],
                                        ...
                                        Led* dataOut[8])
{
    ...
    for (int i = 0; i < 64; i++) {
        And *decoder = own(new And());
        decoder->setInputSize(7);       // 6 address bits + memWrite
        addressDecoders.append(decoder);
        writeClockGates.append(own(new And()));
    }
    for (int i = 0; i < 6; i++) {
        notGatesAddr.append(own(new Not()));
    }
    ...
    for (int b = 0; b < 8; b++) {
        QVector<IC *> bitMuxes;
        for (int g = 0; g < 8; g++) {
            IC *mux = own(loadMux8to1(QString("L0 bit%1 group%2").arg(b).arg(g)));
            bitMuxes.append(mux);
        }
    ...
    for (int b = 0; b < 8; b++) {
        IC *mux = own(loadMux8to1(QString("L1 bit%1").arg(b)));
        readMuxesL1.append(mux);
    }

    // Add caller-supplied elements
    for (int i = 0; i < 6; i++) {
        builder->add(address[i]);
    }
