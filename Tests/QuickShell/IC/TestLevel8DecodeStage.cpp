...
struct DecodeStageFixture {
    ...
    bool build()
    {
        ...
        for (int i = 0; i < 3; i++) {
            auto *led = new Led(); builder->addOwnedElement(led); aluopLeds.append(led);
        }
        regwriteLed = new Led(); builder->addOwnedElement(regwriteLed);
        memreadLed = new Led(); builder->addOwnedElement(memreadLed);
        memwriteLed = new Led(); builder->addOwnedElement(memwriteLed);
        ...
        for (int i = 0; i < 32; i++) {
            auto *led = new Led(); builder->addOwnedElement(led); decodedLineLeds.append(led);
...
void TestLevel8DecodeStage::testDecodeStageStructure()
{
    ...
    // OpCode[5] only -- the stage is purely combinational
