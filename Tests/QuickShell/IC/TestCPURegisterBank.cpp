...
//
// NOTE: master's initTestCase()/cleanup() (setupTestEnvironment() + empty body) both dropped
// -- same reasoning as TestCPUInstructionExecute/TestCPUInstructionFetch/TestCPUMemoryInterface:
// runQuickTestSuite() already does the equivalent setup once, globally.
...
void TestCPURegisterBank::testRegisterBank()
{
    ...
    std::unique_ptr<QuickCircuitBuilder> builder(buildRegisterBank8x8(wAddr.data(), wData.data(), writeEnable, clock, rAddrA.data(), rAddrB.data(), readDataA.data(), readDataB.data()));
