...
/// pulls in App/Scene/ICRegistry.h, which is WIDGETS_ONLY_SOURCES (not linked into
/// test_wiredpanda), and its embedIC() is typed for ICRegistry* rather than
/// CanvasICRegistry*. Bodies are otherwise identical.
QByteArray readPandaFile(const QString &path)
{
    QFile f(path);
...
void embedIcIntoCanvas(IC *ic, const QByteArray &pandaBytes, const QString &blobName,
...
{
    // The Quick test runner already performs environment setup once, globally, before any test
    // class runs (see Tests/QuickShell/Runners/QuickRunnerUtils.h's own "TestUtils::
    // setupTestEnvironment() -- must happen before QGuiApplication/QSettings touch..." comment),
    // so no per-class setup call is needed here.
...
void TestArduino::testCodeStructure()
{
    ...
    qsizetype setupPos = content.indexOf("void setup()");
    qsizetype loopPos = content.indexOf("void loop()");
...
void TestArduino::testWirelessOrphanedRxCodegen()
{
    // An Rx node with no matching Tx must not crash the code generator and
    // should produce a LOW default for the node's output variable.
    auto *rxNode = qobject_cast<Node *>(ElementFactory::buildElement(ElementType::Node));
    QVERIFY(rxNode);
...
void TestArduino::testEmbeddedICGeneration()
{
    ...
    //
    // WorkSpace/Scene/CircuitBuilder (Widgets) -> CanvasItem directly: it owns
    // contextDir()/setContextDir()/icRegistry()/elements() itself, with no separate workspace
    // object needed (same adaptation TestInputElements::testAppearanceReloadsAfterFileModified()
    // already established).
    CanvasItem canvas(nullptr, /*buildDemo=*/false);

    const QString fixtureDir = QuickTestUtils::cpuComponentsDir();
    canvas.setContextDir(fixtureDir);
    ...
    const QByteArray pandaBytes = readPandaFile(fixtureDir + "level2_half_adder.panda");
    QVERIFY2(!pandaBytes.isEmpty(), "Fixture file level2_half_adder.panda must exist");
    embedIcIntoCanvas(ic, pandaBytes, "embedded_adder", fixtureDir, reg);
    ...
    auto wire = [&canvas](GraphicElement *from, int fromPort, GraphicElement *to, int toPort) {
        auto *conn = new Connection();
    ...
    auto code = generateFromElements(canvas.elements());
...
{
    // Mux, Demux, and TruthTable elements must generate Arduino code without exception.
    ...
    const QVector<GraphicElement *> &elements, int maxInputBits)
{
    ...
    // Topological sort and feedback detection
    ...
    QVector<GraphicElement *> sorted(elements);
    std::stable_sort(sorted.begin(), sorted.end(), [&priorities](const auto *a, const auto *b) {
    ...
    for (int i = 0; i < combos; ++i) {
        ...
        for (auto *elm : std::as_const(sorted)) {
            elm->updateLogic();
        }
...
{
    // WorkSpace/Scene/CircuitBuilder (Widgets) -> QuickCircuitBuilder: none of this test
    // actually needs a Scene/CanvasItem, just a flat, simulation-capable element list, which
    // is exactly QuickCircuitBuilder's whole job. addOwned() (not add()) for every element
    // built here: nothing else owns them, matching the pattern already established by
    // TestICUnit.cpp's own addOwnedElement() calls for a freshly loadFile()'d IC.
    ...
    for (int i = 0; i < ic->outputSize(); ++i) {
        auto *led = new Led();
        builder.addOwned(led);
        leds.append(led);
        builder.connect(ic, i, led, 0);
    }

    // Get all elements from the builder
    QVector<GraphicElement *> allElements = builder.elements();

    QVERIFY2(!allElements.isEmpty(), "Builder should contain elements");
    ...
    // Verify pin counts
    int inputPins = static_cast<int>(switches.size());
    int outputPins = static_cast<int>(leds.size());
    int expectedPins = inputPins + outputPins;

    QRegularExpression constIntRegex("const int");
    qsizetype constIntCount = content.count(constIntRegex);
    ...
    qsizetype pinModeCount = content.count("pinMode(");
    ...
    const QString expectedDir = QuickTestUtils::arduinoExpectedDir();
    ...
    if (generateMode) {
        ...
        if (QFileInfo::exists(expectedPath)) {
            ...
            QFAIL(qPrintable(QString("Reference file missing: %1 — run with GENERATE_EXPECTED_ARDUINO=1 to create it").arg(expectedPath)));
        }
    }
...
void TestArduino::testArduinoSequentialMultiCycleCpu8Bit()
{
    ...
    // WorkSpace/Scene/CircuitBuilder (Widgets) -> QuickCircuitBuilder, same reasoning as
    // testArduinoExportHelper() above -- addOwned() for everything built here, nothing else
    // owns it.
    QuickCircuitBuilder builder;
    ...
    builder.addOwned(clk);
    builder.addOwned(rst);
    ...
    // Collect inputs/outputs in add order so the captured vector layout matches the Arduino
    // codegen's input/output pin order exactly.
    QVector<GraphicElement *> allElements = builder.elements();
    QVector<InputSwitch *> scanInputs;
    QVector<Led *> scanOutputs;
    for (auto *elm : std::as_const(allElements)) {
...
{
    if (!qEnvironmentVariableIsSet("GENERATE_EXPECTED_ARDUINO")) {
        QSKIP("Skipped in non-generate mode (no external tools needed)");
    }
    requireLinuxForArduinoTools();
...
void TestArduino::testGeneratedSketchMatchesEngineOnRippleCounter()
{
    ...
    //
    // WorkSpace/Scene (Widgets) -> QuickCircuitBuilder: this needs only a flat element list, a
    // real Connection graph, and a bound Simulation -- exactly QuickCircuitBuilder's whole job,
    // same reasoning as testArduinoExportHelper()/testArduinoSequentialMultiCycleCpu8Bit() above.
    QuickCircuitBuilder builder;
    ...
    auto *led1 = ElementFactory::buildElement(ElementType::Led);
    auto *led2 = ElementFactory::buildElement(ElementType::Led);
    builder.addOwned(clk, ff1, ff2, led1, led2);
    clk->setPos(0, 0); ff1->setPos(120, 0); ff2->setPos(260, 0);
    led1->setPos(400, 0); led2->setPos(400, 80);
    clk->setLabel("CLK");

    const auto wire = [&builder](GraphicElement *a, int ap, GraphicElement *b, int bp) {
        builder.connect(a, ap, b, bp);
    };
    ...
    QVector<GraphicElement *> elements = builder.elements();
