...
void TestElementLogic::test3InputXnor_data()
{
    ...
    // 2-input XNOR gates instead computes a XOR b XOR c: XNOR's "invert" cancels out across
    // the cascade (NOT(NOT(a^b)^c) = a^b^c), so it is not a valid stand-in for the native gate.
