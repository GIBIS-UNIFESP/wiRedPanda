...
void TestMux::testMuxSelection()
{
    ...
    auto *mux = new Mux;
    builder.addOwned(data0, data1, sel, mux);
...
void TestMux::testMuxPainting()
{
    ...
    QVERIFY2(QuickTestUtils::pixmapHasInk(pixmap), "Mux paint() must draw visible pixels");
}
...
void TestMux::testMuxPaintingSelected()
{
    ...
    QVERIFY2(QuickTestUtils::pixmapHasInk(pixmap), "Mux paint() must draw the selection highlight when selected");
}
...
void TestMux::testMuxBigPivotsAtBoundingRectCenter()
{
    ...
    QCOMPARE(mux->pixmapCenter(), mux->boundingRect().center());
}
...
void TestMux::testMuxBigRotationDoesNotDriftInScene()
{
    ...
    const QPointF centerScene = mux->pointToScene(mux->boundingRect().center());

    for (const qreal angle : {90.0, 180.0, 270.0, 0.0}) {
        mux->setRotation(angle);
        QCOMPARE(mux->pointToScene(mux->boundingRect().center()), centerScene);
    }
}
