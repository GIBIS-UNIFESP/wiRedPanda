...
void TestBewavedDolphinGui::testRunResetsICInternalSequentialState()
{
    ...
    // leave a flip-flop nested in an IC holding whatever state the live run left, so the sweep
    // would not be reproducible ("power-on defaults") for hierarchical circuits:
    // IC::resetSimState() recurses into the internals.
    ...
    // during the sweep, so the only way the Q output row can read 0 is the sweep resetting the
    // IC-INTERNAL flip-flop to power-on state; without that it would hold the live-run 1 for
    // the whole sweep.
    std::unique_ptr<BewavedDolphin> dolphin(createDolphin(&ws));
    dolphin->run();
