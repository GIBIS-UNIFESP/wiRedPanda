...
void TestSceneState::testConnectionStateTransitions()
{
    ...
    auto *conn1 = new QNEConnection(nullptr);
    conn1->setStartPort(dynamic_cast<QNEOutputPort *>(input->outputPort(0)));
    ...
    auto *conn2 = new QNEConnection(nullptr);
    conn2->setStartPort(dynamic_cast<QNEOutputPort *>(and1->outputPort(0)));
    conn2->setEndPort(dynamic_cast<QNEInputPort *>(led->inputPort(0)));
    scene.addItem(conn2);
...
void TestSceneState::testMixedElementTypes()
{
    ...
    for (ElementType type : types) {
        ...
        elem->setPos(types.indexOf(type) * 100, 0);
        scene.addItem(elem);
    }
