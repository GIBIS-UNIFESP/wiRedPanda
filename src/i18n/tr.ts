export default {
  // Meta / Layout
  'meta.description':
    'wiRedPanda - Dijital mantık devrelerini öğrenmek ve tasarlamak için ücretsiz, sezgisel bir araç.',

  // Navigation
  'nav.home': 'Ana Sayfa',
  'nav.about': 'Hakkında',
  'nav.download': 'İndir',
  'nav.tryout': 'Çevrimiçi Dene',
  'nav.github': 'GitHub',
  'nav.githubRepo': 'GitHub Deposu',
  'nav.toggleMenu': 'Gezinti menüsünü aç/kapat',
  'nav.apiDocs': 'API Belgeleri',

  // Hero
  'hero.logoAlt': 'wiRedPanda Logosu',
  'hero.tagline': 'Mantık devrelerini öğrenmenin eğlenceli ve kolay yolu',
  'hero.scrollDown': 'Aşağı kaydır',

  // Home page
  'home.title': 'Ana Sayfa',
  'home.intro':
    'wiRedPanda, öğrencilerin, eğitimcilerin ve meraklıların dijital devreler öğrenme yolculuğunda yardımcı olmak için tasarlanmış güçlü ve sezgisel bir araçtır. Kullanıcı dostu arayüzü ve gelişmiş özellikleriyle wiRedPanda, devreleri anlama ve oluşturma sürecini etkileşimli ve ilgi çekici bir deneyime dönüştürür.',
  'home.mainFeatures': 'Temel özellikler',
  'home.feature.simulation.title': 'Gerçek Zamanlı Simülasyon',
  'home.feature.simulation.content':
    'Ayarlamalar yapıp farklı konfigürasyonlar denerken devrelerinizin anında hayata geçtiğini görün.',
  'home.feature.interface.title': 'Sezgisel Arayüz',
  'home.feature.interface.content':
    'Öğeleri sürükleyip bırakın, kablolar bağlayın ve devrelerinizi basit ve verimli şekilde oluşturun.',
  'home.feature.analysis.title': 'Ayrıntılı Analiz',
  'home.feature.analysis.content':
    'Devrelerinizi beWavedDolphin dalga formu simülatörüyle analiz edin, sorunları tespit edip düzeltin.',
  'home.feature.crossplatform.title': 'Çok Platformlu',
  'home.feature.crossplatform.content':
    'Windows, macOS ve Linux için kullanılabilir; herhangi bir cihazda projeleriniz üzerinde çalışabilirsiniz.',
  'home.mission':
    'Teknoloji eğitiminin erişilebilir, ilgi çekici ve pratik olması gerektiğine inanıyoruz. Yazılımımız, öğrenme sürecini basitleştirmek ve gerçekten önemli olana odaklanmanızı sağlamak için geliştirilmiştir: dijital devreleri anlamak ve oluşturmak.',
  'home.screenshot1Alt': 'wiRedPanda devre editörünün ekran görüntüsü',
  'home.screenshot2Alt': "wiRedPanda'da mantık devresi simülasyonunun ekran görüntüsü",
  'home.demoLabel': 'wiRedPanda tanıtım videosu',
  'home.cta':
    "wiRedPanda'yı şimdi indirin ve dijital devreler dünyasını keşfetmeye başlayın. Tüm özelliklerimizi deneyin ve hedeflerinize ulaşmanıza nasıl yardımcı olabileceğimizi görün. São Paulo Federal Üniversitesi'nde zaten kullanılıyor, belki de mantık devreleri dersinizde de kullanılacak!",

  // About page
  'about.title': 'Hakkında',
  'about.heading': 'Proje Hakkında',
  'about.history':
    "Proje, 2015 yılında Dr. Fábio Cappabianco, Davi Morales, Lucas Lellis ve Rodrigo Torres tarafından; mantık devrelerini öğrenmeyi kolay, %100 ücretsiz ve çok platformlu hale getirecek bir yazılım geliştirmek amacıyla başlatıldı. Kullanım kolaylığı temel alınarak oluşturulan yazılım, bugün UNIFESP'te dijital devre eğitiminde ve São José dos Campos devlet okullarının lise öğrencilerine dijital mantık öğreten diğer üniversite projelerinde kullanılmaktadır.",
  'about.campusAlt': 'Unifesp kampüsü',
  'about.contribute':
    "Aktif geliştirme sürecindeyiz; bu nedenle hata bildirmek veya yeni fikirler paylaşmak istiyorsanız GitHub'da bir tartışma başlatın! Yardımınızla yazılımı kesinlikle geliştirebiliriz.",
  'about.team': 'wiRedPanda Ekibi',
  'about.formerDevs': 'Eski Geliştiriciler',

  // Download page
  'download.title': 'İndir',
  'download.heading': 'Yazılımı İndir',
  'download.windows': 'Windows',
  'download.linux': 'Linux',
  'download.macos': 'Mac OS',
  'download.otherOptions': 'Diğer Seçenekler',
  'download.packageManagers':
    "wiRedPanda'yı bazı Linux dağıtımlarında paket yöneticileri aracılığıyla da yükleyebilirsiniz. Bu paketler GitHub releases'taki en güncel sürüm olmayabilir.",
  'download.allReleases': 'Eski sürümler veya ön sürümler dahil tüm sürümler için',
  'download.officialGithub': 'resmi GitHub Releases sayfasını ziyaret edin',
  'download.copiedToClipboard': 'Panoya kopyalandı!',
  'download.copyCommand': '{platform} kurulum komutunu kopyala',
  'download.versionText':
    'wiRedPanda {version} sürümünün ikili dosyalarını buradan indirebilirsiniz:',
  'download.x86_64': 'x86_64',
  'download.arm64': 'ARM64',
  'download.universal': 'Evrensel',
  'download.platformArch': 'Arch tabanlı',
  'download.platformDebian': 'Debian tabanlı',

  // 404 page
  '404.title': '404',
  '404.heading': 'Hay aksi!',
  '404.message': "Karnaugh haritasının 'don't care' bölgesine geldiniz ama endişelenmeyin!",
  '404.submessage':
    'Süreci zaten basitleştirdik — sadece ana sayfaya geri dönün ve her şey yoluna girecek.',
  '404.backHome': 'Ana Sayfaya Dön',

  // Footer
  'footer.unifespAlt': 'UNIFESP logosu',
  'footer.copyright': 'wiRedPanda Ekibi',
  'footer.gibisAlt': 'GIBIS logosu',

  // Theme toggle
  'theme.toggle': 'Karanlık modu değiştir',

  // Language switcher
  'lang.switchLanguage': 'Dil değiştir',
} as const;
