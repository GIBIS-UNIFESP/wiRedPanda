...
class ArithmeticBlocksValidator:
    ...
    def test_basic_adders(self) -> Dict[str, Any]:
        ...
        for adder_name, adder_config in basic_adders.items():
            ...
            for test_case in test_cases:
                ...
                function_result = function(*test_case)
                # Handle both iterable and non-iterable results safely
                try:
                    expected_outputs = list(function_result)  # type: ignore
                except TypeError:
                    # function_result is not iterable, wrap in list
                    expected_outputs = [function_result]
            ...
            test_results[adder_name] = {
                ...
                "accuracy": sum(1 for case in case_results if case["correct"])
                / len(case_results)
                * 100,
            }
    ...
    def _create_half_adder_circuit(self) -> Optional[Tuple[InputIDs, OutputIDs]]:
        ...
        return [cast(ElementID, a_id), cast(ElementID, b_id)], [
            cast(ElementID, sum_out_id),
            cast(ElementID, and_id),
        ]  # Return gate output, not LED
    ...
    def _create_full_adder_circuit(self) -> Optional[Tuple[InputIDs, OutputIDs]]:
        ...
        xor2_id = self.create_element(
            "Xor", xor2_x, xor2_y, "XOR2"
        )  # (A XOR B) XOR Cin = Sum
                ...
                if not case_correct:
                    if not sum_correct:
                        ...
                        if (
                            debug_xor1_id
                            and debug_xor2_id
                            and not actual_sum_bits[0]
                            and expected_sum_bits[0]
                        ):
                            # Debug LSB failure - read intermediate gates for debugging
                            # (Variables for potential future debugging use)
                            pass
    ...
    def _create_ripple_carry_adder_circuit(self, bit_width: int) -> Optional[
        Tuple[
            InputIDs,
            InputIDs,
            Optional[ElementID],
    ...
    def test_subtraction_circuits(self) -> Dict[str, Any]:
        ...
        for sub_name, sub_config in subtraction_tests.items():
            ...
            input_a_ids, input_b_ids, diff_output_ids, borrow_id, _ = circuit_result
            ...
            for a_val, b_val in test_cases_sub:
                ...
                # Set input A bits
                for i, bit_val in enumerate(self._int_to_bits(a_val, bit_width)):
                    if not self.set_input(input_a_ids[i], bit_val):
                        sub_passed = False
                        break

                # Set input B bits
                for i, bit_val in enumerate(self._int_to_bits(b_val, bit_width)):
                ...
                for diff_id in diff_output_ids:
                    ...
                    assert diff_bit is not None, "Diff output returned None"
                ...
                if expected_diff < 0:
                    ...
                    expected_diff_unsigned = expected_diff + (1 << bit_width)
                    ...
                    expected_diff_bits = self._int_to_bits(expected_diff, bit_width)
    ...
    def _create_subtractor_circuit(
        self, bit_width: int
    ) -> Optional[Tuple[List[int], List[int], List[int], Optional[int], List[int]]]:
        ...
        for stage in range(bit_width):
            ...
            diff_out_id = self.create_element(
                "Led", diff_out_x, diff_out_y, f"DIFF{stage}"
            )
        ...
        borrow_not_id = self.create_element(
            "Not", borrow_not_x, borrow_not_y, "NOT_BORROW"
        )
        ...
        if not self.connect_elements(
            cast(ElementID, carry_signals[-1]), 0, cast(ElementID, borrow_not_id), 0
        ):
            return None
        if not self.connect_elements(
            cast(ElementID, borrow_not_id), 0, cast(ElementID, borrow_out_id), 0
        ):
            return None

        # Type assertions and casts for return values
        assert (
            all(x is not None for x in diff_output_ids + diff_gate_ids)
            and borrow_not_id is not None
        )
        return (
    ...
    def _create_full_adder_stage_with_borrow(
        ...
        not_borrow_id = self.create_element(
            "Not", not_borrow_x, not_borrow_y, f"NOT_BORROW_{stage}"
        )
        ...
        assert all(
            x is not None
            for x in [sum_out_id, carry_out_id, not_borrow_id, borrow_out_id]
        )
        ...
        connections = [
            ...
            (
                cast(int, not_borrow_id),
                0,
                cast(int, borrow_out_id),
                0,
            ),  # NOT -> Borrow LED
        ]
        ...
        assert all(
            x is not None
            for x in [sum_out_id, carry_out_id, borrow_out_id, xor2_id, not_borrow_id]
        )
        return (
            cast(ElementID, sum_out_id),
    ...
    def test_comparator_circuits(self) -> Dict[str, Any]:
        ...
        for comp_name, comp_config in comparator_tests.items():
            ...
            if comp_config["comparison"] == "equality":
                equality_result = self._create_equality_comparator_circuit(bit_width)
                ...
                magnitude_result = self._create_magnitude_comparator_circuit(bit_width)
            ...
            for test_case in test_cases_comp:
                ...
                if comp_config["comparison"] == "equality":
                    ...
                    case_correct = (
                        ...
                        and actual_lt == expected_lt
                        and actual_eq == expected_eq
                    )
                    case_result = {
                        "inputs": {"A": a_val, "B": b_val},
                        "expected": {
        ...
        return {
            ...
            "results": test_results,
            "description": (
                "Tests equality and magnitude comparator circuits for CPU"
                " conditional operations"
            ),
        }

    def _create_equality_comparator_circuit(
        self, bit_width: int
    ) -> Optional[Tuple[List[int], List[int], int]]:
        """Create n-bit equality comparator with improved layout:
        ...
        xnor_ids = []
        for i in range(bit_width):
            xnor_x, xnor_y = self._get_grid_position(
                i, 2
            )  # XNOR gates aligned with inputs
        ...
        if bit_width == 1:
            ...
            if not self.connect_elements(
                cast(int, and_tree_result), 0, cast(int, eq_output_id), 0
            ):
                return None
    ...
    def _create_magnitude_comparator_circuit(
        self, bit_width: int
    ) -> Optional[Tuple[List[int], List[int], int, int, int]]:
        """Create n-bit magnitude comparator with ultra-compact horizontal layout.
        ...
        for i in range(bit_width):
            a_x, a_y = self._get_grid_position(
                i * 4, 0
            )  # 4x spacing for adder stage width
            a_id = self.create_element("InputButton", a_x, a_y, f"A{i}")
        ...
        if not self.connect_elements(
            cast(int, borrow_id), 0, cast(int, not_borrow_id), 0
        ):
            return None
        ...
        if not self.connect_elements(
            cast(int, not_borrow_id), 0, cast(int, gt_and_id), 0
        ):
            return None
        if not self.connect_elements(
            cast(int, not_zero_id), 0, cast(int, gt_and_id), 1
        ):
            return None
        ...
        return (
            input_a_ids,
            input_b_ids,
            cast(int, gt_output_id),
            cast(int, lt_output_id),
        ...
        while len(current_signals) > 1:
            ...
            for i in range(0, len(current_signals), 2):
                if i + 1 < len(current_signals):
                    or_x, or_y = self._get_grid_position(
                        start_col + level, start_row + (i // 2)
                    )
                    or_id = self.create_element(
                        "Or", or_x, or_y, f"{prefix}_{level}_{i//2}"
                    )
                    if not or_id:
                        return None
    ...
    def _create_and_tree_positioned(
        self, signal_ids: List[int], prefix: str, start_col: int, start_row: int
    ) -> Optional[int]:
        ...
        while len(current_signals) > 1:
            ...
            for i in range(0, len(current_signals), 2):
                if i + 1 < len(current_signals):
                    and_x, and_y = self._get_grid_position(
                        start_col + level, start_row + (i // 2)
                    )
                    and_id = self.create_element(
                        "And", and_x, and_y, f"{prefix}_{level}_{i//2}"
                    )
                    if not and_id:
                        return None
