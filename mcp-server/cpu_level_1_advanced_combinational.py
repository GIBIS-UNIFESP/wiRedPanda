...
class AdvancedCombinationalValidator:
    def __init__(self) -> None:
        ...
        self.MIN_HORIZONTAL_SPACING = (
            150  # 64px element + margin for labels and connections
        )
        ...
        for gate_name, gate_config in multi_input_tests.items():
            ...
            for i in range(inputs_count):
                ...
                if not input_id:
                    test_results[gate_name] = {
                        "success": False,
                        "error": f"Input {i} creation failed",
                    }
                    all_tests_passed = False
                    break
            ...
            if inputs_count > 2:
                ...
                if not gate_results:
                    test_results[gate_name] = {
                        "success": False,
                        "error": "Multi-input gate cascade failed",
                    }
                    all_tests_passed = False
                    continue
    ...
    def _create_multi_input_gate_cascade(
        self, gate_type: str, input_ids: List[int], num_inputs: int
        ...
        while len(current_inputs) > 1:
            ...
            for i in range(0, len(current_inputs), 2):
                if i + 1 < len(current_inputs):
                    ...
                    gate_id = self.create_element(
                        gate_type, gate_x, gate_y, f"{gate_type}{gate_level}_{i//2}"
                    )
    ...
    def test_compound_boolean_functions(self) -> Dict[str, Any]:
        ...
        for func_name, func_config in compound_functions.items():
            ...
            circuit_result = self._create_compound_function_circuit(
    ...
    def _create_compound_function_circuit(
        self, func_name: str, func_config: dict
    ...
    def _create_parity4_circuit(
        ...
        xor3_id = self.create_element(
            "Xor", xor3_x, xor3_y, "XOR3"
        )  # ((A XOR B) XOR C) XOR D
        output_x, output_y = self._get_grid_position(4, 0)
        output_id = self.create_element("Led", output_x, output_y, "OUT")
        ...
        for test_name, test_config in signal_tests.items():
            ...
            for input_val in [False, True]:
                ...
                if test_name == "buffer_chain":
                    expected_output = (
                        input_val
                        if not test_config["expected_inversion"]
                        else not input_val
                    )
            ...
            test_results[test_name] = {
                ...
                "accuracy": sum(1 for r in condition_results if r["correct"])
    ...
    def _create_buffer_chain_circuit(
    ...
    def _create_signal_integrity_circuit(
        self, gate_levels: int
    ) -> Optional[Tuple[int, List[int]]]:
        ...
        for level in range(gate_levels):
            ...
            supply_id = self.create_element(
                "InputButton", supply_x, supply_y, f"VDD{level}"
            )
        ...
        final_output_id = self.create_element(
            "Led", final_output_x, final_output_y, "OUT"
        )
