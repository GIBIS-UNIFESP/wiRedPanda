...
/// \copydoc (anonymous namespace)::isBareFileName in ICHandler.cpp
...
QuickICHandler::QuickICHandler(QuickAppController *appController, const MCPValidator *validator)
    : QuickBaseHandler(appController, validator)
{
}

QJsonObject QuickICHandler::handleCommand(const QString &command, const QJsonObject &params, const QJsonValue &requestId)
{
...
QJsonObject QuickICHandler::handleCreateIC(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        ...
        QString fullPath = m_appController->currentDir().absoluteFilePath(icFileName);
        ...
        auto *workspace = m_appController->currentTab();
        if (!workspace) {
            return createErrorResponse("No active workspace available", requestId, JsonRpcError::InternalError);
        }

        if (workspace->save(fullPath) == QuickWorkSpace::SaveOutcome::ReadOnlyTarget) {
...
QJsonObject QuickICHandler::handleInstantiateIC(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        ...
        QString fullPath = isAbsolute ? icFileName : m_appController->currentDir().absoluteFilePath(icFileName);
        ...
        if (inlineMode) {
            ...
            icPtr = ic.release();
            canvas->receiveCommand(new CanvasAddItemsCommand({icPtr}, canvas));
        }
...
QJsonObject QuickICHandler::handleListICs(const QJsonObject &, const QJsonValue &requestId)
{
    return tryCommand([&]() -> QJsonObject {
        QJsonArray icsArray;

        QDir currentDir(m_appController->currentDir());
        QStringList filters;
        filters << "*.panda";
...
QJsonObject QuickICHandler::handleEmbedIC(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        ...
        if (blobName.isEmpty()) {
            ...
            // (QuickICHandler::handleExtractIC) — reject path traversal here too, not just
            // there, so a malicious name can't ride through the registry as a stored blob key.
            return createErrorResponse("blob_name must not contain path separators or directory components",
                                       requestId, JsonRpcError::InvalidParams);
        }
...
QJsonObject QuickICHandler::handleExtractIC(const QJsonObject &params, const QJsonValue &requestId)
{
