...
/// \copydoc (anonymous namespace)::statusName in ElementHandler.cpp -- duplicated rather than
/// shared since ElementHandler.cpp itself is Widgets-only (see QuickBaseHandler's own doc
/// comment on why the Quick handler family can't link against it).
...
QuickElementHandler::QuickElementHandler(QuickAppController *appController, const MCPValidator *validator)
    : QuickBaseHandler(appController, validator)
{
}

QJsonObject QuickElementHandler::handleCommand(const QString &command, const QJsonObject &params, const QJsonValue &requestId)
{
...
QJsonObject QuickElementHandler::handleCreateElement(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    // Match the interactive drag-and-drop path: clear the previous selection before adding
    // the new element so it alone ends up selected, rather than accumulating onto every
    // element ever created in this session. Without this, a long MCP-driven build (many
    // create_element calls with nothing else ever clearing selection) leaves the entire
    // circuit selected -- see ElementHandler.cpp's identical comment for the original
    // Widgets-side reasoning (O(N) per call, O(N^2) over the full build).
    canvas->clearSelection();

    try {
        canvas->receiveCommand(new CanvasAddItemsCommand({element}, canvas));
    } catch (const std::exception &e) {
        delete element;
    ...
    // CanvasAddItemsCommand::redo() selects the new element (CanvasCommandUtils::addItems())
    // but that happens inside a QUndoCommand, which has no way to reach CanvasItem's own
    // aggregate selectionChanged() signal -- so, same as addElementFromPalette()/
    // addTourDemoInputSwitch()'s own explicit calls right after their receiveCommand(), this
    // caller has to raise it itself. Without this, ElementEditor (and anything else wired to
    // CanvasItem::selectionChanged, per its own "every selection-mutating method emits this
    // explicitly" doc comment) never learns that create_element left a new element selected.
...
QJsonObject QuickElementHandler::handleDeleteElement(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&] {
        canvas->receiveCommand(new CanvasDeleteItemsCommand({element}, canvas));
        return createSuccessResponse(QJsonObject(), requestId);
    }, "delete element", requestId);
}

QJsonObject QuickElementHandler::handleListElements(const QJsonObject &, const QJsonValue &requestId)
{
    CanvasItem *canvas = currentCanvas();
    if (!canvas) {
        return createErrorResponse("No active circuit scene available", requestId, JsonRpcError::SceneNotAvailable);
    ...
    QJsonArray elements;
    const auto canvasElements = canvas->elements();

    for (const auto *element : canvasElements) {
...
QJsonObject QuickElementHandler::handleMoveElement(const QJsonObject &params, const QJsonValue &requestId)
{
...
QJsonObject QuickElementHandler::handleSetElementProperties(const QJsonObject &params, const QJsonValue &requestId)
{
    if (!validateParameters(params, {"element_id"})) {
        return createErrorResponse("Missing required parameter: element_id", requestId, JsonRpcError::InvalidParams);
    }
    ...
    // Snapshot current state before any mutation so CanvasUpdateCommand can restore it on undo.
    QByteArray oldData;
    {
        ...
        if (auto *node = qobject_cast<Node *>(element)) {
            ...
            if (port) {
                for (auto *conn : port->connections()) {
                    wirelessConnsToDelete.append(conn);
                }
            }
    ...
    if (!newProperties.isEmpty()) {
        ...
        if (canvas) {
            if (needsMacro) {
                canvas->undoStack()->beginMacro(QStringLiteral("Change wireless mode"));
            }
            canvas->receiveCommand(new CanvasUpdateCommand({element}, oldData, canvas));
            if (needsMacro) {
                canvas->receiveCommand(new CanvasDeleteItemsCommand(wirelessConnsToDelete, canvas));
                canvas->undoStack()->endMacro();
            }
        }
    }

    // A frequency/delay/locked change above may have moved this element's (if it's a tracked
    // Clock) next deadline earlier, later, or removed it entirely -- re-derive Simulation's
    // scheduled wake so it doesn't fire on a now-stale deadline. Same reasoning as
    // QuickElementEditor::applyProperty()'s identical call; cheap and correctly a no-op when
    // nothing relevant changed or the simulation isn't running.
...
QJsonObject QuickElementHandler::handleSetInputValue(const QJsonObject &params, const QJsonValue &requestId)
{
...
QJsonObject QuickElementHandler::handleGetOutputValue(const QJsonObject &params, const QJsonValue &requestId)
{
    if (!validateParameters(params, {"element_id"})) {
        return createErrorResponse("Missing required parameter: element_id", requestId, JsonRpcError::InvalidParams);
    }
    ...
    auto *inputElement = dynamic_cast<GraphicElementInput *>(element);
    if (inputElement) {
        // A source element's own on/off intent, not a simulated port status -- always
        // definitively high/low, never unknown/error.
        result["value"] = inputElement->isOn();
        result["status"] = inputElement->isOn() ? QStringLiteral("high") : QStringLiteral("low");
    } else {
...
QJsonObject QuickElementHandler::handleRotateElement(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        canvas->receiveCommand(new CanvasRotateCommand({element}, angle, canvas));
        QJsonObject result;
        result["element_id"] = element->id();
        result["angle"] = angle;
        return createSuccessResponse(result, requestId);
...
QJsonObject QuickElementHandler::handleFlipElement(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        canvas->receiveCommand(new CanvasFlipCommand({element}, axis, canvas));
        QJsonObject result;
        result["element_id"] = element->id();
...
QJsonObject QuickElementHandler::handleUpdateElement(const QJsonObject &params, const QJsonValue &requestId)
{
...
QJsonObject QuickElementHandler::handleChangeInputSize(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        canvas->receiveCommand(new CanvasChangePortSizeCommand({element}, newSize, canvas, true));
        const QRectF bounds = element->boundingRect();
        QJsonObject result;
        result["element_id"] = element->id();
...
QJsonObject QuickElementHandler::handleChangeOutputSize(const QJsonObject &params, const QJsonValue &requestId)
{
    if (!validateParameters(params, {"element_id", "size"})) {
        return createErrorResponse("Missing required parameters: element_id, size", requestId, JsonRpcError::InvalidParams);
    }
    ...
    return tryCommand([&]() -> QJsonObject {
        canvas->receiveCommand(new CanvasChangePortSizeCommand({element}, newSize, canvas, false));
        const QRectF bounds = element->boundingRect();
        QJsonObject result;
        result["element_id"] = element->id();
        result["new_size"] = newSize;
...
QJsonObject QuickElementHandler::handleToggleTruthTableOutput(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        // CanvasToggleTruthTableOutputCommand flips one cell in the truth table's output column.
        canvas->receiveCommand(new CanvasToggleTruthTableOutputCommand(truthTable, position, canvas));
        QJsonObject result;
        result["element_id"] = element->id();
...
QJsonObject QuickElementHandler::handleMorphElement(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        canvas->receiveCommand(new CanvasMorphCommand(elementsToMorph, targetType, canvas));
        // CanvasMorphCommand preserves element IDs via updateItemId(), so the morphed
