...
QuickSimulationHandler::~QuickSimulationHandler()
{
    // Destroyed synchronously (not necessarily from within a running event loop), so
    // deleteLater() may never fire -- delete directly, mirrors SimulationHandler's own dtor.
...
QJsonObject QuickSimulationHandler::handleCreateWaveform(const QJsonObject &params, const QJsonValue &requestId)
{
    CanvasItem *canvas = currentCanvas();
    if (!canvas) {
        return createErrorResponse("No active circuit scene available", requestId, JsonRpcError::SceneNotAvailable);
    ...
    return tryCommand([&]() -> QJsonObject {
        delete m_persistentDolphin;
        m_persistentDolphin = new QuickDolphinController();
        QuickDolphinController *dolphin = m_persistentDolphin;

        dolphin->createWaveform(canvas);
        dolphin->setLength(duration);
        ...
        if (!inputPatterns.isEmpty()) {
            for (auto it = inputPatterns.begin(); it != inputPatterns.end(); ++it) {
                ...
                const int rowIndex = dolphin->inputRow(inputLabel);
                ...
                for (int col = 0; col < duration; ++col) {
                    ...
                    dolphin->setCellValue(rowIndex, col, value);
                }
            }
        }
        ...
        const auto waveform = dolphin->snapshot(duration);
...
QJsonObject QuickSimulationHandler::handleExportWaveform(const QJsonObject &params, const QJsonValue &requestId)
{
    ...
    return tryCommand([&]() -> QJsonObject {
        ...
        if (format == "txt") {
            ...
            // Public façade mirrors BewavedDolphin::exportToPng()'s own bool-returning, swallow-
            // exceptions shape rather than QuickDolphinController::exportToPng()'s dialog-owning
            // Q_INVOKABLE -- an MCP client supplies its own filename, no dialog involved.
            bool ok;
            try {
                ok = QuickDolphinExporter::exportToPng(m_persistentDolphin->model(), PlotType::Line, filename);
            } catch (...) {
                ok = false;
            }
            if (!ok) {
