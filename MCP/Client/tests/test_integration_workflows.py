...
class IntegrationWorkflowTests(MCPTestBase):
    ...
    async def test_fixed_issues(self) -> bool:
        ...
        if resp.success:
            ...
            error_msg = str(resp.error) if resp.error else ""
            if "Invalid file extension" in error_msg or "extension" in error_msg.lower():
                self.test_results.errors.append(
                    "test_fixed_issues: File extension validation - .panda incorrectly rejected"
                )
                print("❌ .panda extension incorrectly rejected")
            else:
                print(f"❌ File save failed for unexpected reason (not extension): {error_msg}")
