...
class WaveformAnalysisTests(MCPTestBase):
    ...
    async def test_waveform_not_gate_analysis(self) -> bool:
        ...
        if len(mapping) == 3:
            ...
            waveform_params = {
                "duration": 4,
                "input_patterns": {"INPUT": [0, 1, 0, 1]},  # Should produce [1, 0, 1, 0] at output
            }
                    ...
                    if "INPUT" in inputs and "OUTPUT" in outputs:
                        ...
                        if not_gate_correct:
                            ...
                            self.test_results.errors.append(
                                "test_waveform_not_gate_analysis: NOT gate waveform logic incorrect"
                            )
                    else:
                        self.test_results.errors.append(
                            "test_waveform_not_gate_analysis: Missing signals in waveform data"
                        )
                else:
    ...
    async def test_waveform_dlatch_sequential_behavior(self) -> bool:
        ...
        if len(mapping) == 5:
            ...
            waveform_params = {
                "duration": 6,
                "input_patterns": {"D": [0, 0, 1, 1, 0, 0], "EN": [0, 1, 1, 0, 0, 1]},  # Data changes  # Enable control
            }
                ...
                if waveform_data:
                    ...
                    if "D" in inputs and "EN" in inputs and "Q" in outputs:
                        ...
                        if latch_behavior_correct:
                            ...
                            self.test_results.errors.append(
                                "test_waveform_dlatch_sequential_behavior: D Latch behavior validation failed"
                            )
                    else:
                        self.test_results.errors.append(
                            "test_waveform_dlatch_sequential_behavior: Missing D Latch signals"
                        )
                else:
                    self.test_results.errors.append(
                        "test_waveform_dlatch_sequential_behavior: No D Latch waveform data"
                    )
            # Note: assert_success already handles failure cases
        else:
    ...
    async def test_waveform_export_formats(self) -> bool:
        ...
        if len(mapping) == 5:
            ...
            if resp.success:
                ...
                resp = await self.send_command("export_waveform", {"filename": txt_filename, "format": "txt"})
                ...
                resp = await self.send_command("export_waveform", {"filename": png_filename, "format": "png"})
