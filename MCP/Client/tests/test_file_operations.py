            ...
            if result and "elements" in result:
                ...
                if len(loaded_elements) == len(test_elements):
                    self.infrastructure.output.success(
                        f"✅ Circuit loaded successfully: {len(loaded_elements)} elements"
                    )
    ...
    async def test_file_operation_errors(self) -> bool:
        ...
        for ext in invalid_extensions[:3]:  # Test subset to avoid excessive output
            ...
            try:
                ...
                all_passed &= await self.assert_failure(
                    resp, f"Load with invalid extension '{ext}' (should be rejected)"
                )
                # Clean up dummy file
