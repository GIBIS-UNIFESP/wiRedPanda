...
class SequentialTests(MCPTestBase):
    ...
    async def test_multi_clock_gating_control(self) -> bool:
        ...
        enabled_output = (
            enabled_resp.result.get("value", False) if enabled_resp.success and enabled_resp.result else False
        )
        ...
        disabled_output = (
            disabled_resp.result.get("value", False) if disabled_resp.success and disabled_resp.result else False
        )
    ...
    async def test_power_on_reset_circuit_validation(self) -> bool:
        ...
        por_circuit_spec = {
            ...
            "connections": [
                ...
                {"source": 5, "target": 3, "source_port": 0, "target_port": 0},  # DATA_IN -> STATE_FF1.D
                {
                    ...
                    "source_port": 0,
                    "target_port": 0,
                },  # STATE_FF1.Q -> STATE_FF2.D (shift register)
                ...
                {"source": 4, "target": 7, "source_port": 0, "target_port": 0},  # STATE_FF2.Q -> STATE2
                {
                    ...
                    "source_port": 0,
                    "target_port": 3,
                },  # POR_RESET -> STATE_FF1.~Clear (port 3)
                {
                    "source": 1,
                    "target": 4,
                    "source_port": 0,
                    "target_port": 3,
                },  # POR_RESET -> STATE_FF2.~Clear (port 3)
            ],
        }
        ...
        await self.send_command(
            "set_input_value", {"element_id": element_mapping[1], "value": False}
        ...
        await self.send_command(
            "set_input_value", {"element_id": element_mapping[1], "value": True}
        ...
        reset_release_state1 = (
            state1_resp.result.get("value", False) if state1_resp.success and state1_resp.result else False
        )
        reset_release_state2 = (
            state2_resp.result.get("value", False) if state2_resp.success and state2_resp.result else False
        )
        ...
        await self.send_command(
            "set_input_value", {"element_id": element_mapping[5], "value": True}
        )  # Set data input high
        ...
        await self.send_command(
            "set_input_value", {"element_id": element_mapping[2], "value": True}
        )  # Clock rising edge
        ...
        if not initial_state1 and not initial_state2:
            ...
            print(
                f"📋 Power-on reset limitation: States not cleared (STATE1={initial_state1}, STATE2={initial_state2})"
            )
    ...
    async def test_reset_timing_synchronization(self) -> bool:
        ...
        if not resp.success:
            self.test_results.errors.append(
                "test_reset_timing_synchronization: Reset timing simulation failed to start"
            )
        ...
        for cycle in range(5):
            ...
            await self.send_command(
                "set_input_value", {"element_id": element_mapping[1], "value": not reset_active}
            )  # Reset is active low
        ...
        reset_effective_cycles = sum(
            1 for state in reset_states if state["reset_active"] and not (state["state1"] or state["state2"])
        )
        ...
        if total_reset_cycles > 0:
            ...
            if reset_effectiveness >= expected_min_reset_effectiveness:
                self.infrastructure.output.success(
                    f"✅ Reset timing behavior acceptable: {reset_effectiveness:.1f}% effectiveness"
                )
            else:
                print(
