    ...
    async def test_large_circuit_100_element_chain(self) -> bool:
        ...
        for i in range(100):
            ...
            if i == 0:
                ...
                large_circuit_connections.append(
                    {"source": gate_id - 1, "target": gate_id, "source_port": 0, "target_port": 0}
                )
        ...
        if element_mapping and len(element_mapping) > expected_min_elements:
            self.infrastructure.output.success(
                f"✅ Large circuit created successfully: {len(element_mapping)} elements in {creation_time:.2f}s"
            )
            ...
            if resp.success:
                ...
                for test_val in [True, False, True]:
                    ...
                    await self.send_command("get_output_value", {"element_id": element_mapping[output_id]})
                    await self.send_command("get_output_value", {"element_id": element_mapping[output_id]})
                ...
                if propagation_success_rate == expected_propagation_success:
                    self.infrastructure.output.success(
                        f"✅ Large circuit propagation: {propagation_success_rate:.1f}% success rate"
                    )
                ...
                for test_input in [True, False]:
                    ...
                    for led_id in range(2, 22):  # LEDs have IDs 2-21
                        if led_id in fanout_mapping:
                            await self.send_command("get_output_value", {"element_id": fanout_mapping[led_id]})
                            await self.send_command("get_output_value", {"element_id": fanout_mapping[led_id]})
                            ...
                            output_val = (
                                output_resp.result.get("value", False)
                                if output_resp.success and output_resp.result
                                else False
                            )
