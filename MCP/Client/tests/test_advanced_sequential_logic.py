...
class AdvancedSequentialLogicTests(MCPTestBase):
    ...
    async def test_sequential_sr_latch_validation(self) -> bool:
        ...
        validation_result = await self.validate_sequential_state_table(
            inputs, outputs, clock_id, sr_latch_state_table, "SR Latch"
        )
    ...
    async def test_sequential_dflipflop_validation(self) -> bool:
        ...
        validation_result = await self.validate_sequential_state_table(
            inputs, outputs, clock_id, dff_state_table, "D Flip-Flop"
        )
        all_passed &= validation_result
    ...
    async def test_jk_flipflop_state_table_validation(self) -> bool:
        ...
        if pass_rate == expected_pass_rate:
            ...
            print(
                f"📋 JK flip-flop limitation detected: {pass_rate:.1f}% pass rate (expected {expected_pass_rate:.1f}%)"
            )
    ...
    async def test_tflipflop_divide_by_2_operation(self) -> bool:
        ...
        if element_mapping:
            ...
            if resp.success:
                ...
                initial_state = (
                    initial_resp.result.get("value", False) if initial_resp.success and initial_resp.result else False
                )
        ...
        if element_mapping:
            ...
            if resp.success:
                ...
                for i, scenario in enumerate(test_scenarios):
                    ...
                    await self.send_command(
                        "set_input_value", {"element_id": element_mapping[2], "value": False}
                    )  # CLK=0

                    # Force a clock edge with DATA=0 to reset the flip-flop
                    await self.send_command(
                        "set_input_value", {"element_id": element_mapping[2], "value": True}
                    )  # CLK rising with DATA=0
                    await self.send_command(
                        "set_input_value", {"element_id": element_mapping[2], "value": False}
                    )  # CLK back to 0
                    ...
                    initial_q = (
                        initial_q_resp.result.get("value", False)
                        if initial_q_resp.success and initial_q_resp.result
                        else False
                    )
                    ...
                    if scenario["data_early"]:
                        ...
                        await self.send_command(
                            "set_input_value", {"element_id": element_mapping[1], "value": data_value}
                        )
                    ...
                    final_q = (
                        final_q_resp.result.get("value", False)
                        if final_q_resp.success and final_q_resp.result
                        else False
                    )
                ...
                if timing_accuracy == expected_timing_accuracy:
                    self.infrastructure.output.success(
                        f"✅ Educational timing model: {timing_accuracy:.1f}% consistency"
                    )
        ...
        if element_mapping:
            ...
            if resp.success:
                ...
                for i, (a, b, c) in enumerate(test_vectors):
                    ...
                    output_val = (
                        output_resp.result.get("value", False) if output_resp.success and output_resp.result else False
                    )
                ...
                if logic_success_rate == expected_logic_success:
                    self.infrastructure.output.success(
                        f"✅ Critical path logic: {logic_success_rate:.1f}% success rate"
                    )
