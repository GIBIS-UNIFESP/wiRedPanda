...
class ElementOperationTests(MCPTestBase):
                    ...
                    if result and "elements" in result:
                        ...
                        if remaining_count == expected_remaining:
                            self.infrastructure.output.success(
                                f"✅ Element deletion verified: {remaining_count} elements remaining"
                            )
                            # Note: assert_success already incremented counters
                        else:
                            print(
                                f"❌ Deletion verification failed: expected {expected_remaining}, got {remaining_count}"
                            )
                            all_passed = False
    ...
    async def test_element_management_commands(self) -> bool:
        ...
        try:
            ...
            move_resp = await self.send_command("move_element", {"element_id": elem1_id, "x": 150.0, "y": 150.0})
            ...
            # Test set_element_properties command (label only — port counts use dedicated commands)
            props_resp = await self.send_command(
                "set_element_properties", {"element_id": elem1_id, "label": "MovedAndGate"}
            )
            ...
            if size_resp.success:
                ...
                print(f"❌ change_input_size failed: {size_resp.error}")
                all_passed = False

            # Create a connection for disconnect testing
            connect_resp = await self.send_command(
                "connect_elements", {"source_id": elem1_id, "source_port": 0, "target_id": elem2_id, "target_port": 0}
            )
            ...
            disconnect_resp = await self.send_command(
                "disconnect_elements", {"source_id": elem1_id, "target_id": elem2_id}
            )
