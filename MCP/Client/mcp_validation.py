    ...
    async def verify_connection_exists(self, source_id: int, target_id: int, test_name: str) -> bool:
        ...
        try:
            ...
            for conn in connections:
                if conn.get("source_id") == source_id and conn.get("target_id") == target_id:
                    self.infrastructure.output.success(
                        f"✅ {test_name}: Connection verified ({source_id} -> {target_id})"
                    )
                    return True
    ...
    async def verify_output_value(self, element_id: int, expected_value: bool, test_name: str) -> bool:
        ...
        try:
            ...
            self.test_results.errors.append(
                f"{test_name}: Value mismatch - expected {expected_value}, got {actual_value}"
            )
            return False
        except (ValueError, KeyError, AttributeError, TypeError) as e:
