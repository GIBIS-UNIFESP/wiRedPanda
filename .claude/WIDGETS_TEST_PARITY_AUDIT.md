...
- **`TestICUnit`** (7 tests) — `testICPreviewPopupRespectsDisabledSetting`/`testPreviewPopupClampsToScreen` are **N/A** (`ICPreviewPopup` is Widgets-only, confirmed dead for Quick in this plan's earlier sweep; `QuickICPreview`/`ICPreviewPopup.qml` is the real replacement and may already have its own coverage — not yet checked). The other 5 (`testICLoadFromFile`, `testICPortLabelResolution`, `testICNestedSaveLoad`, `testICInvalidFile`, `testDoubleClickOpensSubCircuitNotInlineEditor`) are portable domain logic, **MISSING**.
- **`TestICRegistry`** (5 tests) — `testMakeBlobSelfContainedRejectsDeepDependencyChain` matches the already-tracked `makeSelfContained()` deferral (`QuickICController.h`'s own doc comment: "no toolbar-button UI trigger exists yet" — deliberate, not dropped). The other 4 (`testICRegistration`, `testICFileWatcher`, `testRecursiveICLoading`, `testICValidation`) are portable domain logic, **MISSING**.
- **`TestExerciseEngine`** (7 tests) — `testOverlayFontScalesWithApplicationFont` is **N/A** (tests Widgets-only `ExerciseOverlay`, confirmed dead for Quick). The other 6 (retranslate/clamp/warn behavior) are pure `ExerciseEngine` domain logic already compiled into `wiredpanda_quick` — **MISSING**, straightforward port.
