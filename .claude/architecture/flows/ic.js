...
flowRegistry['ic_open_sub'] = {
  ...
  nodes: [
        ...
        ['load', 'loadFromBlob(blob,\nparent, elemId, dir)', 'key', 'Deserializes blob into new inline IC scene \u2014 see Workspace drilldown', 'ws_load_blob'],
        ['resize',      'scene->resizeScene()',                 'step',     'Tighten scene rect so viewport doesn\u2019t jump on first interaction'],
        ['ui_cfg', 'Configure inline tab UI:\ntitle "[blobName]"\nhide IC mgmt buttons', 'step', 'pushButtonAddIC, RemoveIC, MakeSelfContained all hidden', 'icreg_self_contained'],
        ['connect', 'connect icBlobSaved \u2192\nparent::onChildICBlobSaved', 'end', 'Ctrl+S in child tab propagates blob updates back to parent', 'ws_child_blob'],
      ],
  edges: [
        ['start',     'emit'],
        ['emit',      'relay'],
...
flowRegistry['ic_embed'] = {
  ...
  nodes: [
        ...
        ['embed', 'ICRegistry::embedICsByFile\n(filePath, fileBytes, blobName)', 'key', 'Atomic: find targets, registerBlob, loadFromBlob each, UpdateBlobCommand. See ICRegistry drilldown.', 'cmd_update'],
...
flowRegistry['ic_extract'] = {
  ...
  nodes: [
        ...
        ['r_file',    'return',                                  'error',    ''],
        ['suffix',    'Append ".panda"\nif missing',             'step',     ''],
        ['extract', 'ICRegistry::extractToFile\n(blobName, fileName)', 'key', 'Atomic write to disk, convert instances, removeBlob. See ICRegistry drilldown. May throw PANDACEPTION.', 'icreg_extract'],
...
flowRegistry['icreg_embed'] = {
  ...
  nodes: [
        ...
        ['cmd', 'new UpdateBlobCommand\n\u2192 undoStack->push', 'end', 'Undo: restore file-backed state from snapshot + rewire connections. Redo: re-apply embedded.', 'cmd_update'],
...
flowRegistry['icreg_extract'] = {
  ...
  nodes: [
        ...
        ['cmd', 'UpdateBlobCommand\n(setOldBlob, setBlobName)\n\u2192 undoStack->push', 'end', 'Undo: re-register blob + restore embedded state. Redo: re-extract + remove blob.', 'cmd_update'],
        ['retN',      'return targets.size()',                             'end',   ''],
      ],
  edges: [
        ['start',     'sf_open'],
...
flowRegistry['icreg_self_contained'] = {
  ...
  nodes: [
        ...
        ['reser', 'Re-serialize blob:\nwritePandaHeader +\nupdated metadata +\noriginal element bytes', 'end', 'blobs[name] = newBlob (updated in-place)', 'ser_header'],
      ],
  edges: [
        ['start',   'd_vis'],
        ['d_vis',   'circ',     'Yes'],
