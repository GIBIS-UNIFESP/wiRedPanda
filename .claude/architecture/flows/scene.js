...
flowRegistry['scene_clipboard'] = {
  ...
  nodes: [
        ...
        ['ser', 'writePandaHeader +\nserializeItems(selected)', 'step', 'Serialize with centroid for paste offset', 'ser_header'],
        ...
        ['d_fmt',    'Has clipboard\nformat?',             'decision', ''],
        ['r_fmt',    'return\n(wrong format)',             'error',    ''],
        ['deser', 'readPandaHeader +\ndeserializeAndAdd', 'key', 'clearSelection, compute offset from centroid to mouse pos, AddItemsCommand, reposition', 'ser_header'],
      ],
  edges: [
        ['copy',     'cm_copy'],
        ['cm_copy',  'd_sel'],
...
flowRegistry['scene_delete'] = {
  ...
  nodes: [
        ...
        ['restart', 'simulation.restart()', 'end', 'Rebuild sim graph without deleted elements', 'sim_cycle'],
      ],
  edges: [
        ['start',  'sel'],
        ['sel',    'd_empty'],
...
flowRegistry['scene_drop'] = {
  ...
  nodes: [
        ...
        ['build', 'ElementFactory::\nbuildElement(type)', 'key', '', 'ef_build'],
...
flowRegistry['scene_mouse'] = {
  ...
  nodes: [
        ...
        ['hover', 'connectionManager\n.updateHover(pos)', 'step', '', 'conn_create'],
        ['d_item',  'Item at\nclick pos?',                 'decision', ''],
        ['d_port',  'Item is\nQNEPort?',                   'decision', ''],
        ['d_editing','Has edited\nconnection?',            'decision', ''],
        ['try_comp', 'connectionManager\n.tryComplete(pos)', 'end', 'Complete or cancel the wire', 'conn_create'],
        ['d_out',   'Port is\noutput?',                    'decision', ''],
        ['start_w', 'startFromOutput(port)', 'end', 'Begin new wire from output port', 'conn_create'],
        ['d_inp_c', 'Input has\nconnections?',             'decision', ''],
        ['start_i', 'startFromInput(port)', 'end', 'Begin new wire from empty input', 'conn_create'],
        ...
        ['ctx', 'contextMenu(screenPos)', 'end', 'Right-click menu', 'ui_menu_dispatch'],
...
flowRegistry['scene_ownership'] = {
  ...
  nodes: [
        ...
        ['connmgr', 'ConnectionManager', 'step', 'Wire creation, validation, hover feedback', 'conn_create'],
...
flowRegistry['scene_mouse_old'] = {
  ...
  nodes: [
        ...
        ['release', 'mouseReleaseEvent', 'step', 'Commit MoveCommand, finish wire, commit selection', 'cmd_move'],
