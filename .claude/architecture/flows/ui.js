...
flowRegistry['ui_ctx_menu'] = {
  ...
  nodes: [
        ...
        ['menu', 'ElementContextMenu::exec()', 'step', 'Build QMenu with context-sensitive actions', 'ui_menu_dispatch'],
        ['d_act',   'User selected\nan action?',              'decision',''],
...
flowRegistry['ui_palette_drag'] = {
  ...
  nodes: [
        ...
        ['drop', 'Scene::dropEvent()\nreceives the drop', 'key', '', 'scene_drop'],
        ['build', 'buildElement(type)\n+ loadFromDrop()', 'step', '', 'ef_build'],
...
flowRegistry['ui_layout'] = {
  ...
  nodes: [
        ['mw',       'MainWindow',              'start',    ''],
        ['palette', 'ElementPalette\n(left sidebar)', 'step', 'ElementTabNavigator tabs. Populates from ElementFactory. Drag elements onto Scene.', 'ui_palette'],
        ['ws',       'Workspace\n(central, tabbed)', 'key',  'GraphicsView \u2192 Scene. One Workspace per open circuit. Tab bar for multiple files + inline ICs.'],
        ['editor', 'ElementEditor\n(right sidebar)', 'step', 'Property panel for selected element. PropertyDescriptor list. Generates UpdateCommand on change.', 'cmd_update'],
        ...
        ['theme', 'ThemeManager', 'step', 'Dark/Light themes, icon sets. SystemThemeDetector for OS detection.', 'ui_theme'],
...
flowRegistry['core_services'] = {
  ...
  nodes: [
    ...
    ['theme',    'ThemeManager',               'key',   'Dark/Light/System theme switching', 'ui_theme'],
